...
Este arquivo registra justificativas, evidência, alternativas e proveniência;
nunca define comportamento W. O Git preserva material removido, mas não é uma
fonte normativa.
...
- emissão do artefato C conservador em modo C23, source-backed a partir de plano HLO0 contra template de texto e witness Restaurant sem caminho source → plano.
...
PYN1 é proveniência histórica e não é uma decisão corrente. Os casos antigos
de header, body implícito, lock e promotion foram removidos do checkout. Eles
permanecem recuperáveis no histórico Git e têm estado `superseded`. Eles não
entram nos gates correntes.
...
Essas diferenças não escolhem um default para W. A baseline compõe
`pipeline<tasks: ...>`,
...
W-1255 rejeita a barreira cíclica genérica: pipeline tasks, domain barrier ou service
...
| WorkQueue, broadcast, watch e weighted channel | **Rejeitado na baseline** | pipeline tasks, Stream, services, tee e SnapshotCell compõem os casos sem esconder loss ou accounting |
...
| barreira cíclica/reutilizável | **Rejeitado** | W-1255 usa pipeline tasks, domain barrier ou service conforme o lifecycle; uma primitive universal esconderia perda, generation e failure |
...
O produto de referência separa os casos. `WrappedDegrees.modify` usa `defer` para
normalizar o valor depois do borrow. O teste de `Attitude` observa o valor
normalizado, e não uma cópia de backing ou um contador oculto. Um limite
estático do forno pertence ao tipo lógico. Um auditor runtime pertence ao
service ou método que possui a authority. Essa divisão evita capture ambiental
e mantém `try` e `await` no call site. Por isso, W aceita somente
`initialValue` e não expõe argumentos, projection ou backing na aplicação do
behavior.
...
acessível no histórico do Git. O commit `4964d1f` contém o material bruto
retirado e `25ef412` é o último estado que ainda continha a árvore histórica.
...
| W-042 | solicitação de cancelamento | `task#cancel(reason:)` é facet core; reasons do caller são separados de deadline, budget e saída estrutural | statement `cancel` (**Rejeitado por enquanto**); budget como reason; async thread cancellation |
...
| W-076 | bootstrap | seed C23 primário explícito; C11 somente recovery/compatibilidade explícita; self-host cedo (o registro histórico de C11 é preservado) | TypeScript/Bun; C++ compiler inteiro |
...
| W-112 | seed output | W0 para subset C23 conservador, backend normal W/MLIR; C11 é recovery explícito | exigir features C11 como default; MLIR completo no seed; C como backend público |
...
| W-686 | source de pipeline (histórico; superseded-by-W-1511) | Registrou `try await pipeline { let ...; return ... }` como DAG estático. A forma corrente é `pipeline` unificada com `commit`; este registro preserva o spelling antigo somente como proveniência. | builder fluente; closure record-replay; promise lazy universal |
...
| W-688 | source de transaction (histórico; superseded-by-W-1511) | Registrou `try await transaction<...> tx = provider { ...; commit value }`. A forma corrente é `pipeline<transaction: { ... }> tx = provider { ...; commit value }`; este registro preserva a keyword antiga somente como proveniência. | method call como forma idiomática; provider implícito; `return` ambíguo; commit manual fora do scope |
...
| W-775 | expressions restritas | `unsafe` produz tail; pipeline usa `commit`; o modo transaction usa `commit`; panic produz Never | block geral como value; pipeline builder first-class; `return` em pipeline/transaction; unsafe desliga checks |
...
| W-1046 | header contextual PYN1 (superseded) | Forma histórica removida do checkout e preservada no Git; não é surface corrente | header `script` em module source, metadata inline ou parser gate corrente |
| W-1047 | fields e requirements PYN1 (superseded) | Forma histórica removida do checkout e preservada no Git; records correntes pertencem a package/workspace | lock standalone, fields de header e requirements inline como surface corrente |
| W-1048 | context standalone PYN1 (superseded) | Forma histórica removida do checkout e preservada no Git; RU0 seleciona package/workspace ou contexto efêmero | header que vence workspace, merge de locks ou discovery ambiental |
| W-1049 | entry de script PYN1 (superseded) | Forma histórica removida do checkout e preservada no Git; RU0 exige descriptor explícito `.default` ou named | `entryForm` implícito, top-level execution ou args implícitos |
...
| W-1051 | dependency record PYN1 (superseded) | Proveniência removida do checkout e preservada no Git; dependencies correntes pertencem a package/workspace | dependency inline em source module, resolver paralelo |
| W-1052 | sources rejeitados em script shareable (superseded) | Proveniência removida do checkout e preservada no Git; source authority corrente pertence a package/workspace | override local oculto ou registry inferido do módulo |
| W-1053 | lock root e virtual selection (superseded) | Proveniência removida do checkout e preservada no Git; RU0 usa resolution aninhada e não aceita lock root | flatten PYN1 custom, constraint sem resolution, root `package.lock` |
...
| W-1057 | requirements de capability PYN1 (superseded) | Proveniência removida do checkout e preservada no Git; requirements correntes pertencem ao módulo/package conforme W-1412 | header `requires` como surface corrente, source grant ou secret inline |
| W-1058 | deployment grant PYN1 (superseded) | Proveniência removida do checkout e preservada no Git; deployment corrente é record nomeado aninhado no workspace | deployment root independente ou grant automático do host |
...
| W-1062 | commands atômicos PYN1 (superseded) | Proveniência removida do checkout e preservada no Git; RU0 usa operações de package/workspace | editar header, lock parcial ou `w script` como surface corrente |
| W-1063 | promotion equivalente PYN1 (superseded) | Proveniência removida do checkout e preservada no Git; RU0 não promove source para package | promotion implícita ou troca de root |
...
| W-1066 | corpus PYN1 (superseded) | Corpus histórico foi removido do checkout e permanece no Git; RU0 usa 12 casos/58 operações | tratar snapshot histórico como gate corrente |
...
| W-1192 | evidence read/write LM0 (retired) | casos anteriores preservam a alternativa em Git/RATIONALE; LM1 testa o contrato vigente | chamar oracle antigo de runtime ou de decisão corrente |
...
| W-1230 | distribuição de trabalho | collections usam `pipeline<tasks: ...>` bounded; streams usam adapters concorrentes/paralelos; services cobrem admission contínua e distribuição entre instances | `WorkQueue` MPMC, receiver compartilhado, child detached, fila ilimitada |
...
| W-1245 | dependency de script explícita (superseded) | Proveniência removida do checkout e preservada no Git; W-1412/W-1415 usam records package/workspace | compact constructor, metadata inline, sibling manifest, `--with` |
...
| W-1255 | barreira cíclica | safe std não inclui primitive genérica; phase local usa `pipeline<tasks: ...>`, epochs usam domain barrier e participantes duráveis usam service/workflow | participant loss implícita, reset de generation silencioso, cancellation sem outcome, barreira universal |
...
| W-1416 | pipeline e labels observáveis | pipeline de atlas usa duas calls dependentes e `commit`; labels demonstram `continue` para loop externo, preservando DAG/driver/cleanup vigentes | pipeline sem dependência ou sem `commit`, continue que reinicia token do label, goto, label em statement arbitrário ou salto não lexical |
...
| W-1478 | aplicação fechada e hook pós-borrow de behavior (superseded-by-W-1501/W-1512) | Registrou a aplicação one-slot `initialValue: fn(): Value` e o lifecycle de `modify`. A forma corrente mantém esse initializer para behavior de storage e reserva `will*`/`did*` a observers nominais explicitamente compostos; observer aplicado diretamente não é aceito. | histórico preservado; DESIGN §10 e `reference/last-light/orbit.w` exercem W-1501/W-1512 |
...
| W-1481 | first-settled de tasks estruturadas | `(take tasks).firstSettled()` consome uma collection de `Task<Value, Failure>` já criada, não cria child nem escolhe domain; `Array` runtime devolve `TaskSettlement?`, enquanto fixed array ou literal nonempty provado devolve `TaskSettlement`; ambos preservam index e `TaskOutcome`. O arm registra todos os handles, candidates já settled usam menor index e o primeiro settlement posterior usa completion order sem desempate por índice; winner fica fixo, losers recebem cancellation e a call aguarda body/cleanup/outcome/drop antes de publicar; parent cancellation observada antes da publicação suprime o settlement, drena todos e permanece control outcome; effects committed não sofrem rollback. O algoritmo é member normal da collection; somente controles imediatos do handle/context usam facet core. Handles heterogêneos/duplicados, statement select, branch/default/fairness implícitos, first-success, future drop e multiplexing persistente ficam fora | current design contract em DESIGN §12.4.1, estudo host FST0, `TaskSettlement` em std.runtime e witness `task_settlement.w`; semantic checker, overload nonempty, runtime wake/CAS, provider conformance, cross-domain fault/liveness tests, performance e estudos humano/modelo continuam implementation-evidence gaps. Fontes primárias de Swift structured concurrency, Tokio select, Go select e Kotlin select foram verificadas em 2026-08-25 |
| W-1482 | famílias TaskGroup map/collect fechadas (superseded-by-W-1511) | Registro histórico preservado: `TaskGroup.concurrentMap`/`parallelMap<domain>` e `concurrentCollect`/`parallelCollect<domain>` usavam labels `limit`, `ordering`, `using`; limitava children vivos, fazia staging/admission/result preflight e mantinha index, settlement, cancellation e drain. A superfície corrente é `pipeline<tasks: ...>` com `tasks`, `limit`, `ordering` e `errors`; os contratos de staging, admission, arbitration, ordering, index, fault/cancellation, drain e O(count) continuam normativos em W-1511. | histórico preservado no estudo TGM0 e nas fontes antigas; não é superfície corrente nem API namespace |
...
| W-1490 | catálogo BMD3 de language e unidade byte-scan-view | W-1490 fecha um catálogo versionado com exatamente 21 unidades required em sete estratos de três, cada uma com owners, escopo, oracle declarado, classes de input, métricas futuras, baselines, readiness, crosspoint separado do Restaurant e stop condition. `byte-scan-view` é a primeira unidade source-oracle-ready: kernel puro conta um delimitador runtime em `view Bytes` binária bounded a 64 MiB e devolve JSON canônico com bytes e matches como u64 decimal. Learner e idiomatic são formas equivalentes; frontier abre somente a estratégia física SIMD com disclosures completos. O oracle host é independente, bounded e completo para a operação, mas W codegen, runtime, language benchmark runner, timings e results continuam bloqueados. C23 e Rust são referências de correção independentes sem ranking agora; C11 permanece somente como recovery/compatibilidade explícita. Após equivalência, seu papel futuro é comparação independente com toolchain e recipe fixos, enquanto W histórico continua primário. Toolchain que só aceita c2x recebe disclosure correctness-only e não produz ranking final C23 | `benchmarks/language-catalog.json`, `benchmarks/byte-scan-view.manifest.json`, fontes W, C23, Rust, `CMakeLists.txt`, oracle e smoke Bun são source-backed. O smoke testa classes, limites, delimitador runtime, rejeições sem stdout parcial e provenance de recipe; toolchain ausente é skip explícito e falha de toolchain presente é erro. `catalog.status: ready` valida o catálogo, enquanto `oracle.status: declared` é contrato e `readiness.oracle` é evidência. A categoria é `oracle-backed-current` |
| W-1491 | plano HLO0 source-backed de Hello World | O frontend v10 recebe um host scope ativo explícito, publica qualifiers, Unit inferido e identidade discriminada de callee; `print` é um símbolo normal do host prelude `native-process@1` com requirement nominal `Console`, não intrinsic nem global. O lowering HIR0 copia os facts necessários e o verifier fecha o programa bounded antes do HLO0. O adapter caller-owned valida somente HIR0 verificada e produz, para `fn main() { print("Hello, world!") }` com `entry(main)`, um plano bounded com payload, LF, stdout de 14 bytes, digest e exit success. A evidência HLO0 termina em source → parser → frontend → HIR0 → plano; HLO0 não emite C nem executa W. `benchmarkDisposition` é deferred para `hlo3-hello-world-runtime-benchmark`, e timing fica proibido neste corte | W-1505 supersede W-1491 quanto ao milestone Hello-only; a evidência histórica permanece: Source, HIR0, adapter e gates HLO0 estão em `compiler/seed-c`; `bun run check:hlo0`, frontend seed, parse focal, HIR0 e CTest provam o subset e suas barreiras. A decisão HLO0 permanece `implementation-evidence-gap` para HIR geral, execução W, Console provider, w-linker, runner e runtime |
...
| W-1524 | typed topological interpolation values in verified HIR0 | HIR0 schema `w-seed-hir0-3` adds canonical `i64` and Bool types, typed scalar and binary values, explicit value ownership, and ordered text/value interpolation segments. Values are postorder, children point backward, parentheses normalize away, and every root is owned by one call argument. Counts, capacities, aliases, bridge, digests, receipt, spans, dense byte coverage, and adversarial verification include the new records. The witness retains `6 * 7`; it does not precompute output. | `source-backed-current` for the bounded frontend→verified-HIR cut. W-1525 adds only the signed-`i64` native subset; other Display domains, general expressions and performance remain gaps. `benchmarkDisposition: compiler-lifecycle`, correctness-only, no timing or result |
| W-1525 | bounded signed-`i64` interpolation through native MLIR | MLIR0 schema `w-seed-mlir0-4` and Native0 schema `w-seed-native0-3` accept the W-1522 linear shape plus interpolated String roots with panic-free signed-`i64` constants and `+`, `-`, `*`, `/`, `%` trees. The selector computes only a safety proof. The artifact retains constants and LLVM arithmetic operations, builds one runtime format, writes through a bounded 4097-byte stack buffer, and has no W `printInt` surface or precomputed output. The Restaurant witness executes exact `Table 42 remains open\n`; tests also execute all operators, a negative result, and `%` data. | `source-backed-current` only for the bounded Linux x86_64 GNU seed route. Target-ABI `snprintf` is temporary seed infrastructure, not the final runtime or a performance claim. Bool/String Display segments, NUL inside interpolated text, panic paths, general expressions, targets, runtime, and performance remain gaps. `benchmarkDisposition: compiler-lifecycle`, correctness-only, no timing or result |
| W-1526 | internal bounded signed-`i64` Display in LLVM dialect | MLIR0 schema `w-seed-mlir0-5` and Native0 schema `w-seed-native0-4` retain the W-1525 arithmetic subset but replace the target-ABI format string with an immutable text bank and internal LLVM-dialect `w_seed_copy` and `w_seed_append_i64` helpers. One 4097-byte stack buffer receives exact counted text and decimal digits before one checked POSIX `write`. Dynamic artifacts contain no `snprintf`, `%ld`, variadic call, `printInt`, or precomputed result. Counted text now preserves NUL and `%`. | `source-backed-current` for the retained bounded signed-`i64` path; W-1528 owns the current adapter schemas and adds typed immutable binding roots. The helpers are private seed details, not a stable W runtime ABI. `benchmarkDisposition: compiler-lifecycle`, correctness-only, no timing or result |
| W-1527 | bounded built-in Bool and String Display through native MLIR | MLIR0 schema `w-seed-mlir0-6` and Native0 schema `w-seed-native0-5` retain signed-`i64` and add `CONST_BOOL`, `CONST_STRING`, and one prior immutable String `BINDING_READ` as interpolation value segments. Frontend v12 now supplies canonical Bool identity and resolves binding reads inside the bounded argument tree; HIR0 v3 already represents and verifies both. Bool emits exact lowercase ASCII through an artifact-private helper present only when used. Compile-time-known String values enter the counted text bank unchanged, including NUL. The Restaurant witness executes exact `Kitchen true/false; table: open\n`. | `source-backed-current` only for the bounded Linux x86_64 GNU seed route. General protocol dispatch, user-defined Display, runtime String/nonconstant Bool, mutable locals, stable runtime ABI, additional targets, and performance remain gaps. `benchmarkDisposition: compiler-lifecycle`, correctness-only, no timing or result |
| W-1528 | typed immutable binding initializers through native MLIR | HIR0 schema `w-seed-hir0-4` replaces the byte-only String binding payload with one typed `initializer_value` root in the common postorder graph. Frontend v12 infers the bounded canonical `i64`, Bool, or String type and publishes an indexed prior binding relation. HIR verifies same-block, strictly earlier, type-equal reads without downstream name lookup. MLIR0 schema `w-seed-mlir0-7` and Native0 schema `w-seed-native0-6` preserve the initializer SSA value; `6 * 7` remains `llvm.mul`. The Restaurant witness executes exact `Table 42; open: true; state: open\n`. | `source-backed-current` only for the bounded single-function, single-block Linux x86_64 GNU seed route. `var`, assignment, nested scopes, general CFG/SSA, runtime String, protocol dispatch, stable runtime ABI, other targets, and performance remain gaps. `benchmarkDisposition: compiler-lifecycle`, correctness-only, no timing or result |
...
cancellation ou drain. `(take tasks).firstSettled()` nomeia a semântica e trabalha sobre
...
No Restaurante, `inspectEveryFailure` agora recebe `TaskOrdering` e devolve
`Array<TaskSettlement<...>>` pelo modo `pipeline<tasks: ...>`. Em `.completion`,
um forno que falha cedo ainda é identificável por `index`; em `.input`, o mesmo
record permite reconstrução estável. O estudo TGM0 também exige preflight do
array de resultado antes dos effects e distingue fail-fast de collection sem
transformar panic em error.
...
representa um provider real por si só. CHK3 acrescenta o scanner C
caller-owned de origins e o frontend seed de identities separadas e edges
resolvidos explícitos. CHK4 acrescenta o graph builder caller-owned. CHK5
acrescenta o core C23 bounded de aquisição/revalidação para requests explícitos
e o adapter Linux real ancorado em `openat2`, com stub fail-closed fora de
Linux. CHK6 acrescenta um driver C23 interno caller-owned de discovery local
iterativo a partir de root explícita, compondo aquisição/revalidação CHK5,
parser/module scan e graph CHK4. O driver entrega documentos em ordem lógica e
imports resolvidos a um caller futuro; não chama frontend nem abre `w check`
público multi-file. As waves não formam uma transação única de snapshot:
candidates antigos podem ser readquiridos e o CHK4 publica somente a
reachability alcançada. A API deriva a root, o mapping root-relative, a ordem
do inventory, os edges, ciclos, colisões, limits e a projeção lógica da recipe.
O digest e o byte count são calculados dos bytes do source. O `sourceText` dos snippets é
evidence do oracle, e a aquisição exige facts estáveis do mesmo provider, root
e source/provider owner token.

CHK7 acrescenta a composição interna caller-owned CHK6→frontend→D0 JSON-only,
com preflight integral dos diagnostics. Todo o trabalho falível termina antes do
commit: o JSONL é copiado uma vez para o buffer final e então `jsonl_length` é
atualizado, sem novo ramo falível. Em qualquer falha esses dois outputs
permanecem inalterados. A fixture prova import e chamada de export de
`root` para `child` e o diagnostic `W-SEM-0001` com source lógico `child.w`, em
ordem determinística; não abre CLI pública, filesystem novo, provider `std`,
package/workspace ou frontend completo.

CHK8 acrescenta a prova bounded do adapter Windows real para o mesmo provider.
A escolha usa `NtCreateFile` com `OBJECT_ATTRIBUTES.RootDirectory` para abrir
cada componente relativo a um `HANDLE` de diretório, `OBJ_DONT_REPARSE` para
impedir que a resolução siga reparse points e `FILE_OPEN_REPARSE_POINT` para
inspecionar o componente final. O adapter confirma `FileAttributeTagInfo`,
obtém a identidade por `FILE_ID_INFO` e rejeita qualquer objeto que não seja
arquivo regular. O perfil suporta root relativa e root absoluta drive-local;
UNC é `UNSUPPORTED`, enquanto namespace, device e ADS inválidos são
`INVALID`.

A documentação Microsoft de [`NtCreateFile`](https://learn.microsoft.com/en-us/windows/win32/api/winternl/nf-winternl-ntcreatefile)
descreve `RootDirectory` e `FILE_OPEN_REPARSE_POINT`. A documentação de
[`OBJECT_ATTRIBUTES`](https://learn.microsoft.com/en-us/windows/win32/api/ntdef/ns-ntdef-_object_attributes)
define `OBJ_DONT_REPARSE`. A documentação de
...
e [`FILE_ID_INFO`](https://learn.microsoft.com/en-us/windows/win32/api/winbase/ns-winbase-file_id_info)
define as consultas de tag e de identidade usadas pelo adapter.

Canonicalização por path, `realpath` ou `CreateFile` permissivo depois da
canonicalização foi rejeitada. Essas formas transformam um nome reaberto em
autoridade separada do handle verificado e deixam uma janela de
time-of-check/time-of-use (TOCTOU) entre resolução, validação e leitura. O
adapter não trata `NtCreateFile` como fallback universal. Se a API, o
filesystem ou a consulta de identidade não fornece o contrato, ele retorna
`UNSUPPORTED` e o core mantém a publicação all-or-nothing.

CHK9 acrescenta a composição pública `w check` para uma root explícita em
contexto efêmero, com imports locais alcançáveis root-relative. Linux exige
`openat2`, Windows exige `NtCreateFile` e outras plataformas ou capabilities
falham fechadas. A root usa basename ASCII `[A-Za-z_][A-Za-z0-9_]*.w` como
`SourceId`; o core/provider aceita diretório físico codificado em UTF-8, e o gate
Windows prova cwd Unicode. Um path Unicode recebido por `argv` narrow não está
provado e permanece gap. Header override altera somente o module path. Storage
de sources, CST e JSON cresce
adaptativamente, com 64 sources, 4096 edges, depth 64, 16 MiB por source e
agregado, CST 32768 por source/262144 agregado e JSON até 64 MiB; cada retry
repete a composição inteira em limite bounded.

O gate CHK9 prova Last Light single-source e Restaurant multifile temporário
com child nested, diagnóstico determinístico, source inalcançado, missing,
`std`, cycle, identidade/UTF-8/parse/frontend, limites e escape por
symlink/junction. JSON usa SourceId lógico, preflight integral e uma única
...
W-1494 fecha a primeira fronteira intermediária que o seed pode verificar sem
manter o frontend vivo. O lowering copia para storage caller-owned os nomes,
identidades e bytes de constantes usados pelo subset. O schema fechado inclui
...
`w_seed_hir0_verify` na própria fronteira e o verifier recompõe um semantic
digest field-by-field com encoding big-endian explícito, sem padding, spans,
source bytes ou provenance. Um provenance digest separado cobre source id,
`module.source_sha256`, comprimento e spans dos records HIR; o receipt
serializa counts, semantic_digest e provenance_digest. Assim,
whitespace e comentários podem alterar provenance sem alterar semantic digest,
enquanto qualquer mutação de record, count, owner, range, type, ordinal,
identity, requirement, terminator, entry, alias, overlap ou receipt falha
fechada.
...
blocks, calls, arguments e values dentro do schema validado. O seletor HLO0
W-1505 aplica uma forma mais estreita somente depois de verificar essa HIR0.

`measure`, `run` e `program_from_output` fazem preflight completo e têm
semântica all-or-nothing. O programa verificado não consulta records frontend,
source ou CST depois do lowering e sobrevive ao lifetime desses buffers. HLO0
recebe apenas `program` e `hir_result`; não há ponte de compatibilidade para o
frontend antigo. O gate integrado prova a composição exata
`source → parser → frontend → lower HIR0 → verify HIR0 → HLO0 → HLO1 →
compilador C (modo C23) → execução`, incluindo o witness Restaurant em comentário,
callee e payload incorretos.

Esta promoção é deliberadamente limitada. O seed ainda não prova HIR geral,
type checking normativo, lowering de outros constructs, backend nativo,
linker, runtime, provider Console geral ou `w run`. O benchmark permanece
deferred até existir um runner W público e recipe pinada; nenhum timing ou
...
A classificação histórica de W-1495 era source-backed-current somente para o
subset bounded; esse milestone Hello-only é superseded por W-1505. A evidência
histórica de W-1495 permanece no caso, units e gate abaixo. O binário público
continua sem `w run`. Aquisição pública ou geral de source,
...
#### W-1505 — subset print-literal input-driven

W-1505 corrige o limite de especialização que ficou explícito nos primeiros
milestones HLO0/HLO1/RUN0. O objetivo não é transformar HLO0 em HIR geral.
HIR0/W-1494 continua uma representação intermediária bounded de schema fechado,
mais ampla que a seleção final; ela pode carregar múltiplas funções e records
de blocks, calls, arguments e values. Sobre uma HIR0 já verificada, o seletor
...
dos textos da HIR0 verificada: não vazios, terminados em NUL, com zero-tail e
iguais byte a byte. O verifier isolado do plano comprova somente essa
representação e igualdade; ele não prova source provenance nem que o conteúdo
é um identifier válido. Profile, slot, callee, requirement, policies, effects
e counts permanecem fechados. O payload é uma faixa de zero a
`W_SEED_HLO0_MAX_PAYLOAD` bytes copiada da HIR0; bytes NUL não são truncados
quando a HIR0 os publicar. O tail não usado é zero, stdout é o payload seguido
por LF com soma checked, e o digest cobre exatamente essa concatenação.
`measure`, `run` e o verifier compartilham o preflight caller-owned, bounded,
fail-closed e all-or-nothing. A provenance source é coberta pela rota integrada
e pela HIR0 verificada, não por este verifier isolado; nenhum decoder ou novo
contrato de identidade é adicionado neste bundle.

HLO1 continua emitindo `w-seed-hlo1-1`, porque o formato C não muda. O array
hexadecimal é derivado do payload verificado e o programa C23 escreve todos os
bytes mais LF. RUN0 stageia a mesma sequência e só chama o sink depois do
preflight. Os gates atravessam a composição real e comparam Hello,
`Table 42 remains open` e vazio byte a byte; trivia não muda o artifact.
Comentário contendo `print`, noop, duas calls e forma fora do subset são
rejeitados em etapas anteriores, sem stdout ou artifact parcial. O caso de
entry com nome diferente é coberto somente se parser/frontend já o suportarem;
nenhuma gramática foi ampliada.

A classificação é `source-backed-current` somente para esse subset bounded.
`benchmarkDisposition` é `compiler-lifecycle`: correctness-only agora, sem
timing ou result. Produto/runtime, backend/MLIR, provider W geral, `w run` e
performance continuam deferred. W-1505 supersede W-1491, W-1493 e W-1495
quanto aos milestones Hello-only e preserva W-1494 current; C23 permanece
primary e C11 recovery explícita. Toolchain ausente dá SKIP explícito, mas
falha de toolchain presente dá FAIL.

#### W-1506 — terminal MLIR0-to-LLVM bridge (historical; superseded by W-1522)

W-1506 is historical. It recorded the first MLIR0 native route with schema
`w-seed-mlir0-1` and an HLO0-plan input. W-1520 replaced that incompatible
pre-1.0 contract with schema-v2 direct verified-HIR consumption, and W-1522
supersedes that schema-v2 route with the bounded sequence contract; no parallel
legacy adapter is retained. The details below preserve the decision's
provenance and are not the current MLIR0 API.
...
somente `x86_64-unknown-linux-gnu` tem evidence neste bundle, e nenhum target
é supported. W dialect, lowering HIR geral, targets adicionais, provider ABI,
linker/runtime, packaging/distribuição MLIR, `w run` e performance são gaps/tasks
W-1506. O `benchmarkDisposition` é `compiler-lifecycle`, correctness-only, sem
...
native seed path. W-1528 replaces its byte-only HIR representation with typed
initializer roots. W-1505 remains current for the narrow HLO0 recovery form.
...
HIR0 schema `w-seed-hir0-2` added the first caller-owned binding record with a
byte-only String initializer. W-1528 advances the current schema to
`w-seed-hir0-4`. Each binding owns one typed `initializer_value` root in the
common postorder graph. Indexed reads preserve the lexical relation.
...
It requires the binding before its read. Value bytes remain dense without
gaps or overlap. Alias barriers remain fail-closed.

The W-1519 form accepted only one immutable `let` String literal and one later
read. W-1528 generalizes the native linear form to typed immutable `i64`, Bool,
and String initializers. It never performs downstream textual lookup. `var`,
assignment, shadowing, nested scopes, and general CFG remain unsupported.
...
#### W-1520 — verified-HIR direct MLIR0 native route (historical; superseded by W-1522)

W-1520 is historical for the bounded native seed route. Before W 1.0, it
replaced the incompatible MLIR0 schema v1 in place with `w-seed-mlir0-2`; W-1522
later superseded that schema-v2 contract in place, with no parallel legacy
adapter. This preserved the historical W-1506 and W-1520 evidence in the record
without treating compatibility inertia as a design constraint. At that time,
W-1505 remained current for the HLO0 direct subset, and W-1519 remained current
for immutable local binding semantics.

The primary route was `source → parser/frontend → verified HIR0 → MLIR0 →
mlir-opt → mlir-translate → clang/native`. HLO0, HLO1, and RUN0 remained
bootstrap, audit, and recovery paths, not prerequisites for native emission.
HLO0 independently proved its binding plan; MLIR0 consumed the same HIR
directly. The internal MLIR0 API accepted only
`w_seed_mlir0_input { program, hir_result }`. The HIR was re-verified at this
boundary by private `native_subset0`, the single exact selector shared
independently by HLO0 and MLIR0. It accepted direct `CONST_STRING` or one
immutable prior String `BINDING → CALL` with `BINDING_READ`, published only
borrowed structured records and payload slices, and performed no textual lookup.

The choice was intentionally narrow. MLIR0 preserved caller-owned storage,
bounded no-heap operation, transactional preflight, and all-or-nothing output.
It kept the x86_64 Linux target, textual builtin plus LLVM dialect artifact,
payload+LF bytes, and artifact digest. Invalid HIR, unsupported subset or
target, capacity, and descriptor/result/HIR-range aliases were rejected before
publication. Hello, empty, and the Restaurant binding/literal forms were
products; the two Restaurant artifacts were byte-identical and native stdout
was exact. No timing or result was reported.

The historical evidence was the real source→parser/frontend→HIR0→MLIR0→MLIR verification→
LLVM IR→native link→execution gate, with the pinned factual toolchain 20.1.2
marked `update-required`. The finite gaps were general HIR, multiple bindings or
values, CFG/general SSA, a W MLIR dialect, an MLIR C API builder,
ownership/effects/tasks lowering, an optimizer/pass pipeline,
provider/runtime/linker/SDK/packaging, other hosts or targets, the general
public `w run` surface, and performance. W-1521 closed only the bounded public
seed subset; W-1522 later became the current sequence successor.
`benchmarkDisposition` was `compiler-lifecycle`, correctness-only, with no
timing or result.

#### W-1521 — bounded public seed `w run`

W-1521 records the first public execution surface, but only as a bounded seed
product. The grammar is exactly `w run <explicit-path.w> [-- <args...>]`.
The path is explicit and singular. The command accepts non-empty valid UTF-8
source up to 4096 bytes. It does not perform recursive, cwd or PATH source
discovery, imports, package/workspace selection, registry access or network
access. The logical source identity is the basename supplied to Native0,
...
propagated; signal exits use `128 + signal`. Invocation, source, unsupported
...
identity, including a hyphen and the terminal `.w`; it is not an identifier or
module name. Only `w run` uses this basename rule. The `w check` helper and
`w_seed_check_root_source_id` remain unchanged. The versioned
...
MLIR0 schema `w-seed-mlir0-3` and Native0 schema `w-seed-native0-2`. W-1525
later advances those adapters while retaining this static shape. W-1523
advances the frontend schema without widening static NAT1; HIR0 schema
...
and has no effects. The block has 1..32 instructions, consisting only of 0..32
immutable String bindings and 1..32 ordered `print` calls. Every binding is
read at least once by a later instruction; repeated reads remain bounded and
are allowed. Instruction count equals binding count plus call count. Each call
has one positional String argument that is either a direct literal or a read of
a prior binding. Each payload is at most 256 bytes, and ordered stdout payload
plus LF is at most 4096 bytes. No other instruction or call is accepted.

MLIR0 may coalesce the pure ordered calls into one private global and one
`write`. This preserves the exact bytes and W source order, but does not promise
a syscall boundary for each source call. The static artifact maximum of 13190
bytes is derived from the fixed skeleton, worst-case escaped output, and
bounded decimal fields. W-1525 keeps this path inside a larger adapter
capacity. `measure` and `emit` remain caller-owned,
no-heap, deterministic, digest-bearing and all-or-nothing. Invalid HIR,
unsupported shape, short capacity and descriptor/result/HIR-range aliases are
rejected before publication and leave caller buffers unchanged.

HLO0, HLO1 and RUN0 retain their existing single-print schemas and reject
multi-call sequences. The cut adds no Int, interpolation, `var`, CFG, general
SSA, DCE, general locals, general types, W dialect, general runtime, HLO0
requirement or performance claim.
...
binding forms are products, and equivalent ordered forms produce byte-identical
artifacts. The public `w run` path uses W-1521's opaque-basename correction;
`w check` remains unchanged.

#### W-1523 — structured interpolation in the seed frontend

The native arithmetic milestone needs a real semantic path for
`"The answer is ${6 * 7}"`. Precomputing the result in a fixture, adding a
special `printInt`, or treating runtime `expect` as output would bypass the W
contracts that the product must eventually exercise. W-1523 therefore closes
only the parser/frontend boundary first.

Lexer events already distinguished literal start, text, interpolation start,
interpolation end, and literal end. The parser previously consumed those
events as an opaque run. It now retains literal/event identity on lossless CST
leaves and nests the ordinary expression parser between interpolation
boundaries. This avoids a second delimiter scanner in the frontend.

Frontend schema `w-seed-frontend-12` represents interpolation as a parent
expression plus a dense segment range. Text and expression segments are
different enum cases, not sentinel combinations inferred by consumers. Text
uses the existing constant-byte arena. Expression segments point to the same
normalized expression graph used elsewhere. Owner, ordinal, span, range, and
payload invariants are explicit.

The current supported seed subset is intentionally narrow. It accepts
ordinary String interpolation and built-in integer, Boolean, or String values
whose Display contract is already known by the language design. Escape
decoding and other Display conformances remain unsupported. Two unconstrained
integer operands and a standalone unconstrained integer interpolation default
to signed `i64`. A dedicated canonical record makes that type identity equal
in dry and emit passes. During this work, the same class of latent dry/emit
bug was found for inferred String bindings; both passes now always publish one
...
ranges, capacities, aliases, bytes, digests, and receipts. W-1525 later adds a
bounded native signed-`i64` path. Other Display domains and the final runtime
remain separate milestones. The evidence is compiler-lifecycle correctness
only, with no timing or performance result.

#### W-1525 — bounded signed-i64 interpolation through native MLIR

The native milestone must preserve the computation. A fixture that contains
`42`, a frontend fold, or a `printInt` host function would bypass String
interpolation and provide weak evidence. The accepted artifact therefore
contains the original integer constants and the corresponding LLVM dialect
operation. A unit check also rejects the precomputed output spelling.

MLIR0 schema `w-seed-mlir0-4` accepts the existing linear print shape and up
to 64 HIR values and 64 interpolation segments. The selector evaluates an
integer tree only to prove that it cannot panic. This proof admits all five
represented operators when overflow and division faults are impossible. It
never supplies the result to the emitter.

The seed adapter builds one ordered runtime format. It doubles percent bytes,
uses a 4097-byte stack buffer, formats signed `i64` with the Linux x86_64 GNU
`long` ABI, and performs one checked POSIX `write`. This implementation keeps
the output bounded and preserves one external write. It also makes the libc
dependency explicit. It is not the final Display runtime or a performance
baseline.

The bounded implementation rejects NUL inside interpolated text because the
temporary format is NUL-terminated. It also rejects Bool and String value
segments, unproven panic paths, nested interpolation, and output above 4096
bytes. Static String output still supports NUL through the W-1522 path. These
rejections are implementation gaps, not changes to W String or Display.

`restaurant-interpolation.w` is the adversarial witness. It crosses source,
parser, frontend, verified HIR, MLIR verification, LLVM translation, native
link, and execution. The exact result is `Table 42 remains open\n`. Additional
products exercise `+`, `-`, `*`, `/`, `%`, negative decimal output, and a
literal percent sign. Division by zero, overflow, and interpolated NUL tests
fail before artifact publication. The evidence is correctness-only.

#### W-1526 — internal bounded signed-i64 Display in LLVM dialect

The W-1525 `snprintf` adapter proved runtime formatting, but it also coupled
the artifact to C varargs, the GNU LP64 meaning of `%ld`, NUL-terminated format
text, and a large libc formatter. None of those constraints belongs to W
String interpolation. Keeping them would weaken portability and make a future
runtime boundary harder to audit.

MLIR0 schema `w-seed-mlir0-5` replaces that adapter with two internal helpers.
`w_seed_copy` copies counted text into the final buffer. `w_seed_append_i64`
derives an unsigned magnitude, counts digits with unsigned division, and writes
decimal digits backwards. Modular subtraction produces the magnitude of
`i64.min` without signed overflow. These operations remain in LLVM dialect,
whose operations map to LLVM IR semantics; the route continues through the
documented two-stage LLVM-dialect-to-LLVM-IR flow. Sources:
[MLIR LLVM dialect](https://mlir.llvm.org/docs/Dialects/LLVM/) and
[MLIR LLVM IR target](https://mlir.llvm.org/docs/TargetLLVMIR/).

The emitter builds an immutable text bank plus ordered text and integer
actions. It allocates one 4097-byte stack buffer, completes all actions, then
performs one checked POSIX `write`. Counted copying removes the format-string
interpretation completely. NUL and percent bytes in interpolated text are now
ordinary data. Tests require dynamic artifacts to omit `snprintf`, `%ld`,
varargs, and the precomputed Restaurant result.

This is an artifact-local seed runtime, not a stable W runtime ABI. It removes
one C ABI dependency but retains the target-specific POSIX sink. It does not
implement runtime panic paths, general Display, nonconstant values, or
additional targets. W-1527 later adds bounded built-in Bool and
compile-time-known String value segments without changing that runtime
boundary. The 98304-byte artifact limit and 4096-byte stdout limit remain
explicit. The gate executes NUL-bearing interpolation and multiple ordered
integer fields in addition to the Restaurant witness. Evidence is
correctness-only; no speed or size result is claimed.

#### W-1527 — bounded built-in Bool and String Display through native MLIR

W-1524 had already reserved typed HIR records for Bool and String values, but
two implementation gaps prevented a complete source route. An unconstrained
Bool literal could retain `inferred_type = NONE` when no Bool declaration had
created a frontend type record. A String binding used below an interpolation
root was not associated with its statement because binding resolution checked
only the argument root. Relaxing HIR verification would have erased type or
lexical evidence at the boundary. W-1527 instead repairs the frontend: it
publishes one canonical Bool identity and performs a depth-bounded structural
containment walk when locating the owning statement. The HIR verifier remains
unchanged and fail-closed.

The backend extension is deliberately narrower than general `Display`.
`CONST_BOOL` writes lowercase ASCII `true` or `false` through a private LLVM-
dialect helper. `CONST_STRING` and a prior immutable literal
`BINDING_READ` already have verified counted bytes, so the plan copies those
bytes into its immutable text bank. This preserves NUL and avoids inventing a
runtime String ABI. It is valid only because the selected subset excludes
mutation, runtime-produced String values, and unknown storage.

The Bool helper is omitted from artifacts that have no Bool action. The bound
proof charges four bytes for true, five for false, and the exact verified byte
count for String. Repeated String reads remain visible to the selector's
binding-use accounting. The same 4096-byte stdout and 98304-byte artifact
limits remain; exact-bound and over-bound interpolation cases test the new
path.

The adversarial Restaurant witness combines both Bool values with a String
binding and requires exact `Kitchen true/false; table: open\n`. A second native
product carries NUL through the String binding. These products pass MLIR
verification, LLVM translation, native link, and execution; the public bounded
`w run` route executes the Restaurant witness as well. This evidence does not
prove protocol witness dispatch, user-defined Display, runtime String layout,
nonconstant Bool, a stable runtime ABI, another target, or performance.

#### W-1528 — typed immutable binding initializers through native MLIR

W-1519 stored one immutable String binding as a separate byte range. That form
could not represent a typed arithmetic or Bool initializer without a second
value model. W-1528 removes the special case. Each HIR binding owns one root
in the existing typed postorder value graph.

The frontend infers only the closed built-in subset `i64`, Bool, and String.
It records the exact prior binding statement and effective type. HIR copies
that relation and never resolves the source name again. Verification requires
the read and binding to share one block and one type. The binding instruction
must strictly precede the read.

The native selector remains narrower than the HIR representation. It accepts
a String literal, Bool literal, or panic-free signed-`i64` expression as the
initializer. MLIR actions refer to the original initializer SSA index. The
Restaurant multiplication therefore remains `llvm.mul` in the artifact.
This rule rejects a precomputed output path and preserves optimization
authority for the MLIR pipeline.

The adversarial witness binds `6 * 7`, `true`, and `"open"`, then reads all
three values in one interpolation. The MLIR and public `w run` gates require
exact `Table 42; open: true; state: open\n` output. Structural tests mutate
initializer indices, owners, types, ordering, bounds, aliases, and receipts.

This evidence does not prove mutable locals, assignment, nested scopes,
general control flow, phi values, runtime String storage, protocol dispatch,
another target, or performance. The disposition is `compiler-lifecycle` and
publishes correctness only.

#### W-1529 — typed direct Unit calls through native MLIR

The prior native route could preserve local values but still required all
observable work to occur in the entry function. W-1529 adds the smallest call
boundary that tests source evaluation order, ABI mapping, parameter identity,
and multi-function emission without introducing return-value SSA.

HIR stores source order and target parameter order separately. This is needed
because W permits named arguments to move globally while `_` parameters remain
positional. The frontend resolves each named label to one declaration slot and
HIR rejects duplicate or missing slots. It compares represented built-in types
by meaning, not by frontend record index. Parameter reads use an indexed edge;
later phases do not resolve names again.

The native selector rejects recursion and all non-Unit or indirect calls. MLIR
emits distinct internal functions with typed `i64` and `i1` parameters. The
caller evaluates argument trees in source order, then supplies operands in ABI
slot order to a real `llvm.call`. The callee reads those operands during String
interpolation. A checked native product and public bounded `w run` require exact
Restaurant output. Structural checks also require that `llvm.mul` remains and
that the call is not flattened.

This cut does not claim runtime String parameters, return values, overload
selection, generic specialization, recursion, indirect calls, exceptions,
async, general CFG, a stable ABI, other targets, or performance. Its benchmark
disposition is `compiler-lifecycle`, correctness-only.

#### W-1530 — scalar function results through verified HIR and native MLIR

W-1529 proved a real call boundary but intentionally discarded every W-to-W
result. W-1530 adds the next SSA edge: a final signed-`i64` or Bool return is
owned by the function terminator, and a direct caller receives it through an
indexed `CALL_RESULT`. An immutable binding can use that value without a
redundant source annotation because the frontend reuses the callee signature.

The HIR verifier checks the return and call relation independently of source
text. The MLIR emitter produces a typed `llvm.return` and a result-producing
`llvm.call`. The checked Restaurant product computes `6 * 7` in the callee,
passes the SSA result to caller interpolation, and writes exact `Table 42\n`.
The artifact must retain the multiplication and call; a precomputed payload is
not equivalent evidence.

The cut remains deliberately linear and bounded. It excludes nested calls,
return-call expressions, runtime String results, multiple control-flow exits,
phi values, recursion, indirect calls, exceptions, async, a stable ABI, other
...
única. [`tooling/platform-support.json`](tooling/platform-support.json) guarda
targets emitted, compiler hosts, evidence, policy e planos de toolchain.
[`PLATFORM-SUPPORT.md`](PLATFORM-SUPPORT.md) é uma projeção humana
determinística gerada por `tooling/platform-support.mjs`. A projeção não é uma
segunda autoridade.

Targets usam `candidate`, `evidence`, `supported`, `deprecated` e `removed`.
`candidate` e `evidence` usam verification level nulo. `supported` exige um
nível entre `experimental`, `level-3`, `level-2`, `level-1` e `long-term`.
Cada row exige os eixos `backend`, `runtime`, `hostAdapter`, `sdkProfile`,
`linkerSysrootPackaging` e `ciEvidence`. Cada eixo publica `pass`, `partial`
ou `missing` e uma lista de evidence. `supported` exige todos os eixos em
`pass` com evidence não vazia. Blockers são exatamente os eixos que não estão
em `pass`.
...
`evidence` e scope dev-only. WSL não é Windows nativo. Candidates native
cobrem Linux x86_64/AArch64, Windows x86_64/AArch64 e macOS x86_64/AArch64.
Os eixos de host são `nativeToolchain`, `toolchainBundle`,
`releasePackaging` e `ciEvidence`. No composite Windows+WSL,
`nativeToolchain` é `partial`, aparece em `blockers` e nunca é `pass`: tools
Linux não são toolchain Windows nativo.

O policy registra a meta comparativa `referenceBreadth.goal:
at-least-rust-breadth`, as fontes oficiais, `observed: 2026-08-31` e
`importsRustTiers: false`. A meta não herda tier ou claim do Rust. A triple
...
`aarch64-apple-darwin`, além dos mesmos três emitted targets. A fonte mantém
os nove edges host→target, incluindo self edges, com IDs determinísticos,
referências separadas, state, evidence e blockers. Todos são `candidate`.
O checker rejeita matriz incompleta, duplicada ou somente self, referências
inválidas, conflation de host e target e edge WSL contada como native.

A edge outer Windows + tools Linux WSL2 → target Linux x86_64 fica como
development evidence separada e `nativeHost: false`. Ela publica roles
`development`, `toolchain`, `build` e `execution`; somente execution usa
`executionMode: local`. Seus blockers derivados mantêm `nativeHost`, endpoints
e `toolchainSysrootLinkerPackaging`; `buildExecution` desaparece porque build
e execution têm evidence. Um edge supported exige endpoints supported, evidence
de toolchain, sysroot, linker, packaging, build e execution. Linux↔Windows
precisa SDK, sysroot, object e linker explícitos. Apple precisa SDK, licença e
provenance, sem pressupor redistribuição. Remote execution fica separada de
build. A meta é distribuição W first-class: cada host primário deve compilar
...
#### W-1508 — catálogo operacional de dependency currency

W-1508 fecha uma fonte operacional única para currency sem transformar
metadata em semântica W. `tooling/dependency-currency.json` é a fonte máquina;
`DEPENDENCIES.md` é a projeção humana gerada de forma determinística. O
checker valida os selectors e estados contra os package manifests, o lockfile
Tree-sitter, o workflow CI, os READMEs e os registros de platform support.

Managed ativos precisam coincidir com a versão latest stable registrada e usar
...
Unicode `17.0.0` continua o snapshot vigente. O successor MLIR `23.1.0` fica
selected/not promoted com tag object assinado
`9b0f9b1eb4a233717c6ed014cff6f8a7c65512de` e commit peeled
`ea7d852a70e8bdfaf601d6626a760f9771b2c4b4`; os planos native usam esse pin
exato, mas permanecem bloqueados por `native-build-acquisition-provenance`.

O catálogo registra fontes oficiais primárias para claims de currency e também
separa fontes de repository, recipes e avaliações externas. A avaliação
`portable-mlir-toolchain` conserva o repositório, release e provenance de
componente LLVM como contexto não autoritativo. O ambiente observado nunca é
apresentado como pin exato. A classificação é `source-backed-current` somente
para o catálogo, o checker e a projeção; não há claim de backend, runtime,
host/target support, CMake/Ninja pin ou toolchain nativo.
...
tem evidência C23 do subset bounded e do gate Linux real em duas waves; C11 é
somente recovery explícito, enquanto
...
O unit C23 usa um link fake para cobrir success, statuses de failure, alias,
...
#### W-1500 — política de dialeto C23 do C controlado

W-1500 resolve a ambiguidade que ficou entre o seed C conservador histórico,
as APIs internas recentes e as referências do BMD. A regra vigente é C23 como
default explícito para CMake do seed, artefato gerado por HLO1, probes/checkers
C diretos e baseline byte-scan-view. O CMake usa `W_SEED_C_STANDARD=23` por
default, aceita somente `23` e `11`, exige o standard e desliga extensões.

GCC e Clang que suportam C23 usam `-std=c23`. Uma toolchain antiga que só
aceita `-std=c2x` pode executar correctness com o disclosure
`c2x-preview (correctness-only; not a final C23 result)`, mas não pode produzir
resultado ou ranking final rotulado C23. A ausência de C23 em MSVC produz SKIP
claro no gate principal, salvo quando o caller solicita a lane C11 recovery.
Nenhum caminho converte essa ausência em fallback silencioso.

A lane C11 é deliberadamente finita: serve para recovery do seed e
compatibilidade de headers. O código do seed continua compilável nela, mas o
...
`bun run demo:seed-hello` reusa a rota real e valida stdout exatamente
`Hello, world!\n`; ele é chamado de seed demo e não de `w run`.

A evidência desta decisão é o CMake focal, a sonda única em
`tooling/c-dialect.mjs`, o gate HLO1, o recovery C11 explícito, o demo, os
checkers/probes diretos e o smoke estrutural/byte-scan BMD. As limitações
...
tipo depois de `for`. O witness corrente é `WrappedDegrees` em
`reference/last-light/orbit.w`: seu `set` e seu hook de `modify` normalizam o
valor, e `Attitude.rotate` observa o resultado `15` depois de `350 + 25`.
...
inline-canceled de budget exhaustion. `task#cancel` não o consome e `await`,
`join` e `(take task)#outcome()` consomem o handle. `TaskOutcome`,
`TaskSettlement` e `TaskOrdering` são os records e enums publicados em
`std/runtime/task.w`; controles core são facets e esses records continuam dados
com `.`. `Allocator`, `AllocatorLease`, `AllocatorPlanDescriptor` e
`AllocatorPlan` seguem `std/memory/contracts.w`; a rationale não inventa um
...
`sync`; uma function ordinary também pode inferir `maySuspend`. O modo
`pipeline<tasks: ...>` é a região corrente para uma coleção dinâmica homogênea,
com schema explícito e `TaskOrdering`.
`pipeline` unifica os modos dependent, tasks e transaction em um
`pipeline_region`/HIR comum, sem promise implícita; `unknownOutcome` e
`unknownCommit` permanecem tipados. Lowering compartilhado não esconde os
schemas da source surface.
...
W-1504 é um estudo separado do estudo de allocation/placement. Ele registrou a
antiga fronteira em que `pipeline` era um DAG estático com `return` e
`transaction` era uma região async separada com `commit`. Essa fronteira e sua
proposta research-gated foram superseded por W-1511; o texto e o ledger abaixo
preservam somente proveniência histórica e não definem grammar, corpus, atlas,
highlighting ou Última Luz correntes.
...
#### W-1509 — facets ligadas a property places

W-1509 fecha a distinção que faltava entre um member de valor e uma facet
compiler-owned. O ponto `.` continua resolvendo somente a API lógica do valor;
`place#facet` resolve uma facet declarada no behavior aplicado ou uma facet
intrínseca de controle imediato num handle/context core. Não existe fallback
entre os namespaces e uma
...
geral oculta ou authority implícita. Uma facet mutating exige place exclusivo e
`take` facet fica fora deste bundle. A resolução é estática e imediata:
`attitude.yaw#reset()` opera no place da property, enquanto copiar `yaw` perde
a ligação. Reificação (`let f = place#reset`, `place#` e dynamic lookup) é
...
`yaw#degrees.reset()` altera o valor pelo storage behavior sem expor backing.
...
`execution#checkCancellation` e `execution#yield`. `firstSettled`,
`Task.withTimeout`, `Task.withDeadline` e `Task.spawn` (somente
para `ExecutionDomainRef` dinâmico) usam members normais; o initializer estático
...
`SharedTask` observador não recebe
...
cada etapa à seguinte e faz da forma uma composição funcional legível, enquanto
`pipeline` continua reservada ao graph com nodes, effects e `commit`.
...
W-1511 fecha a convergência que W-1504 deixou em pesquisa. `pipeline` passa a
ser a única superfície de execution graph. Os modos `dependent`, `tasks` e
`transaction` compartilham `pipeline_region`/HIR e `commit`, mas schemas e
restrições permanecem explícitos. O bloco dependent usa `commit`, e a forma
curta é somente uma cadeia de dois ou mais calls de service/capability; ela
entrega o último resultado sem permitir branch, fan-out ou function local.

O modo `tasks` exige `tasks`, `limit`, `ordering` e `errors`, sem defaults.
`.concurrent` e `.parallel<domain>` conservam as provas de domain e
transferability. `.failFast` cancela/drena e devolve `Array<Output>`; `.collect`
devolve `Array<TaskSettlement<Output, Failure>>`, preserva o índice e não
transforma application failures em throw. Cada input tem uma instância bounded
e um commit. `TaskGroup.*` e `TaskGroupOrdering` são apenas spelling histórico;
a superfície corrente é `pipeline<tasks: ...>` com `TaskOrdering`.

O modo `transaction` usa a forma
`pipeline<transaction: { isolation: ..., access: ... }> tx = provider { ...
commit value }`. A antiga expressão `transaction` é rejeitada antes de 1.0;
...
uma tuple rotulada e estática, por exemplo
`export behavior VersionedDegrees for u16 = (degrees: WrappedDegrees, version:
Versioned)`, enquanto a property continua com uma única aplicação
`var VersionedDegrees yaw: u16 = 0`.

O papel de storage é inferido pela presença de `get`/`set`/`modify`; no máximo
um componente pode ocupá-lo. Uma composição nominal sem storage usa storage
...
alias duplicado, ciclo ou path ausente são diagnostics, não uma prioridade
...
`VersionedDegrees` dá uma razão observável para a composição: `WrappedDegrees`
normaliza o valor e `Versioned` publica epoch/reset. A evidência source-backed
inclui declaration, aplicação, mutation, ordem e resultado; não há claim de
compiler/runtime/provider. O `benchmarkDisposition` deste plano é `required`,
com status `blocked/deferred`: composição no checker/lowering, ABI/fingerprint,
runtime/provider de hooks e round-trip/cancel-drain continuam blockers. Nenhum
timing ou resultado foi coletado.

O plano de evidência é separado por decisão: FCT0 cobre lookup, place,
immediate use e custo de facets W-1509; CMP0 cobre composição, aliases, ordem,
hooks e fingerprint W-1512; PIP0 cobre expansão, precedência, avaliação única e
modifiers W-1510; PLN0 cobre schemas, cardinalidade, admission, arbitration,
drain e uncertainty W-1511. Cada plano permanece `required` e sem timing ou
resultado: FCT0/CMP0/PLN0 estão `blocked` por compiler/lowering/runtime/provider
e PIP0 está `deferred` até checker/compiler e execução equivalente existirem.

O contraste de `#` com TypeScript `private` é intencional: `#` é uma projeção
estática de uma property place para uma facet de behavior/core, não um marcador
de encapsulamento lexical; a visibilidade continua sendo a mínima de
facet/behavior/property. As fontes primárias já registradas dão precedentes
limitados, não semântica W: [Swift projected values](https://docs.swift.org/swift-book/documentation/the-swift-programming-language/properties/#Projected-Values-for-Property-Wrappers)
e [protocol composition](https://docs.swift.org/swift-book/documentation/the-swift-programming-language/protocols/#Combining-Protocols),
[Kotlin delegated properties](https://kotlinlang.org/docs/delegated-properties.html),
[TypeScript decorators](https://www.typescriptlang.org/docs/handbook/2/decorators.html)
e [private class members](https://www.typescriptlang.org/docs/handbook/2/classes.html#private),
[F# pipelining](https://learn.microsoft.com/en-us/dotnet/fsharp/language-reference/functions/#function-composition-and-pipelining)
e [D pseudo-members/UFCS](https://dlang.org/spec/function.html#pseudo-member-functions).
...
Uma projection não amplia authority. A raiz só expõe facts, controls e
capabilities já concedidos pelo product/target; ela não descobre host resources
nem oferece lookup ambiental. `execution.openSerial(...)`, por exemplo, deriva
uma lane bounded da scheduling authority já presente e continua sujeito aos
limites dela. Filesystem, network, device, allocator e telemetry entram somente
quando seus contratos foram concedidos explicitamente.
