...
| W-1491 | plano HLO0 source-backed de Hello World | O frontend v10 recebe um host scope ativo explícito, publica qualifiers, Unit inferido e identidade discriminada de callee; `print` é um símbolo normal do host prelude `native-process@1` com requirement nominal `Console`, não intrinsic nem global. O adapter caller-owned valida o grafo estruturado sem substring de source/receipt e produz, para `fn main() { print("Hello, world!") }` com `entry(main)`, um plano bounded com payload, LF, stdout de 14 bytes, digest e exit success. A evidência HLO0 termina em source → parser → frontend → plano; HLO0 não emite C nem executa W. `benchmarkDisposition` é deferred para `hlo3-hello-world-runtime-benchmark`, e timing fica proibido neste corte | Source, adapter e gates HLO0 estão em `compiler/seed-c`; `bun run check:hlo0`, frontend seed, parse focal e CTest provam o plano e suas barreiras. A decisão HLO0 permanece `implementation-evidence-gap` para verified HIR, execução W, Console provider, w-linker, runner e runtime |
...
| W-1493 | emissão C11 source-backed de Hello World | HLO1 acrescenta `w_seed_hlo1_measure`/`w_seed_hlo1_emit` como API caller-owned sem heap. A API revalida o plano HLO0 completo e emite bytes C11 determinísticos, LF-terminados, começando pelo comentário de schema `/* w-seed-hlo1-1 */`, com array hexadecimal `unsigned char`, `<stdio.h>`, `fwrite` e `fflush`; `_WIN32` usa `_setmode(_fileno(stdout), _O_BINARY)` para preservar LF. O buffer de emissão é all-or-nothing, mas o efeito externo em stdout não é atômico. O plano isolado não prova source provenance. O gate integrado prova source → parser → frontend → HLO0 → HLO1 → C11 → stdout/stderr/exit e rejeita witnesses Restaurant sem substring bypass. `benchmarkDisposition` é deferred para `hlo3-hello-world-runtime-benchmark` e nenhum timing é publicado | HLO1 é `source-backed-current` para este subset bounded. A evidência está em `compiler/seed-c`, `tooling/check-hlo1.mjs` e `bun run check:hlo1`, com C11 build e execução real. CMake/Ninja/compiler ausentes geram SKIP explícito. Falha de toolchain presente gera FAIL. HIR verificado, Console provider W geral, w-linker, runtime W, `w run` e language benchmark runner permanecem fora do escopo |
