...
| ergonomia com evidência | 65–72% | R0 cobre 70/70, R0S mede a superfície derivada por script e R1 possui 25 bundles contrabalanceados do Última Luz que promovem 41/70 casos R0; participantes e modelos ainda não foram executados |
...
| W-886 | corpus R1 ampliado | 25 bundles, 62 variantes e 100 tarefas cobrem os domínios R1 atuais com source base, inputs, digests e oracle; 41/70 casos R0 foram promovidos | extrapolar R0; variante sem contexto; outcome não fixado; chamar host oracle de execução W |
...
| W-891 | catálogo std verificável | profiles cobrem 334 APIs em 25 módulos, 25/25 requisitos e oito carriers sem interface missing; declarations estão draft-ready, 87 superfícies são verificadas e 19/19 providers continuam missing | contar arquivo como cobertura, inferir API sem scan, tratar provider missing como execução ou duplicar o grafo de readiness |
...
| W-920 | cobertura de promoção R1 | índice e checker contam IDs R0 únicos ligados a bundles; 41/70 mede planejamento, não evidência humana, de modelo ou runtime | contar referências duplicadas; dividir bundles por requisitos; chamar promoção de ratificação; esconder o denominador |
...
| W-936 | estudo R1 de callables | três variantes completas comparam representação separada, callable universal e protocols nominais; outcomes do restaurante coincidem, enquanto dispatch, custo, consumo, function-value labels/defaults, compatibilidade invariável e recovery de erasure ficam observáveis; promove cinco casos R0 | snippet sem capture; comparar somente tokens; esconder segunda call; transportar default no valor; widening implícito; chamar host oracle de execução W |
...
| W-1147 | PYN4 status and evidence | fixture Last Light e machine/checker/snapshot/test host cobrem positivos e negativos e não executam W; W-1254 fecha C Exchange N0 como fast path scoped da bridge Python | chamar machine de runtime, omitir adversarial ou snapshot, promover provider missing ou tratar C Exchange como semântica do core |
