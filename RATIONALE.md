...
fact é um source/provider owner token, não um package/workspace owner. O
discovery loop interno bounded tem evidência CHK6, e CHK8 tem evidência do
adapter Windows real, mas NFC completo, provider std, owner detection,
package/workspace, diagnostics completos, compiler, runtime, resolução pública
multi-file e conformance cross-target continuam gaps. CHK7 fornece somente
composição interna caller-owned CHK6→frontend→D0 JSON-only e mapeia
`W-SEM-0001`; isso não é
frontend completo nem resolução pública multi-file. A proveniência de capacity
preservada pelo parser é evidência interna e não cria mapping D0 público. CHK1
ainda expõe `w check` somente no perfil closed-single-source.
