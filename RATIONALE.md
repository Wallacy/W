...
W-1521 remains current for the bounded CLI contract and now points to W-1522
for NAT1. The runner's explicit path basename is an opaque logical source
...
`hlo0-hello.w` and `restaurant-linear.w` fixtures are therefore executable by
...
a syscall boundary for each source call. The artifact maximum
`W_SEED_MLIR0_MAX_BYTES = 13190` is derived from the fixed skeleton, worst-case
escaped output and bounded decimal fields, and is proved by compile-time static
assertions rather than trial. `measure` and `emit` remain caller-owned,
...
`x86_64-unknown-linux-gnu`, no scope `w-seed-mlir0-3-linear-print-sequence`. O backend
