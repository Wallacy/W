...
Linux. A API deriva a root, o mapping root-relative, a ordem do inventory, os
edges, ciclos, colisões, limits e a projeção lógica da recipe. O digest e o
byte count são calculados dos bytes do source. O `sourceText` dos snippets é
...
fact é um source/provider owner token, não um package/workspace owner. Discovery
loop, NFC completo, provider std, adapter Windows real, owner detection,
package/workspace, diagnostics completos, compiler, runtime e conformance
cross-target continuam gaps. CHK1 ainda expõe `w check` somente no perfil
