...
discovery loop interno bounded tem evidência CHK6, mas NFC completo, provider
std, adapter Windows real, owner detection, package/workspace, diagnostics
completos, compiler, runtime, resolução pública multi-file e conformance
cross-target continuam gaps. CHK7 fornece somente composição interna
caller-owned CHK6→frontend→D0 JSON-only e mapeia `W-SEM-0001`; isso não é
