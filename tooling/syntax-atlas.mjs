...
const VISIBLE_RULES_MUST_NOT_BE_INTERNAL = new Set([
  ...
  "get_accessor", "set_accessor", "modify_accessor", "accessor_implementation", "behavior_parameter_list",
...
const DIRECT_RULES = new Set([
  ...
  "behavior_declaration", "entry_declaration", "foreign_declaration", "const_declaration", "test_declaration", "type",
...
function buildManifest(snapshot, cheatsheet) {
  return {
    $schema: SCHEMA,
    purpose: "Human syntax atlas for the current W Tree-sitter surface.",
    sourceOfTruth: "atlas-manifest.json metadata plus terse source markers and snippets in the atlas .w files. CHEATSHEET.md is generated.",
    ...
    generated: { cheatsheet: "CHEATSHEET.md", cheatsheetDigest: digestBytes(Buffer.from(cheatsheet, "utf8")) },
  };
}
...
function checkManifest(snapshot, expected) {
  ...
  if (expected?.generated?.cheatsheet !== "CHEATSHEET.md" || expected.generated.cheatsheetDigest !== digestBytes(Buffer.from(cheatsheet, "utf8"))) errors.push("cheatsheet digest is stale.");
  if (!fs.existsSync(CHEATSHEET) || fs.readFileSync(CHEATSHEET, "utf8") !== cheatsheet) errors.push("CHEATSHEET.md is stale or manually edited.");
  ...
  if (JSON.stringify(expected?.rootForms) !== JSON.stringify(buildManifest(snapshot, cheatsheet).rootForms)) errors.push("root disposition inventory is stale.");
  return errors;
}
...
function writeArtifacts(snapshot) {
  ...
  fs.writeFileSync(CHEATSHEET, cheatsheet, "utf8");
}
...
export { buildManifest, deriveSnapshot, renderCheatsheet, checkManifest, VISIBLE_RULES_MUST_NOT_BE_INTERNAL, REQUIRED_VARIANT_IDS };
