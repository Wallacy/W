...
const kernelSource = `module models<kernels: { forecast: forecastKernel }, contracts: [surfaceReady()]>
...
let kernels = 1
configure(contracts: value)
let contracts = 1`;
const kernelTokens = portalTokens(kernelSource);
const kernelValues = kernelTokens.filter((token) => token.value === "kernel" || token.value === "kernels" || token.value === "contracts");
assert.deepEqual(kernelValues.map(({ value, kind }) => ({ value, kind })), [
  { value: "kernels", kind: "keyword" },
  { value: "contracts", kind: "keyword" },
  ...
  { value: "kernels", kind: "identifier" },
  { value: "kernels", kind: "identifier" },
  { value: "contracts", kind: "identifier" },
  { value: "contracts", kind: "identifier" },
...
assert.match(moduleKernelPattern, /contracts/u);
...
assert.doesNotMatch(keywordPatterns.filter((pattern) => pattern.match?.includes("contracts")).map((pattern) => pattern.match).join("\n"), /contracts/u, "TextMate must keep contracts out of global keyword patterns");
