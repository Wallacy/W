...
Os checks repo-wide usam o command registry, por exemplo
`bun tooling/command-runner.mjs --command check:wire`,
`bun tooling/command-runner.mjs --command check:hir` e
`bun tooling/command-runner.mjs --command check:design`. Para a suíte rápida use
`bun check --target quick`; para a suíte integrada use `bun check`. O CLI está fixado em
