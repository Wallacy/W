...
| `std-api-contracts.json` + checker de std | perfis cobrem 338 APIs em 26 módulos, 87 superfícies qualificadas, 25/25 requisitos contratados e 0/8 carriers missing | catálogo e snapshot são projeções; módulos catalogados são drafts e seus 20/20 providers intrinsics continuam missing; Blob é composição W sem provider próprio; bounds determinísticos entram na recipe key |
