...
| `design-freeze-audit.json` + checker | combina eixos source, oracle e disposition explícita; 492/1309 decisões estão classificadas (142 source, 391 oracle e 8 explícitas), dois contratos exigem múltiplos eixos e 49 overlaps não inflam a cobertura | worklist do freeze; não transforma cobertura parcial em aprovação |
...
| `std-api-contracts.json` + checker de std | perfis cobrem 371 APIs em 28 módulos, 87 superfícies qualificadas, 30/30 requisitos contratados e 0/8 carriers missing | catálogo e snapshot são projeções; módulos catalogados são drafts e seus 22/22 providers intrinsics continuam missing; Blob é composição W sem provider próprio; bounds determinísticos entram na recipe key |
