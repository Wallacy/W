...
| `process-root-cases.json` + máquina/checker/snapshot | PR0 cobre 38 casos/183 operações (21 aceitos + 17 rejeitados) para root projections, argv nativo, Context, stdio, signals, service drain e ExitCode; nove testes host usam inputs independentes | oracle host de design; não executa W, compiler, OS, signal adapter, scheduler ou provider `std.process@1` |
...
| `design-freeze-audit.json` + checker | combina eixos source, oracle e disposition explícita; 476/1301 decisões estão classificadas (142 source, 375 oracle e 8 explícitas), dois contratos exigem múltiplos eixos e 49 overlaps não inflam a cobertura | worklist do freeze; não transforma cobertura parcial em aprovação |
...
| `std-api-contracts.json` + checker de std | perfis cobrem 349 APIs em 27 módulos, 87 superfícies qualificadas, 27/27 requisitos contratados e 0/8 carriers missing | catálogo e snapshot são projeções; módulos catalogados são drafts e seus 21/21 providers intrinsics continuam missing; Blob é composição W sem provider próprio; bounds determinísticos entram na recipe key |
