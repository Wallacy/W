...
O estado atual é `91 source-backed-current`, `490 oracle-backed-current`, `0
research-gated`, `813 implementation-evidence-gap`, `57 superseded` e `8
rejected` (1459/1459). A lacuna histórica de 920 fica distribuída em 72
...
| [`compiler/seed-c/README.md`](../compiler/seed-c/README.md) + `check-seed-source-reader.mjs` + `check-seed-lexer.mjs` + `check-seed-unicode.mjs` + `check-seed-parser.mjs` + `check-seed-formatter.mjs` + `check-seed-diagnostic.mjs` + `check-seed-foreign.mjs` + `check-seed-frontend.mjs` + `check-seed-constir.mjs` | source reader C11 allocation-free, byte-first, UTF-8 estrito, spans e pontos; lexer lossless com profile Unicode 17.0.0 pinado e handshake de foreign; scanner C `c-inline-1` source-validation-only com spans/limites/SHA-256; parser seed caller-owned/incremental de CST/recovery, formatter CST-driven, adapter D0 bounded sobre 28 IDs F0, frontend seed caller-owned COMPLETE-only para AST/declarations, graph facts e type subset, e executor ConstIR D1 para Bool, integers, enums e listas estáticas borrowed em corpos bounded (`guard`/`if`/`for`/`return`), com quotas determinísticas e teto explícito de elementos | componente interno do seed; parser/formatter/adapter/scanner/frontend/ConstIR não são compiler normativo, HIR, runtime, provider ou publicação de build; frontend é uma fatia semântica bounded, um documento por module ID, com imports externos somente por stubs estruturados; checkers validam dados pinados, witnesses, corpus, outputs canônicos e records D0; NFC, resolver completo, CR isolado, compiler/typechecker completo e build publication continuam gaps |
...
| `design-freeze-classification.json` + `check-design-freeze-audit.mjs` | registro versionado e explícito dos 1459 IDs, com uma categoria fechada, claim do ledger, authority ref, digests, casos de fonte/oráculo e stop condition; preserva a cobertura legada 170 source, 413 oracle, 8 explícitas e 52 overlaps; PFU0 e AEG0 promovem W-1451–W-1458 a `oracle-backed-current`, SIMD1 promove W-1459 com oracle host-only, W-1354 é superseded por W-1437 e W-1448–W-1450 permanecem implementation-evidence-gap | auditoria de design; fonte/oráculo host não são compiler, runtime ou provider |
