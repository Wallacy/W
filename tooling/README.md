...
| `formatter-cases.json` + checker | 25 pares input/output CST-equivalentes e snapshots de `w fmt --check`; o par foreign formata a assinatura e preserva cada byte do body opaco | oracle de design; formatter ainda não existe |
...
| `process-root-cases.json` + máquina/checker/snapshot | PR0 cobre 40 casos/195 operações (22 aceitos + 18 rejeitados) para root projections, argv nativo, Context, filesystem, stdio, signals, service drain e ExitCode; dez testes host usam inputs independentes | oracle host de design; não executa W, compiler, OS, signal adapter, scheduler ou provider `std.process@1` |
...
| `design-freeze-audit.json` + checker | combina eixos source, oracle e disposition explícita; 490/1307 decisões estão classificadas (142 source, 389 oracle e 8 explícitas), dois contratos exigem múltiplos eixos e 49 overlaps não inflam a cobertura | worklist do freeze; não transforma cobertura parcial em aprovação |
...
| `std-api-contracts.json` + checker de std | perfis cobrem 367 APIs em 28 módulos, 87 superfícies qualificadas, 29/29 requisitos contratados e 0/8 carriers missing | catálogo e snapshot são projeções; módulos catalogados são drafts e seus 22/22 providers intrinsics continuam missing; Blob é composição W sem provider próprio; bounds determinísticos entram na recipe key |
