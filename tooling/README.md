...
| `check-maintained-parse.mjs` | mantém os gates `parse:reference` e `parse:std` no `tree-check`, preserva properties Web `Blob.type`/`Response.type` e bloqueia `type` em parameters, bindings e call labels | gate de integração lexical; não substitui o parse real nem altera a grammar |
