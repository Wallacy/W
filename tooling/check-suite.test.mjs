...
describe("check-suite manifest", () => {
  test("validates the repository manifest and preserves expanded order", () => {
    ...
    expect(flattenCheckSuite({ suites: loaded.suites, suiteName: "root-check" })).toHaveLength(118);
    ...
    expect(flattenCheckSuite({ suites: loaded.suites, suiteName: "root-quick" })).toHaveLength(10);
    expect(flattenCheckSuite({ suites: loaded.suites, suiteName: "root-compiler" })).toHaveLength(29);
    ...
    expect(flattenCheckSuite({ suites: loaded.suites, suiteName: "tree-check" })).toHaveLength(115);
    ...
    for (const suiteName of ["root-compiler", "tree-check", "root-check"]) {
      ...
      expect(expanded.slice(acquisitionIndex - 1, acquisitionIndex + 5))
        .toEqual([
          ...
          { package: "root", script: "check:source-binding" },
    ...
    for (const suiteName of ["root-compiler", "tree-check", "root-check"]) {
      ...
      expect(expanded.slice(indices[0] - 1, indices[0] + 4)).toEqual([
        { package: "root", script: "check:acquisition" },
        { package: "root", script: "check:owner-guard" },
        { package: "root", script: "check:source-binding" },
        { package: "root", script: "check:seed-manifest" },
