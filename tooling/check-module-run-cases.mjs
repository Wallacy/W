...
import {
  runModuleRunProgram,
  validateModuleRunOperation,
} from "./module-run-machine.mjs";

const toolingDirectory = path.dirname(fileURLToPath(import.meta.url));
const wDirectory = path.resolve(toolingDirectory, "..");
const casesPath = path.join(toolingDirectory, "module-run-cases.json");
const snapshotPath = path.join(toolingDirectory, "module-run-results.snapshot.jsonl");
...
if (corpus.$schema !== "w-module-run-cases-1") errors.push("unexpected module-run schema");
if (corpus.status !== "design-oracle-input") errors.push("module-run corpus must be design-oracle-input");
if (corpus.machine !== "module-run-machine-ru0") errors.push("module-run machine name is invalid");
if (!Array.isArray(corpus.decisions) || corpus.decisions.length === 0) {
  errors.push("module-run corpus must declare ledger IDs");
} else {
  for (const decision of corpus.decisions) {
    if (!ledgerIdSet.has(decision)) errors.push(`unknown module-run ledger decision ${decision}`);
  }
}
...
for (const [index, testCase] of (corpus.cases ?? []).entries()) {
  ...
  if (!/^RU0-[a-z0-9-]+$/.test(testCase.id ?? "") || caseIds.has(testCase.id)) {
    errors.push(`${location}.id is invalid or duplicated.`);
  } else {
    caseIds.add(testCase.id);
  }
  if (!Array.isArray(testCase.decisions) || testCase.decisions.length === 0) {
    errors.push(`${location}.decisions must link to current ledger IDs.`);
  } else {
    for (const decision of testCase.decisions) {
      if (!ledgerIdSet.has(decision)) errors.push(`${location}.decisions references unknown ${decision}.`);
    }
  }
  ...
  operations.forEach((operation, operationIndex) => {
    if (!validateModuleRunOperation(operation)) errors.push(`${location}.operations[${operationIndex}] is malformed.`);
  ...
  if (testCase.expected.status === "rejected" && (!requireString(testCase.expected.code, `${location}.expected.code`) || testCase.expected.at !== "last")) {
    errors.push(`${location}.expected rejection must identify code at the last operation.`);
  }
  const actual = runModuleRunProgram(operations);
  ...
  if (testCase.expected.status === "rejected") {
    ...
    if (actual.operation !== operations.length - 1) errors.push(`${testCase.id} rejected before the final operation.`);
  }
  results.push({
    caseId: testCase.id,
...
process.stdout.write(`Module-run RU0 oracle: ${results.length} cases, ${operationCount} operations, ${accepted} accepted, ${rejected} rejected.\n`);
