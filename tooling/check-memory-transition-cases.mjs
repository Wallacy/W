...
const casesPath = path.join(toolingDirectory, "memory-transition-cases.json");
const snapshotPath = path.join(toolingDirectory, "memory-transition-results.snapshot.jsonl");
...
for (const [caseIndex, testCase] of (corpus.cases ?? []).entries()) {
  ...
  const actual = runMemoryProgram(testCase.operations);
