...
const output = {
  ...
  metrics: {
    ...
    acceptedCandidateTraceCount: results.filter((result) => result.status === "accepted" && result.route === "historical-candidate").length,
    actionResultPublishedCount: results.filter((result) => result.actionResultPublished === true).length,
    candidateInterfacePublishedCount: results.filter((result) => result.interfacePublished === true && result.route === "historical-candidate").length,
