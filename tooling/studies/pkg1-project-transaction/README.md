...
The current route keeps one physical `package.w` or `workspace.w`. The owner
basis excludes nested `resolution` and `deployments`. A resolution record and
each deployment record have separate derived digests.
