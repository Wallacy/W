...
function checkStudyArtifacts(errors) {
  ...
  for (const token of ["build.w", "package-centric", "exact local root", "w.build/1", "exact selector", "stream fn updates(...): Item throws Failure", "some Stream<Item,Failure>", "ServiceFailure", "willSet", "didSet", "get mut ref + defer"]) if (!candidateText.includes(token)) errors.push(`PFU0 candidate text omits ${token}.`);
