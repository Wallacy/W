...
| GPU compiler/linkage experiment | Canonical `accelerator.module<{...}>()` source now crosses parser and Frontend27 into independently verified, provider-neutral device-module records for the bounded signed-`i32` Hello slice. A caller-owned projection selects one verified module field and carries its const identity, field label, private implementation name, and payload into the target-neutral two-function/seven-operation GPU0 witness. GPU0 emits separate MLIR, lowers through GPU/NVVM to PTX, and returns `42` on the available RTX A400 through a dynamic CUDA Driver adapter. [`GPU0.md`](benchmarks/GPU0.md) records diagnostic phase metrics. Typed launch, public runtime/provider linkage, and public GPU product support remain open. |
