...
| Parallel placement IR | Exact `spawn<.domain>` can cross Frontend28 into HIR37 only with caller-owned concurrent-plus-parallel domain evidence. The HIR owns and independently verifies identity, mode, capability, lexical joins, and a pure non-suspending scalar child graph. |
...
| GPU compiler/linkage experiment | Canonical `accelerator.module<{...}>()` source now crosses parser and Frontend28 into independently verified, provider-neutral device-module records for the bounded signed-`i32` Hello slice. Frontend28 also preserves the exact static accelerated-domain launch relation. ACCINV0 and ACCBIND0 close the independent invocation and root/profile selection; ACCREQ0 now joins that binding with the verified GPU0 device artifact in one copied provider-neutral request. GPU0 emits separate MLIR, lowers through GPU/NVVM to PTX, and returns `42` on the available RTX A400 through a still-separate dynamic CUDA Driver adapter. [`GPU0.md`](benchmarks/GPU0.md) records diagnostic phase metrics. The adapter does not yet consume ACCREQ0; public runtime/provider linkage and public GPU product support remain open. |
