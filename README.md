...
| Windows parallel provider | PARPROV1 consumes measured PARSEL1/PARINV1 records and executes five logical tasks through constant-size Windows waves at capacity one or two. Both capacities produce identical semantic outcomes; a monotonic rendezvous proves two simultaneous workers without timing inference. A seventeen-argument task also crosses the provider. The private PLATFORM1 primitive preserves tagged success/error/canceled completions. PARBIND1 now admits that exact statically linked provider through a compiler-owned local authority, and the separate typed TASKLIFE bridge preserves nominal failure and cleanup order. This is local component evidence, not cryptographic provider authentication, a public W executable, or a CRT-free target artifact. |
