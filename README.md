...
| GPU compiler/linkage experiment | Canonical module-contract source (`module ...<kernels: { label: directFn }>`), with a local defining-module kernel launch, crosses parser and Frontend31 into independently verified, provider-neutral device-module records for the bounded signed-`i32` Hello slice. Frontend31 also preserves the exact static accelerated-domain launch relation. ACCINV0 and ACCBIND0 close the independent invocation and root/profile selection; ACCREQ0 joins that binding with the verified GPU0 device artifact in one copied provider-neutral request. The physical gate consumes its device MLIR, kernel symbol, and expected result, lowers through GPU/NVVM to PTX, and returns `42` on the available RTX A400 through a private dynamic CUDA Driver adapter. [`GPU0.md`](benchmarks/GPU0.md) records diagnostic phase metrics. Kernel imports are currently grammar/CST/module-scan evidence only; multi-module resolution, qualified dynamic launch, product reachability, and public runtime/provider linkage remain open. |
...
- GPU0 has target-neutral semantic records, separate host/device MLIR, an
  ...
  parser and Frontend31 now preserve the canonical module-contract family and
