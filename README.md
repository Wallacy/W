...
| Process/parallel HIR composition | HIR37 now preserves one bounded chain from a resolved process `Arguments.isEmpty` value through a pure scalar binding into one parallel task argument and lexical join. This is verified HIR evidence only; target composition and public execution remain open. |
