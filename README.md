...
| Parallel selection and invocation proof | PARSEL1 measures caller-owned task relations without the PARSEL0 four-task ceiling. PARINV1 then measures exact task and argument storage, normalizes named arguments into parameter order, and evaluates the pure scalar proof without a fixed arity array. A verified source `panic(...)` child is represented as a private non-value task fact and rejected by scalar evaluation without publication. Logical task count remains separate from provider capacity. Fixed PARSEL0/PARINV0 remain compatibility witnesses for downstream bounded slices. PARPANIC1 carries that source-selected panic through a separate private local-provider bridge, and PANICLIFE1 publishes its target-neutral private boundary decision and three ordered events. Public `PanicEvent` and Task ABI remain outside this boundary. |
| Windows parallel provider | PARPROV1 consumes measured PARSEL1/PARINV1 records and executes five logical tasks through constant-size Windows waves at capacity one or two. Both capacities produce identical semantic outcomes; a monotonic rendezvous proves two simultaneous workers without timing inference. A seventeen-argument task also crosses the provider. PLATFORM1 preserves success/error/canceled/panic as distinct private physical completions. PARBIND1 admits the exact statically linked provider through a compiler-owned local authority, preserves nominal failure and cleanup through a separate TASKLIFE bridge, and remains unchanged for its success/typed-error lanes. Separate PARPANIC1 validates the source panic receipt and publishes copied identity/message bytes with no semantic value. PANICLIFE1 consumes that verified view without a provider call or own workspace, and keeps source/provider/receipt facts in provenance. PANICHOSTREG1 adds a private Windows x64 host witness that creates exactly one anonymous event, owns it from `CreateEventW`, and records one `CloseHandle` release transition. This is local Windows component evidence, not cryptographic authentication, physical fault containment, a general resource registry, a public W executable, a Task ABI/runtime, or a CRT-free target artifact. |
...
| GPU compiler/linkage experiment | Canonical module-contract source (`module ...<kernels: { label: directFn }>`), with a local defining-module kernel launch, crosses parser and Frontend32 into independently verified, provider-neutral device-module records for the bounded signed-`i32` Hello slice. Frontend32 also preserves the exact static accelerated-domain launch relation and resolves grouped named kernel projections across explicit local-document edges. ACCINV0 and ACCBIND0 close the independent local invocation and root/profile selection; ACCREQ0 joins that binding with the verified GPU0 device artifact in one copied provider-neutral request. Private ACCPROV0 now consumes that verified request plus a reproducible native-artifact receipt and commits a semantic result only after its provider-neutral lifecycle; the physical gate lowers device MLIR through GPU/NVVM to PTX and returns `42` on the available RTX A400 through the existing dynamic CUDA Driver adapter. [`GPU0.md`](benchmarks/GPU0.md) records diagnostic phase metrics. Qualified projection, imported-invocation HIR, product reachability, public runtime/provider linkage, and other targets remain open. |
...
- GPU0 has target-neutral semantic records, separate host/device MLIR, an
  ...
  parser and Frontend32 now preserve the canonical module-contract family and
  direct local kernel bindings; `w-seed-gpu-module-2` independently
  ...
  ACCPROV0 now provides one private target-neutral launch/join/result boundary:
  its semantic signed-`i32` outcome and lifecycle order exclude concrete
  provider identity, while a separate pointer-free receipt retains physical
  facts and raw status. The existing Windows CUDA path exercises that boundary
  and keeps the plumbing sentinel at `42`; profile authenticity, general
  provider/runtime behavior, and the public W route remain open.
