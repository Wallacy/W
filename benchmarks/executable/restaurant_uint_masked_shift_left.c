// C23 reference for the Restaurant UInt masked-shift-left workload.
...
static uint64_t masked_shift_left_u64(uint64_t value, uint64_t count) {
...
int main(void) {
  ...
  const uint64_t masked =
      masked_shift_left_u64(runtime_value, runtime_count);
