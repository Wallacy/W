...
static const char HIR0_NEVER_NAME[] = "Never";
static const char HIR0_I8_NAME[] = "i8";
...
/* Fixed-width integer records deliberately use canonical spellings in HIR.
 * Int/i64 and UInt/u64 remain the two historical canonical 64-bit records;
 * all other widths use the one appended TYPE_INTEGER kind plus these facts. */
static const char *hir0_fixed_integer_name(bool is_signed,
                                           uint16_t bit_width) {
  if (is_signed) {
    if (bit_width == 8u) return HIR0_I8_NAME;
...
static bool frontend_integer_spelling_matches(
  ...
  const char *canonical =
      hir0_fixed_integer_name(type->is_signed, type->bit_width);
  return canonical != NULL && text_is(type->spelling, canonical);
}

static bool frontend_type_is_fixed_integer(
    const w_seed_frontend_type *type) {
...
static size_t frontend_fixed_integer_type_count(
  ...
  for (size_t sign = 0u; sign < 2u; sign += 1u)
    for (size_t width = 0u; width < sizeof(widths) / sizeof(widths[0]);
      ...
      bool present = false;
      for (size_t index = 0u;
           index < input->frontend_result->written.types; index += 1u) {
        const w_seed_frontend_type *type =
            &input->frontend_output->types[index];
...
static bool frontend_fixed_integer_present(
    const w_seed_hir0_input *input, bool is_signed, uint16_t bit_width) {
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL || input->frontend_output->types == NULL)
    return false;
...
static bool hir0_builtin_integer_wrapping_operation_is_supported(
    w_seed_frontend_builtin_operation operation) {
  return operation == W_SEED_FRONTEND_BUILTIN_INTEGER_WRAPPING_ADD ||
         operation == W_SEED_FRONTEND_BUILTIN_INTEGER_WRAPPING_SUBTRACT ||
         operation == W_SEED_FRONTEND_BUILTIN_INTEGER_WRAPPING_MULTIPLY ||
         operation == W_SEED_FRONTEND_BUILTIN_INTEGER_WRAPPING_NEGATE ||
         operation == W_SEED_FRONTEND_BUILTIN_INTEGER_WRAPPING_POWER ||
         operation == W_SEED_FRONTEND_BUILTIN_INTEGER_WRAPPING_SHIFT_LEFT;
}

static bool hir0_builtin_integer_wrapping_operation_is_unary(
    w_seed_frontend_builtin_operation operation) {
  return operation == W_SEED_FRONTEND_BUILTIN_INTEGER_WRAPPING_NEGATE;
}
...
static bool hir0_builtin_u64_operation_member_matches(
  ...
  return (operation == W_SEED_FRONTEND_BUILTIN_U64_WRAPPING_ADD &&
         ...
         (operation == W_SEED_FRONTEND_BUILTIN_U64_MASKED_SHIFT_LEFT &&
          text_is(member_name, "maskedShiftLeft")) ||
         (operation == W_SEED_FRONTEND_BUILTIN_U64_MASKED_SHIFT_RIGHT &&
...
static bool frontend_type_supported(const w_seed_frontend_type *type) {
  ...
  if (type->kind == W_SEED_FRONTEND_TYPE_NEVER)
    return text_is(type->spelling, HIR0_NEVER_NAME);
...
static bool frontend_type_is_u64(const w_seed_frontend_type *type) {
...
static bool frontend_type_is_i64(const w_seed_frontend_type *type) {
  return type != NULL && text_valid(type->spelling) &&
         type->task_result_type == W_SEED_FRONTEND_NONE &&
         type->kind == W_SEED_FRONTEND_TYPE_INTEGER && type->is_signed &&
         type->bit_width == 64u &&
...
static bool frontend_type_is_u64_bool_tuple(
    const w_seed_frontend_type *type) {
  return type != NULL && text_valid(type->spelling) &&
         type->task_result_type == W_SEED_FRONTEND_NONE &&
         type->kind == W_SEED_FRONTEND_TYPE_TUPLE &&
         text_is(type->spelling, HIR0_U64_BOOL_TUPLE_NAME);
}
...
static bool frontend_has_never_type(const w_seed_hir0_input *input) {
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL || input->frontend_output->types == NULL)
    return false;
  for (size_t index = 0u; index < input->frontend_result->written.types;
       index += 1u)
    if (input->frontend_output->types[index].kind ==
        W_SEED_FRONTEND_TYPE_NEVER)
      return true;
  return false;
}

static bool frontend_has_u64_type(const w_seed_hir0_input *input) {
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL || input->frontend_output->types == NULL)
    return false;
  for (size_t index = 0u; index < input->frontend_result->written.types;
       index += 1u)
    if (frontend_type_is_u64(&input->frontend_output->types[index]))
      return true;
  return false;
}

static bool frontend_has_f64_type(const w_seed_hir0_input *input) {
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL || input->frontend_output->types == NULL)
    return false;
  for (size_t index = 0u; index < input->frontend_result->written.types;
...
static bool frontend_has_u64_bool_tuple_type(
    const w_seed_hir0_input *input) {
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL || input->frontend_output->types == NULL)
    return false;
  for (size_t index = 0u; index < input->frontend_result->written.types;
       index += 1u)
    if (frontend_type_is_u64_bool_tuple(
            &input->frontend_output->types[index]))
      return true;
  return false;
}
...
static bool frontend_hir_type_supported(
  ...
  if (frontend_type_is_u64_bool_tuple(type)) return true;
...
static bool frontend_supported_types_equal(
  ...
  if (!frontend_type_supported(left) || !frontend_type_supported(right))
    return frontend_type_is_u64_bool_tuple(left) &&
           frontend_type_is_u64_bool_tuple(right);
...
static bool frontend_task_result_is_unsupported(
  ...
  for (size_t index = 0u; index < result->written.types; index += 1u) {
    ...
    if (task_result->kind == W_SEED_FRONTEND_TYPE_BOOL ||
        frontend_type_is_fixed_integer(task_result))
      continue;
    return true;
  }
  return false;
...
/* A resolved local call keeps its source callee name as an identifier
 * expression with no value type. It is a designator, not a runtime scalar.
 * The owning CALL record below remains the authority for target identity. */
static bool frontend_elision_call_designator(
  ...
  for (size_t candidate = 0u;
       candidate < input->frontend_result->written.expressions;
       candidate += 1u) {
    const w_seed_frontend_expression *call =
        &input->frontend_output->expressions[candidate];
    if (call->owner_function == function_index &&
        call->kind == W_SEED_FRONTEND_EXPR_CALL &&
        call->left == expression_index)
      return true;
  }
  return false;
}
...
static bool frontend_elision_function_never(
    const w_seed_hir0_input *input, size_t function_index,
    uint32_t required_module, frontend_elision_path *path, size_t depth) {
  if (input == NULL || input->frontend_output == NULL ||
  ...
  const w_seed_frontend_function *function =
      &output->functions[function_index];
  bool valid = (required_module == W_SEED_FRONTEND_NONE ||
                function->module_index == required_module) &&
  ...
  for (size_t expression = 0u;
    ...
    if (value->kind == W_SEED_FRONTEND_EXPR_IDENTIFIER &&
        value->inferred_type == W_SEED_FRONTEND_NONE &&
        frontend_elision_call_designator(input, function_index, expression))
      continue;
 ...
 * pure scalar expressions, bindings, and bounded acyclic local calls already
 * accepted by the ordinary never-suspending proof. Yield is a scheduling
 * opportunity rather than an observable barrier, so a closed launch/join
 * scope may discharge each marker without allocating a Task/frame. */
static bool frontend_elision_function_static_yields(
  ...
  for (size_t expression = 0u;
    ...
    if (callee->module_index != function->module_index || callee->is_async ||
        callee->is_anonymous_entry ||
        !frontend_elision_function_never(input,
                                         value->resolved_function_index,
                                         function->module_index, &path, 0u))
      valid = false;
  }
  return valid && yield_count != 0u;
}

/* Cooperative0 narrows the ordinary static-yield proof to the fixed frame
 * budget.  Keep this cap out of the historical W-1582 predicate: normal HIR
 * still accepts any finite static-yield body. */
static bool frontend_cooperative_function_static_yields(
    const w_seed_hir0_input *input, size_t function_index) {
  if (!frontend_elision_function_static_yields(input, function_index) ||
      input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL ||
      function_index >= input->frontend_result->written.functions)
    return false;
  const w_seed_frontend_function *function =
  ...
  if (!range_valid(function->first_statement, function->statement_count,
    ...
    return false;
  for (size_t expression = 0u;
       expression < input->frontend_result->written.expressions; expression += 1u) {
    const w_seed_frontend_expression *item =
        &input->frontend_output->expressions[expression];
    if (item->owner_function == function_index &&
        item->kind == W_SEED_FRONTEND_EXPR_EXECUTION_YIELD)
      yield_count += 1u;
  }
  return yield_count != 0u &&
...
static bool frontend_physical_trace_preflight(
  ...
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL ||
      (task_limit != 0u && required_task_count > task_limit))
    return false;
  const w_seed_frontend_output *output = input->frontend_output;
  const w_seed_frontend_result *result = input->frontend_result;
  if (result->written.modules != 1u || result->written.entries != 1u ||
      result->written.functions > W_SEED_HIR0_COOPERATIVE_MAX_FUNCTIONS ||
      output->entries == NULL || output->functions == NULL ||
  ...
  if ((!root->is_anonymous_entry && !process_root) ||
      ...
      root->statement_count >
          result->written.statements - root->first_statement)
    return false;
  ...
  size_t first_await_position = 0u;
  size_t last_launch_position = 0u;
  uint32_t statement = root->first_statement;
  for (size_t position = 0u; position < root->statement_count;
       position += 1u) {
    if (statement == W_SEED_FRONTEND_NONE ||
    ...
    if (item->expression_index == W_SEED_FRONTEND_NONE ||
        item->expression_index >= result->written.expressions)
      return false;
    ...
    if (final_process_return) {
      ...
      if (item->kind != W_SEED_FRONTEND_STMT_LET ||
          host_print_seen || joins_started ||
          (task_limit != 0u && launch_count >= task_limit) ||
          item->declared_type != W_SEED_FRONTEND_NONE ||
          item->effective_type == W_SEED_FRONTEND_NONE)
        return false;
      if (expression->task_call_expression == W_SEED_FRONTEND_NONE ||
          expression->task_call_expression >= result->written.expressions)
        return false;
      const w_seed_frontend_expression *call =
          &output->expressions[expression->task_call_expression];
      ...
      if (call->kind != W_SEED_FRONTEND_EXPR_CALL ||
          ...
          (parallel_child
               ...
               : !frontend_cooperative_function_static_yields(
                     input, call->resolved_function_index)))
        return false;
      last_launch_position = position;
      launch_count += 1u;
    } else if (expression->kind == W_SEED_FRONTEND_EXPR_AWAIT) {
      if (item->kind != W_SEED_FRONTEND_STMT_LET ||
          host_print_seen || launch_count == 0u ||
          (task_limit != 0u && await_count >= task_limit) ||
          item->declared_type != W_SEED_FRONTEND_NONE)
        return false;
      if (expression->task_binding_statement == W_SEED_FRONTEND_NONE ||
          expression->task_binding_statement >= result->written.statements)
        return false;
      if (!joins_started) {
        joins_started = true;
        first_await_position = position;
      }
      uint32_t expected_statement = root->first_statement;
      size_t launch_ordinal = 0u;
      bool found = false;
      for (size_t scan = 0u; scan < root->statement_count; scan += 1u) {
        if (expected_statement == W_SEED_FRONTEND_NONE ||
            expected_statement >= result->written.statements)
          return false;
        const w_seed_frontend_statement *candidate =
            &output->statements[expected_statement];
        if (candidate->expression_index == W_SEED_FRONTEND_NONE ||
            candidate->expression_index >= result->written.expressions)
          return false;
        if (output->expressions[candidate->expression_index].kind ==
            expected_launch) {
          if (launch_ordinal == await_count) {
            found = true;
            break;
          }
          launch_ordinal += 1u;
        }
        expected_statement = candidate->next_sibling;
      }
      if (!found || expression->task_binding_statement != expected_statement)
        return false;
      await_count += 1u;
      ...
      process_prelude_count += 1u;
               ...
               input->frontend_input == NULL ||
               input->frontend_input->host_scope == NULL ||
               input->frontend_input->host_scope->symbols == NULL ||
               expression->resolved_host_symbol_index >=
                   input->frontend_input->host_scope->symbol_count ||
               !text_is(input->frontend_input->host_scope
  ...
  if (launch_count == 0u || launch_count != await_count ||
      ...
      last_launch_position >= first_await_position)
    return false;
...
static bool frontend_structured_elision_preflight(
  ...
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL)
    return false;
  if (input->execution_profile ==
      W_SEED_HIR0_EXECUTION_PROFILE_COOPERATIVE_TRACE)
    return frontend_cooperative_trace_preflight(input);
  if (input->execution_profile != W_SEED_HIR0_EXECUTION_PROFILE_NORMAL)
    return false;
  const w_seed_frontend_output *output = input->frontend_output;
  const w_seed_frontend_result *result = input->frontend_result;
  ...
  for (size_t expression = 0u;
    ...
    if (target->is_async &&
        (target->is_const ||
          (!frontend_elision_function_never(
              input, call->resolved_function_index, W_SEED_FRONTEND_NONE,
              &path, 0u) &&
...
static bool frontend_value_common_ok(
  ...
  if (value == NULL || !value->supported || value->module_index != module_index ||
      ...
      (value->kind != W_SEED_FRONTEND_EXPR_PANIC &&
       value->panic_code != W_SEED_FRONTEND_PANIC_CODE_INVALID) ||
      (value->inferred_type != W_SEED_FRONTEND_NONE &&
       ((size_t)value->inferred_type >= input->frontend_result->written.types ||
        !frontend_hir_type_supported(input,
            &input->frontend_output->types[value->inferred_type]))) ||
...
static bool frontend_call_expression_ok(
    ...
    uint32_t root_index, bool result_value, bool allow_throwing,
    bool materialize_result, size_t *expression_cursor,
...
static bool frontend_tuple_projection_value_ok(
    const w_seed_hir0_input *input, size_t module_index,
    size_t function_index, size_t document_index,
    const w_seed_frontend_expression *value);
...
static bool frontend_expression_is_i64(
  ...
  return output != NULL && expression != NULL &&
         ...
         frontend_type_is_i64(&output->types[expression->inferred_type]);
}

static bool frontend_expression_is_u64(
    const w_seed_frontend_output *output,
    const w_seed_frontend_expression *expression) {
  if (output == NULL || expression == NULL ||
      expression->inferred_type == W_SEED_FRONTEND_NONE)
    return false;
  return frontend_type_is_u64(&output->types[expression->inferred_type]);
}

static bool frontend_expression_is_integer(
    const w_seed_frontend_output *output,
    const w_seed_frontend_expression *expression) {
  return output != NULL && expression != NULL &&
         expression->inferred_type != W_SEED_FRONTEND_NONE &&
...
static bool frontend_expression_is_signed_integer(
    const w_seed_frontend_output *output,
    const w_seed_frontend_expression *expression) {
  return frontend_expression_is_integer(output, expression) &&
         output->types[expression->inferred_type].is_signed;
}

static bool frontend_expression_is_unsigned_integer(
    const w_seed_frontend_output *output,
    const w_seed_frontend_expression *expression) {
  return frontend_expression_is_integer(output, expression) &&
         !output->types[expression->inferred_type].is_signed;
...
static bool frontend_expression_is_u64_bool_tuple(
    const w_seed_frontend_output *output,
    const w_seed_frontend_expression *expression) {
  return output != NULL && expression != NULL &&
         expression->inferred_type != W_SEED_FRONTEND_NONE &&
         frontend_type_is_u64_bool_tuple(
             &output->types[expression->inferred_type]);
}
...
static bool frontend_value_tree_ok(
  ...
  if (value->kind == W_SEED_FRONTEND_EXPR_CALL) {
    return frontend_call_expression_ok(
        input, module_index, function_index, document_index, use_statement,
        root_index, true, false, true, expression_cursor, segment_cursor,
  ...
  if (value->kind == W_SEED_FRONTEND_EXPR_MEMBER) {
    if (frontend_tuple_projection_value_ok(
            input, module_index, function_index, document_index, value)) {
      if (!frontend_value_tree_ok(
              input, module_index, function_index, document_index,
              use_statement, value->left, depth + 1u, expression_cursor,
              segment_cursor, const_byte_cursor, value_total, segment_total,
              value_bytes, call_total, argument_total, logical_total) ||
          (size_t)root_index != *expression_cursor ||
          !add_size(*expression_cursor, 1u, expression_cursor) ||
...
static bool frontend_call_expression_ok(
  ...
  if (hir0_builtin_u64_operation_is_supported(call->builtin_operation)) {
    const bool integer_wrapping =
        hir0_builtin_integer_wrapping_operation_is_supported(
            call->builtin_operation);
    const bool unary = integer_wrapping
                           ? hir0_builtin_integer_wrapping_operation_is_unary(
                                 call->builtin_operation)
    ...
    if (receiver->kind != W_SEED_FRONTEND_EXPR_IDENTIFIER ||
        receiver->builtin_operation != W_SEED_FRONTEND_BUILTIN_INTEGER_RECEIVER ||
        !receiver->supported ||
    ...
    for (size_t ordinal = 0u; ordinal < expected_argument_count;
      ...
      const w_seed_frontend_expression *argument_value =
          argument->expression_index == W_SEED_FRONTEND_NONE
              ? NULL
              : &output->expressions[argument->expression_index];
      const w_seed_frontend_type *argument_type =
          argument_value == NULL ||
                  argument_value->inferred_type == W_SEED_FRONTEND_NONE ||
                  (size_t)argument_value->inferred_type >=
                      result->written.types
              ? NULL
              : &output->types[argument_value->inferred_type];
      const bool expected_integer_type =
          integer_wrapping &&
          ((unary || ordinal == 0u ||
            (call->builtin_operation !=
      ...
      if (argument->module_index != module_index ||
          ...
          (integer_wrapping
               ...
               : !frontend_expression_is_u64(output, argument_value)) ||
          !frontend_value_tree_ok(
              input, module_index, function_index, document_index,
  ...
  if (host_call) {
    ...
    if (target_function->is_throws != allow_throwing) return false;
    ...
    if ((!result_value && return_kind != W_SEED_FRONTEND_TYPE_UNIT) ||
        ...
        (return_kind == W_SEED_FRONTEND_TYPE_INTEGER &&
         !frontend_type_is_fixed_integer(&output->types[return_type])))
      return false;
...
/* A `try` expression is a control transfer, not a value wrapper.  The
 * nested throwing call is therefore counted as an invoke-owned call, while
 * its normal and error results are materialized later as typed successor
 * block arguments. */
static bool frontend_try_expression_ok(
  ...
  if (input == NULL || input->frontend_output == NULL ||
      ...
      call_total == NULL || argument_total == NULL || value_total == NULL ||
      segment_total == NULL || value_bytes == NULL || logical_total == NULL ||
      invoke_total == NULL || root_index == W_SEED_FRONTEND_NONE ||
      (size_t)root_index >= input->frontend_result->written.expressions)
    return false;
  const w_seed_frontend_output *output = input->frontend_output;
  const w_seed_frontend_result *result = input->frontend_result;
  const w_seed_frontend_expression *try_expression =
      &output->expressions[root_index];
  if (!frontend_value_common_ok(input, try_expression, module_index,
                                function_index, document_index) ||
      try_expression->kind != W_SEED_FRONTEND_EXPR_TRY ||
      !try_expression->supported ||
      !text_is(try_expression->operator_text, "try") ||
      try_expression->left == W_SEED_FRONTEND_NONE ||
      (size_t)try_expression->left >= result->written.expressions ||
      try_expression->right != W_SEED_FRONTEND_NONE ||
      try_expression->first_argument != W_SEED_FRONTEND_NONE ||
      try_expression->argument_count != 0u ||
      try_expression->first_interpolation_segment != W_SEED_FRONTEND_NONE ||
      try_expression->interpolation_segment_count != 0u ||
      try_expression->const_byte_offset != W_SEED_FRONTEND_NONE ||
      try_expression->const_byte_count != 0u ||
      try_expression->has_bool_value || try_expression->has_integer_value ||
      !frontend_value_has_no_resolution(try_expression))
    return false;
  const w_seed_frontend_function *owner = &output->functions[function_index];
  const w_seed_frontend_expression *call =
  ...
  if (!owner->is_throws || owner->is_const ||
      ...
      (size_t)call->resolved_function_index >= result->written.functions ||
      call->inferred_type == W_SEED_FRONTEND_NONE ||
      try_expression->inferred_type != call->inferred_type)
    return false;
  const w_seed_frontend_function *target =
      &output->functions[call->resolved_function_index];
  if (!target->is_throws || target->is_async ||
      ...
      (size_t)target->error_type >= result->written.types ||
      output->types[owner->error_type].kind != W_SEED_FRONTEND_TYPE_ENUM ||
      output->types[target->error_type].kind != W_SEED_FRONTEND_TYPE_ENUM ||
      output->types[owner->error_type].enum_base_index !=
          try_expression->propagated_error_enum ||
      output->types[target->error_type].enum_base_index !=
          try_expression->propagated_error_enum ||
      !frontend_type_is_scalar(&output->types[call->inferred_type]))
    return false;
  if (!frontend_call_expression_ok(
          input, module_index, function_index, document_index, statement_index,
          try_expression->left, true, true, false, expression_cursor,
      ...
      (size_t)try_expression->left + 1u != *expression_cursor ||
      !add_size(*value_total, 2u, value_total) ||
      !add_size(*invoke_total, 1u, invoke_total) ||
      !add_size(*expression_cursor, 1u, expression_cursor))
    return false;
  return true;
}
...
static bool frontend_task_launch_expression_ok(
  ...
  if (call->kind != W_SEED_FRONTEND_EXPR_CALL ||
      ...
      !frontend_call_expression_ok(
          input, module_index, function_index, document_index, statement_index,
          launch->task_call_expression, true, false, true, expression_cursor,
          interpolation_segment_cursor, const_byte_cursor, call_total,
          argument_total, value_total, segment_total, value_bytes,
          logical_total) ||
      (size_t)root_index != *expression_cursor ||
...
/* Panic is a statement-level flow terminator in this bounded HIR slice.  Its
 * one admitted argument is still walked through the ordinary frontend value
 * tree, so literal bytes, source order, and the no-call boundary remain
 * independently checkable.  The panic owner itself is deliberately not
 * counted as a value root. */
static bool frontend_panic_expression_ok(hir0_statement_walk *walk,
                                         uint32_t statement_index,
                                         uint32_t root_index) {
  if (walk == NULL || root_index == W_SEED_FRONTEND_NONE ||
      (size_t)root_index >= walk->result->written.expressions ||
      walk->arguments == NULL || walk->expression_cursor == NULL)
    return false;
  const w_seed_frontend_expression *root =
      &walk->output->expressions[root_index];
  if (!frontend_value_common_ok(walk->input, root, walk->module_index,
                                walk->function_index, walk->document_index) ||
      root->kind != W_SEED_FRONTEND_EXPR_PANIC || !root->supported ||
      !text_is(root->operator_text, "explicit") ||
      root->panic_code != W_SEED_FRONTEND_PANIC_CODE_EXPLICIT ||
      root->left != W_SEED_FRONTEND_NONE ||
      root->right != W_SEED_FRONTEND_NONE ||
      root->inferred_type == W_SEED_FRONTEND_NONE ||
      (size_t)root->inferred_type >= walk->result->written.types ||
      walk->output->types[root->inferred_type].kind !=
          W_SEED_FRONTEND_TYPE_NEVER ||
      root->first_argument != *walk->arguments ||
      root->argument_count != 1u ||
      !range_valid(root->first_argument, root->argument_count,
                   walk->result->written.arguments) ||
      !frontend_value_has_no_resolution(root) ||
      root->resolved_binding_statement != W_SEED_FRONTEND_NONE ||
      root->const_byte_offset != W_SEED_FRONTEND_NONE ||
      root->const_byte_count != 0u || root->has_bool_value ||
      ...
      root->first_interpolation_segment != W_SEED_FRONTEND_NONE ||
      root->interpolation_segment_count != 0u ||
      root->else_expression != W_SEED_FRONTEND_NONE ||
      root->task_result_type != W_SEED_FRONTEND_NONE ||
      root->task_call_expression != W_SEED_FRONTEND_NONE ||
      root->task_binding_statement != W_SEED_FRONTEND_NONE ||
      root->propagated_error_enum != W_SEED_FRONTEND_NONE ||
      !text_valid(root->spelling))
    return false;
  const w_seed_frontend_argument *argument =
      &walk->output->arguments[root->first_argument];
  if (argument->module_index != walk->module_index ||
      argument->owner_expression != root_index ||
      argument->resolved_parameter_ordinal != W_SEED_FRONTEND_NONE ||
      argument->label.length != 0u || !text_valid(argument->label) ||
      argument->expression_index == W_SEED_FRONTEND_NONE ||
      (size_t)argument->expression_index >= walk->result->written.expressions ||
      !frontend_span_ok(&walk->input->frontend_input
                             ->documents[walk->document_index],
                        argument->span) ||
      !span_equal(argument->span,
                  walk->output->expressions[argument->expression_index].span) ||
      (size_t)argument->expression_index != *walk->expression_cursor)
    return false;
  if (!frontend_value_tree_ok(
          walk->input, walk->module_index, walk->function_index,
          walk->document_index, statement_index, argument->expression_index,
          0u, walk->expression_cursor, walk->interpolation_segment_cursor,
          walk->const_byte_cursor, walk->values, walk->segments,
          walk->value_bytes, walk->calls, walk->arguments,
          walk->logical_total))
    return false;
  const w_seed_frontend_expression *message =
      &walk->output->expressions[argument->expression_index];
  if (message->kind != W_SEED_FRONTEND_EXPR_STRING || !message->supported ||
      message->const_byte_offset == W_SEED_FRONTEND_NONE ||
      message->const_byte_count > W_SEED_HIR0_MAX_VALUE_BYTES ||
      (size_t)root_index != *walk->expression_cursor)
    return false;
  if (!add_size(*walk->expression_cursor, 1u, walk->expression_cursor) ||
      !add_size(*walk->arguments, 1u, walk->arguments))
    return false;
  return true;
}
...
static bool hir0_walk_statement(hir0_statement_walk *walk, uint32_t index,
  ...
  if (statement->kind == W_SEED_FRONTEND_STMT_EXPRESSION) {
    ...
    if (expression->kind == W_SEED_FRONTEND_EXPR_PANIC) {
      /* Panic is a terminal flow edge. It is allowed in a terminal branch,
       * but never in a branch that is expected to continue normally. */
      if ((branch && !allow_branch_exit) ||
          statement->next_sibling != W_SEED_FRONTEND_NONE ||
          !frontend_panic_expression_ok(walk, index,
  ...
  if (statement->kind == W_SEED_FRONTEND_STMT_DEFER) {
    /* HIR41 admits one lexical cleanup registration for the bounded program
     * subset (and therefore at most one in any function). Its body is still a
     * source statement, but it is lowered twice as an ordinary direct Unit
     * call: once on each typed INVOKE successor. */
    if (branch || walk->loop_seen || walk->cleanup_seen ||
        statement->expression_index != W_SEED_FRONTEND_NONE ||
        statement->condition_expression != W_SEED_FRONTEND_NONE ||
        statement->first_child == W_SEED_FRONTEND_NONE ||
    ...
    if (body->module_index != walk->module_index ||
        body->owner_function != walk->function_index ||
        !frontend_span_ok(&walk->input->frontend_input
                               ->documents[walk->document_index],
                          body->span) ||
        body->kind != W_SEED_FRONTEND_STMT_EXPRESSION ||
        body->next_sibling != W_SEED_FRONTEND_NONE ||
        body->first_child != W_SEED_FRONTEND_NONE ||
        body->child_count != 0u || body->else_child != W_SEED_FRONTEND_NONE ||
        body->condition_expression != W_SEED_FRONTEND_NONE ||
        body->range_lower_expression != W_SEED_FRONTEND_NONE ||
        body->range_upper_expression != W_SEED_FRONTEND_NONE ||
        body->loop_local_ordinal != W_SEED_FRONTEND_NONE ||
        body->binding_name.length != 0u ||
        body->declared_type != W_SEED_FRONTEND_NONE ||
        body->effective_type != W_SEED_FRONTEND_NONE ||
        body->expression_index == W_SEED_FRONTEND_NONE ||
        (size_t)body->expression_index >= walk->result->written.expressions)
      return false;
    if (!frontend_call_expression_ok(
            walk->input, walk->module_index, walk->function_index,
            walk->document_index, statement->first_child,
            body->expression_index, false, false, false,
            walk->expression_cursor, walk->interpolation_segment_cursor,
            walk->const_byte_cursor, walk->calls, walk->arguments,
            walk->values, walk->segments, walk->value_bytes,
            walk->logical_total))
      return false;
    walk->cleanup_seen = true;
    return add_size(*walk->cleanup_total, 1u, walk->cleanup_total);
  }
  ...
  if (statement->kind == W_SEED_FRONTEND_STMT_RETURN) {
    ...
    if (statement->expression_index == W_SEED_FRONTEND_NONE ||
      ...
      return false;
    if (return_expression->kind == W_SEED_FRONTEND_EXPR_TRY) {
...
static bool frontend_statement_and_expression_cfg_ok(
    ...
    size_t *switch_capture_total, size_t *cleanup_total) {
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL || binding_total == NULL ||
      call_total == NULL || invoke_total == NULL || yield_total == NULL ||
  ...
  for (size_t function_index = 0u;
    ...
    hir0_statement_walk walk = {
        ...
        .cleanup_total = &function_cleanup_count,
    ...
    if (walk.cleanup_seen) {
       ...
       * return-try in this function. This keeps the HIR obligation tied to
       * one INVOKE rather than inventing a join or a runtime stack. */
      uint32_t tail = function->first_statement;
      uint32_t last = W_SEED_FRONTEND_NONE;
      size_t tail_guard = 0u;
      while (tail != W_SEED_FRONTEND_NONE &&
             tail_guard < function->statement_count) {
        if ((size_t)tail >= result->written.statements) return false;
        last = tail;
        tail = output->statements[tail].next_sibling;
        tail_guard += 1u;
      }
      if (tail != W_SEED_FRONTEND_NONE ||
          last == W_SEED_FRONTEND_NONE ||
          output->statements[last].kind != W_SEED_FRONTEND_STMT_RETURN ||
          output->statements[last].expression_index ==
              W_SEED_FRONTEND_NONE ||
          (size_t)output->statements[last].expression_index >=
              result->written.expressions ||
          output->expressions[output->statements[last].expression_index].kind !=
    ...
    if (!add_size(cleanup_count, function_cleanup_count, &cleanup_count) ||
        cleanup_count > 1u)
      return false;
  }
  if (arguments != result->written.arguments ||
      ...
      !count_u32(bindings) || !count_u32(calls) || !count_u32(invokes) ||
      !count_u32(yields) ||
      !count_u32(arguments) ||
      ...
      !count_u32(cleanup_count))
    return false;
  *binding_total = bindings;
  *call_total = calls;
  *invoke_total = invokes;
  *yield_total = yields;
...
static bool frontend_statement_and_expression_ok(
  ...
  return frontend_statement_and_expression_cfg_ok(
      input, binding_total, call_total, invoke_total, yield_total,
      argument_total, value_total,
      ...
      switch_capture_total, cleanup_total);
}
...
static bool frontend_panic_argument_count(const w_seed_hir0_input *input,
                                          size_t *total) {
  if (input == NULL || input->frontend_output == NULL ||
      input->frontend_result == NULL || total == NULL)
    return false;
...
static bool frontend_tuple_projection_value_ok(
  ...
  if (input == NULL || input->frontend_output == NULL ||
      ...
      value->inferred_type == W_SEED_FRONTEND_NONE ||
      (size_t)value->inferred_type >= input->frontend_result->written.types ||
      !frontend_tuple_projection_has_no_resolution(value) ||
      value->const_byte_offset != W_SEED_FRONTEND_NONE ||
      value->const_byte_count != 0u || value->has_bool_value ||
      value->has_integer_value || value->has_float_value ||
      value->first_interpolation_segment != W_SEED_FRONTEND_NONE ||
  ...
  const w_seed_frontend_expression *receiver =
      &input->frontend_output->expressions[value->left];
  if (!frontend_expression_is_u64_bool_tuple(input->frontend_output,
                                             receiver))
    return false;
  return (text_is(value->member_name, "0") &&
          frontend_expression_is_u64(input->frontend_output, value)) ||
         (text_is(value->member_name, "1") &&
  ...
  while (cursor != W_SEED_FRONTEND_NONE &&
    ...
    if (statement->next_sibling == W_SEED_FRONTEND_NONE) {
      ...
      if (statement->kind == W_SEED_FRONTEND_STMT_EXPRESSION &&
          ...
          (size_t)statement->expression_index <
              input->frontend_result->written.expressions)
        return input->frontend_output
                   ->expressions[statement->expression_index]
                   .kind == W_SEED_FRONTEND_EXPR_PANIC;
      return false;
    }
...
static bool text_size_for_input(const w_seed_hir0_input *input, size_t *total) {
  ...
  if (frontend_has_never_type(input) && !add_size(value, 5u, &value))
    return false;
  if (frontend_has_u64_bool_tuple_type(input) &&
      !add_size(value, sizeof(HIR0_U64_BOOL_TUPLE_NAME) - 1u, &value))
    return false;
  if (frontend_has_public_process(input) && !add_size(value, 5u, &value))
    return false;
  ...
  for (size_t sign = 0u; sign < 2u; sign += 1u)
    for (size_t width = 0u;
         ...
         width += 1u) {
      if (!frontend_fixed_integer_present(input, sign != 0u,
                                          fixed_widths[width]))
        continue;
      const char *name =
          hir0_fixed_integer_name(sign != 0u, fixed_widths[width]);
      if (name == NULL || !add_size(value, strlen(name), &value)) return false;
    }
  for (size_t index = 0u; index < result->written.modules; index += 1u) {
    const w_seed_frontend_module *module = &output->modules[index];
...
static hir0_prepare_status collect(const w_seed_hir0_input *input,
  ...
  if ((input->frontend_input->external_module_count == 0u &&
    ...
    return HIR0_PREPARE_UNSUPPORTED;
  if (!frontend_module_ranges_ok(input) ||
      !frontend_type_records_ok(input) ||
  ...
  size_t panic_argument_count = 0u;
  ...
  if (!frontend_statement_and_expression_ok(
          input, &binding_count, &call_count, &invoke_count, &yield_count,
          &argument_count, &value_count, &interpolation_segment_count,
  ...
  if (!frontend_external_enum_constructor_argument_count(
      ...
      external_enum_constructor_argument_count > argument_count -
                                                 enum_payload_count)
    return HIR0_PREPARE_UNSUPPORTED;
  if (!frontend_panic_argument_count(input, &panic_argument_count) ||
      panic_argument_count > argument_count - enum_payload_count -
  ...
  if (!add_size(modules, functions, &identities) ||
      ...
      !count_u32(switch_capture_count) || !count_u32(cleanup_count))
    return HIR0_PREPARE_UNSUPPORTED;
  ...
  const size_t fixed_integer_type_count =
      frontend_fixed_integer_type_count(input);
  ...
  if (!add_size(counts->types, enum_subset_type_count, &counts->types) ||
      ...
      !add_size(counts->types, frontend_has_never_type(input) ? 1u : 0u,
                &counts->types) ||
      !add_size(counts->types,
                frontend_has_u64_bool_tuple_type(input) ? 1u : 0u,
                &counts->types) ||
      !add_size(counts->types, counts->external_symbols == 7u ? 1u : 0u,
                &counts->types) ||
      !add_size(counts->types, fixed_integer_type_count, &counts->types) ||
  ...
  if (!add_size(if_count, logical_count, &diamond_count) ||
      ...
      invoke_count > (SIZE_MAX - block_count) / 2u ||
      !add_size(block_count, invoke_count * 2u, &block_count) ||
  ...
  if (invoke_count > (SIZE_MAX - block_argument_count) / 2u ||
      !add_size(block_argument_count, invoke_count * 2u,
                &block_argument_count))
    return HIR0_PREPARE_UNSUPPORTED;
  if (!count_u32(block_argument_count)) return HIR0_PREPARE_UNSUPPORTED;
  counts->block_arguments = block_argument_count;
  if (explicit_edge_block_argument_count > SIZE_MAX / 2u ||
      !add_size(explicit_edge_block_argument_count,
                explicit_edge_block_argument_count,
                &counts->edge_arguments) ||
      !count_u32(counts->edge_arguments))
    return HIR0_PREPARE_UNSUPPORTED;
  counts->bindings = binding_count;
  if (invoke_count > call_count ||
      !add_size(binding_count, call_count - invoke_count,
                &counts->instructions) ||
...
#define HIR0_OUTPUT(field, capacity_field)                                     \
  ...
  HIR0_OUTPUT(cleanups, cleanup_capacity);
#undef HIR0_OUTPUT
...
#define HIR0_ADD_OUTPUT(field, capacity_field, type)                          \
  ...
  HIR0_ADD_OUTPUT(cleanups, cleanup_capacity, w_seed_hir0_cleanup);
...
#define HIR0_ADD_PROGRAM(field, capacity_field, type)                         \
  ...
  HIR0_ADD_PROGRAM(external_symbols, external_symbol_capacity,
                   w_seed_hir0_external_symbol);
  HIR0_ADD_PROGRAM(cleanups, cleanup_capacity, w_seed_hir0_cleanup);
...
static uint32_t hir_type_from_frontend(const w_seed_frontend_output *output,
  ...
  for (size_t index = 0u; index < result->written.types; index += 1u)
    if (output->types[index].kind == W_SEED_FRONTEND_TYPE_NEVER) {
      has_never_type = true;
      ...
      has_f64_type = true;
    } else if (frontend_type_is_u64_bool_tuple(&output->types[index])) {
      has_u64_bool_tuple_type = true;
    }
  if (type->kind == W_SEED_FRONTEND_TYPE_ENUM &&
      type->enum_base_index != W_SEED_FRONTEND_NONE) {
  ...
  const size_t optional_type_base = local_enum_type_base +
                                    result->written.enums +
                                    enum_subset_type_count;
  ...
  if (frontend_type_is_noncanonical_integer(type)) {
    ...
    for (size_t sign = 0u; sign < 2u; sign += 1u)
      for (size_t width = 0u; width < 3u; width += 1u) {
        ...
        if (present) {
          if (sign == (type->is_signed ? 1u : 0u) &&
              widths[width] == type->bit_width)
            return (uint32_t)(integer_type_base + compact_ordinal);
          compact_ordinal += 1u;
        }
      }
    return W_SEED_HIR0_NONE;
  }
  if (frontend_type_is_u64(type) && has_u64_type)
    return (uint32_t)optional_type_base;
...
static uint32_t hir0_emit_block_argument_read_m2(
  ...
  if (context == NULL || owner_block >= context->counts->blocks ||
    ...
    return W_SEED_HIR0_NONE;
  const uint32_t result = (uint32_t)*context->value_index;
  const uint32_t type_index = context->output
                                  ->block_arguments[block_argument_index]
                                  .type_index;
  context->output->values[*context->value_index] = (w_seed_hir0_value){
...
static size_t hir0_emit_expression_values_m2(hir0_emit_context *context,
  ...
  if (source->kind == W_SEED_FRONTEND_EXPR_TRY)
    return hir0_emit_expression_values_m2(context, source->left, current_block,
                                          statement_index, depth + 1u);
  if (source->kind == W_SEED_FRONTEND_EXPR_PANIC) {
...
static void hir0_emit_try_return_terms_m2(
    hir0_emit_context *context, const w_seed_frontend_statement *statement,
    size_t invoke_block) {
  const size_t normal_block = invoke_block + 1u;
  const size_t error_block = invoke_block + 2u;
  const w_seed_frontend_expression *try_expression =
      &context->frontend->expressions[statement->expression_index];
  ...
  while (cursor != W_SEED_FRONTEND_NONE &&
    ...
    if (statement->kind == W_SEED_FRONTEND_STMT_LET ||
      ...
      const w_seed_frontend_expression *root =
          &context->frontend->expressions[statement->expression_index];
      if (statement->kind == W_SEED_FRONTEND_STMT_EXPRESSION &&
          root->kind == W_SEED_FRONTEND_EXPR_PANIC) {
        current_block = hir0_emit_expression_terms_m2(
            context, statement->expression_index, current_block, cursor, 0u);
        const uint32_t message_expression =
            context->frontend->arguments[(size_t)root->first_argument]
                .expression_index;
        context->output->terminators[current_block].value_index =
            hir0_emit_value_m2(
                context, message_expression,
                W_SEED_HIR0_VALUE_OWNER_TERMINATOR,
                (uint32_t)current_block, 0u, current_block, 0u);
        return;
      }
      ...
      const w_seed_frontend_expression *root =
          &context->frontend->expressions[statement->expression_index];
      if (statement->kind == W_SEED_FRONTEND_STMT_RETURN &&
          root->kind == W_SEED_FRONTEND_EXPR_TRY) {
        hir0_emit_try_return_terms_m2(context, statement, current_block);
        return;
      }
      if (statement->kind == W_SEED_FRONTEND_STMT_RETURN &&
          root->kind == W_SEED_FRONTEND_EXPR_SWITCH) {
...
static void hir0_set_panic_m2(hir0_emit_context *context, size_t block_index,
                              const w_seed_frontend_expression *source,
                              w_seed_span span) {
  hir0_finish_block_m2(context, block_index);
  const w_seed_hir0_block *block = &context->output->blocks[block_index];
  context->output->terminators[block_index] = (w_seed_hir0_terminator){
      .owner_block = (uint32_t)block_index,
      .kind = W_SEED_HIR0_TERMINATOR_PANIC,
      .ordinal = block->instruction_count,
      .call_index = W_SEED_HIR0_NONE,
      .value_index = W_SEED_HIR0_NONE,
      .result_type = hir_type_from_frontend(
          context->frontend, context->frontend_result,
          source == NULL ? W_SEED_FRONTEND_NONE : source->inferred_type),
      ...
      .source_span = span,
      .panic_code = W_SEED_HIR0_PANIC_CODE_EXPLICIT};
}
...
static size_t hir0_emit_call_layout_m2(hir0_emit_context *context,
  ...
  *context->argument_offset += source->argument_count;
  ...
  const uint32_t emitted_instruction =
      invoke ? W_SEED_HIR0_NONE : (uint32_t)*context->instruction_offset;
  w_seed_hir0_block *block = &context->output->blocks[current_block];
  w_seed_hir0_instruction *instruction = NULL;
  if (!invoke) {
    hir0_begin_block_m2(context, current_block);
    instruction = &context->output->instructions[*context->instruction_offset];
  ...
  call->owner_terminator = invoke ? (uint32_t)current_block
                                  : W_SEED_HIR0_NONE;
  call->owner_block = (uint32_t)current_block;
  call->ordinal = invoke
                      ? context->output->blocks[current_block].instruction_count
                      : instruction->ordinal;
  ...
  if (!invoke) instruction->result_type = call->result_type;
  for (size_t argument = 0u; argument < source->argument_count; argument += 1u)
    context->output->arguments[first_argument + argument].owner_call =
        emitted_call;
  if (!invoke) *context->instruction_offset += 1u;
  *context->call_offset += 1u;
  return current_block;
}

/* Locate the one source cleanup already admitted by the frontend walk.  The
 * emitter never lowers the defer statement itself; it reuses its direct call
 * expression for each typed successor of the owning INVOKE. */
static bool hir0_cleanup_source_m2(
    const hir0_emit_context *context,
    const w_seed_frontend_statement **cleanup_statement,
    const w_seed_frontend_statement **body_statement,
    uint32_t *call_expression, bool *found) {
  if (context == NULL || context->frontend == NULL ||
      context->frontend_result == NULL || cleanup_statement == NULL ||
      body_statement == NULL || call_expression == NULL || found == NULL ||
      context->function >= context->frontend_result->written.functions)
    return false;
  *cleanup_statement = NULL;
  *body_statement = NULL;
  *call_expression = W_SEED_FRONTEND_NONE;
  *found = false;
  const w_seed_frontend_function *function =
      &context->frontend->functions[context->function];
  ...
  while (cursor != W_SEED_FRONTEND_NONE &&
    ...
    const w_seed_frontend_statement *statement =
        &context->frontend->statements[cursor];
    if (statement->owner_function != context->function) return false;
    if (statement->kind == W_SEED_FRONTEND_STMT_DEFER) {
      if (*found || statement->first_child == W_SEED_FRONTEND_NONE ||
          statement->child_count != 1u ||
      ...
      if (body->owner_function != context->function ||
          body->kind != W_SEED_FRONTEND_STMT_EXPRESSION ||
          body->next_sibling != W_SEED_FRONTEND_NONE ||
          body->expression_index == W_SEED_FRONTEND_NONE ||
          (size_t)body->expression_index >=
              context->frontend_result->written.expressions)
        return false;
      const w_seed_frontend_expression *call =
          &context->frontend->expressions[body->expression_index];
      if (call->kind != W_SEED_FRONTEND_EXPR_CALL ||
          call->resolved_callee_kind !=
              W_SEED_FRONTEND_CALLEE_LOCAL_FUNCTION ||
          call->argument_count != 0u)
        return false;
      *cleanup_statement = statement;
      *body_statement = body;
      *call_expression = body->expression_index;
      *found = true;
    }
    cursor = statement->next_sibling;
    guard += 1u;
...
static void hir0_emit_try_return_layout_m2(
    hir0_emit_context *context, const w_seed_frontend_statement *statement,
    size_t current_block) {
  const w_seed_frontend_expression *try_expression =
      &context->frontend->expressions[statement->expression_index];
  ...
  const size_t normal_block = invoke_block + 1u;
  const size_t error_block = invoke_block + 2u;
  const uint32_t call_index = hir0_find_call_m2(
      context, try_expression->left, invoke_block);
  const uint32_t result_type = hir_type_from_frontend(
      context->frontend, context->frontend_result,
      try_expression->inferred_type);
  const uint32_t error_type = hir_type_from_frontend(
      context->frontend, context->frontend_result,
      context->frontend->functions[context->function].error_type);
  const w_seed_frontend_statement *cleanup_statement = NULL;
  const w_seed_frontend_statement *cleanup_body = NULL;
  uint32_t cleanup_expression = W_SEED_FRONTEND_NONE;
  bool has_cleanup = false;
  if (!hir0_cleanup_source_m2(context, &cleanup_statement, &cleanup_body,
                              &cleanup_expression, &has_cleanup))
    return;
  hir0_finish_block_m2(context, invoke_block);
  context->output->terminators[invoke_block] = (w_seed_hir0_terminator){
      .owner_block = (uint32_t)invoke_block,
      .kind = W_SEED_HIR0_TERMINATOR_INVOKE,
      .ordinal = context->output->blocks[invoke_block].instruction_count,
  ...
  *context->block_argument_index += 1u;
  if (has_cleanup)
    (void)hir0_emit_call_layout_m2(
        context, cleanup_expression, normal_block,
        (uint32_t)(cleanup_body - context->frontend->statements), 0u, false);
  hir0_finish_block_m2(context, normal_block);
  context->output->terminators[normal_block] = (w_seed_hir0_terminator){
      .owner_block = (uint32_t)normal_block,
  ...
  *context->block_argument_index += 1u;
  if (has_cleanup)
    (void)hir0_emit_call_layout_m2(
        context, cleanup_expression, error_block,
        (uint32_t)(cleanup_body - context->frontend->statements), 0u, false);
  hir0_finish_block_m2(context, error_block);
  context->output->terminators[error_block] = (w_seed_hir0_terminator){
      .owner_block = (uint32_t)error_block,
      ...
      .logical_operator = W_SEED_HIR0_LOGICAL_NONE,
      .source_span = statement->span};
  if (call_index != W_SEED_HIR0_NONE)
    context->output->calls[call_index].owner_terminator =
        (uint32_t)invoke_block;
  if (has_cleanup) {
    const uint32_t normal_call =
        hir0_find_call_m2(context, cleanup_expression, normal_block);
    const uint32_t error_call =
        hir0_find_call_m2(context, cleanup_expression, error_block);
    if (context->cleanup_index == NULL ||
        *context->cleanup_index >= context->counts->cleanups ||
        normal_call == W_SEED_HIR0_NONE || error_call == W_SEED_HIR0_NONE ||
        context->output->blocks[normal_block].instruction_count != 1u ||
        context->output->blocks[error_block].instruction_count != 1u)
      return;
    const size_t cleanup_index = *context->cleanup_index;
    const uint32_t normal_instruction =
        context->output->blocks[normal_block].first_instruction;
    const uint32_t error_instruction =
        context->output->blocks[error_block].first_instruction;
    context->output->cleanups[cleanup_index] = (w_seed_hir0_cleanup){
        .owner_function = (uint32_t)context->function,
        .invoke_terminator = (uint32_t)invoke_block,
        .cleanup_identity = context->output->calls[normal_call].callee_identity,
        .normal_block = (uint32_t)normal_block,
        .error_block = (uint32_t)error_block,
        .normal_instruction = normal_instruction,
        .error_instruction = error_instruction,
        .normal_call = normal_call,
        .error_call = error_call,
        .source_span = cleanup_statement->span};
    *context->cleanup_index += 1u;
  }
}
  ...
  while (cursor != W_SEED_FRONTEND_NONE &&
    ...
    if (statement->kind == W_SEED_FRONTEND_STMT_LET ||
      ...
      if (statement->kind == W_SEED_FRONTEND_STMT_EXPRESSION) {
        ...
        if (expression->kind == W_SEED_FRONTEND_EXPR_PANIC) {
          current_block = hir0_emit_expression_layout_m2(
              context, statement->expression_index, current_block, cursor, 0u);
          hir0_set_panic_m2(context, current_block, expression,
                            statement->span);
          return;
        }
        if (expression->kind == W_SEED_FRONTEND_EXPR_ASSIGNMENT) {
      ...
      if (statement->kind == W_SEED_FRONTEND_STMT_RETURN &&
          context->frontend->expressions[statement->expression_index].kind ==
              W_SEED_FRONTEND_EXPR_TRY) {
        hir0_emit_try_return_layout_m2(context, statement, current_block);
        return;
      }
      if (statement->kind == W_SEED_FRONTEND_STMT_RETURN &&
          context->frontend->expressions[statement->expression_index].kind ==
...
static size_t hir0_emit_expression_layout_m2(hir0_emit_context *context,
  ...
  if (frontend_task_launch_kind(source->kind)) {
    ...
    const w_seed_hir0_input elision_input = {
        .frontend_input = context->frontend_input,
        .frontend_output = context->frontend,
        .frontend_result = context->frontend_result};
    const bool cooperative_trace =
        context->frontend_input != NULL &&
  ...
  if (source->kind == W_SEED_FRONTEND_EXPR_AWAIT)
    return hir0_emit_expression_layout_m2(context, source->left, current_block,
                                          statement_index, depth + 1u);
  if (source->kind == W_SEED_FRONTEND_EXPR_TRY)
    return hir0_emit_call_layout_m2(context, source->left, current_block,
                                    statement_index, depth + 1u, true);
  if (source->kind == W_SEED_FRONTEND_EXPR_PANIC) {
    size_t block = current_block;
    for (size_t ordinal = 0u; ordinal < source->argument_count; ordinal += 1u)
  ...
  if (source->kind == W_SEED_FRONTEND_EXPR_CALL) {
    ...
    return hir0_emit_call_layout_m2(context, expression, current_block,
                                    statement_index, depth + 1u, false);
  }
  if (source->kind == W_SEED_FRONTEND_EXPR_INTERPOLATED_STRING) {
    size_t block = current_block;
    for (size_t ordinal = 0u; ordinal < source->interpolation_segment_count;
...
static uint32_t hir0_emit_value_m2(
  ...
  if (source->kind == W_SEED_FRONTEND_EXPR_CALL &&
    ...
    if (hir0_builtin_u64_operation_is_unary(
      ...
      if (source->builtin_operation ==
        ...
        unary_operator = W_SEED_HIR0_UNARY_REVERSED_BYTES;
      context->output->values[*context->value_index] = (w_seed_hir0_value){
          .kind = signed_result ? W_SEED_HIR0_VALUE_UNARY_I64
    ...
    if (source->builtin_operation ==
      ...
      binary_operator = W_SEED_HIR0_BINARY_SATURATING_MULTIPLY;
    else if (source->builtin_operation ==
    ...
    context->output->values[*context->value_index] = (w_seed_hir0_value){
        .kind = signed_result ? W_SEED_HIR0_VALUE_BINARY_I64
  ...
  if (source->kind == W_SEED_FRONTEND_EXPR_MEMBER) {
    const bool tuple_projection =
        source->left != W_SEED_FRONTEND_NONE &&
        frontend_expression_is_u64_bool_tuple(
            context->frontend,
            &context->frontend->expressions[source->left]) &&
        (text_is(source->member_name, "0") ||
         text_is(source->member_name, "1"));
    const uint32_t receiver = hir0_emit_value_m2(
        context, source->left,
        tuple_projection ? W_SEED_HIR0_VALUE_OWNER_TUPLE_ELEMENT
    ...
    w_seed_hir0_value *target =
        &context->output->values[*context->value_index];
    *target = (w_seed_hir0_value){
        .kind = tuple_projection ? W_SEED_HIR0_VALUE_TUPLE_ELEMENT
        ...
        .unsigned_integer_value =
            tuple_projection && text_is(source->member_name, "1") ? 1u : 0u,
        .float_bits = 0u,
        .bool_value = false,
        .byte_offset = 0u,
        .byte_count = 0u,
        .source_span = source->span,
        .external_module_index = tuple_projection
                                     ? W_SEED_HIR0_NONE
                                     : source->resolved_external_module_index,
        .external_symbol_index = tuple_projection
                                     ? W_SEED_HIR0_NONE
                                     : source->resolved_external_symbol_index,
        .member_name = tuple_projection
                           ? (w_seed_hir0_text){0u, 0u}
                           : context->output->external_symbols[
                                 source->resolved_external_symbol_index]
                                 .name,
  ...
  *context->value_index += 1u;
  if (source->kind == W_SEED_FRONTEND_EXPR_STRING) {
    ...
    } else if (frontend_expression_is_unsigned_integer(context->frontend,
                                                       source)) {
      target->kind = W_SEED_HIR0_VALUE_CONST_U64;
      target->type_index = hir_type_from_frontend(
          context->frontend, context->frontend_result, source->inferred_type);
      (void)frontend_integer_u64(source, &target->unsigned_integer_value);
    } else {
  ...
  } else if (source->kind == W_SEED_FRONTEND_EXPR_FLOAT) {
    target->kind = W_SEED_HIR0_VALUE_CONST_FLOAT;
    target->type_index = hir_type_from_frontend(
        context->frontend, context->frontend_result, source->inferred_type);
    target->float_bits = source->float_bits;
...
static void emit_records(const w_seed_hir0_input *input,
  ...
  zero_bytes(output->cleanups, counts->cleanups * sizeof(*output->cleanups));
  ...
  const size_t optional_type_base =
      4u + (counts->external_modules == 0u ? 0u : 3u);
  size_t optional_type_index =
      optional_type_base + counts->enums + counts->enum_subsets;
  size_t usize_type_index = SIZE_MAX;
  text_offset = 15u;
  if (has_u64_type) {
    (void)memcpy(output->text_bytes + text_offset, HIR0_U64_NAME, 3u);
  ...
  if (has_never_type) {
    (void)memcpy(output->text_bytes + text_offset, HIR0_NEVER_NAME,
                 sizeof(HIR0_NEVER_NAME) - 1u);
    output->types[optional_type_index] = (w_seed_hir0_type){
        .kind = W_SEED_HIR0_TYPE_NEVER,
        .owner_module = W_SEED_HIR0_NONE,
        .name = {(uint32_t)text_offset, 5u},
        .external_module_index = W_SEED_HIR0_NONE,
        .external_symbol_index = W_SEED_HIR0_NONE,
        .enum_index = W_SEED_HIR0_NONE,
        .first_subset_member = W_SEED_HIR0_NONE,
  ...
  if (has_u64_bool_tuple_type) {
    const size_t name_length = sizeof(HIR0_U64_BOOL_TUPLE_NAME) - 1u;
    (void)memcpy(output->text_bytes + text_offset,
                 HIR0_U64_BOOL_TUPLE_NAME, name_length);
    output->types[optional_type_index] = (w_seed_hir0_type){
        .kind = W_SEED_HIR0_TYPE_U64_BOOL_TUPLE,
        .owner_module = W_SEED_HIR0_NONE,
        .name = {(uint32_t)text_offset, (uint32_t)name_length},
        .external_module_index = W_SEED_HIR0_NONE,
        .external_symbol_index = W_SEED_HIR0_NONE,
        .enum_index = W_SEED_HIR0_NONE,
        .first_subset_member = W_SEED_HIR0_NONE,
        ...
        .release_contract = W_SEED_HIR0_RELEASE_CONTRACT_NONE};
    optional_type_index += 1u;
    text_offset += name_length;
  }
  if (has_public_process) {
    usize_type_index = optional_type_index;
    (void)memcpy(output->text_bytes + text_offset, HIR0_USIZE_NAME, 5u);
    output->types[optional_type_index] = (w_seed_hir0_type){
  ...
  for (size_t sign = 0u; sign < 2u; sign += 1u)
    for (size_t width = 0u; width < 3u; width += 1u) {
      ...
      if (!frontend_fixed_integer_present(input, is_signed, bit_width))
        continue;
      const char *name = hir0_fixed_integer_name(is_signed, bit_width);
      const size_t name_length = strlen(name);
      (void)memcpy(output->text_bytes + text_offset, name, name_length);
      output->types[optional_type_index] = (w_seed_hir0_type){
          .kind = W_SEED_HIR0_TYPE_INTEGER,
          .owner_module = W_SEED_HIR0_NONE,
          .name = {(uint32_t)text_offset, (uint32_t)name_length},
          .integer_is_signed = is_signed,
          .integer_bit_width = bit_width,
          .external_module_index = W_SEED_HIR0_NONE,
          .external_symbol_index = W_SEED_HIR0_NONE,
          .enum_index = W_SEED_HIR0_NONE,
          .first_subset_member = W_SEED_HIR0_NONE,
  ...
  for (size_t terminator = 0u; terminator < counts->terminators;
    ...
    if (output->terminators[terminator].kind !=
      ...
      output->terminators[terminator].call_index = W_SEED_HIR0_NONE;
    ...
    if (output->terminators[terminator].kind !=
      ...
      output->terminators[terminator].panic_code =
...
static void digest_counts(w_seed_sha256_state *state,
  ...
  digest_u64(state, counts->cleanups);
}
...
#define HIR0_RECORD_TAG(value) digest_u32(&state, (uint32_t)(value))
  ...
  for (size_t index = 0u; index < counts->types; index += 1u) {
    ...
    digest_u32(&state, value->owner_module);
    digest_text(&state, program, value->name);
    digest_bool(&state, value->integer_is_signed);
    digest_u32(&state, value->integer_bit_width);
  ...
  for (size_t index = 0u; index < counts->values; index += 1u) {
    ...
    digest_u64(&state, value->float_bits);
  ...
  for (size_t index = 0u; index < counts->cleanups; index += 1u) {
    const w_seed_hir0_cleanup *value = &program->cleanups[index];
    HIR0_RECORD_TAG(27u);
    digest_u32(&state, value->owner_function);
    digest_u32(&state, value->invoke_terminator);
    digest_u32(&state, value->cleanup_identity);
    digest_u32(&state, value->normal_block);
    digest_u32(&state, value->error_block);
    digest_u32(&state, value->normal_instruction);
    digest_u32(&state, value->error_instruction);
    digest_u32(&state, value->normal_call);
    digest_u32(&state, value->error_call);
  }
...
static void digest_provenance(const w_seed_hir0_program *program,
  ...
  for (size_t index = 0u; index < counts->cleanups; index += 1u)
    digest_span(&state, program->cleanups[index].source_span);
  w_seed_sha256_final(&state, digest);
}
...
#define HIR0_PROGRAM(field, count_field, capacity_field, type)                 \
  ...
  HIR0_PROGRAM(cleanups, cleanup_count, cleanup_capacity, w_seed_hir0_cleanup);
#undef HIR0_PROGRAM
...
/* TYPE_INTEGER records are a compact optional tail, emitted in the single
 * order unsigned 8/16/32 followed by signed 8/16/32.  Keep this structural
 * rule in one validator so type lookup and full verification cannot silently
 * accept a reordered or duplicated sign/width pair. */
static bool hir_integer_tail_layout(const w_seed_hir0_program *program,
                                    size_t subset_base,
                                    size_t *integer_type_base) {
  if (program == NULL || integer_type_base == NULL ||
      subset_base > program->type_count)
    return false;
  size_t tail = program->type_count;
  while (tail > subset_base &&
         program->types[tail - 1u].kind == W_SEED_HIR0_TYPE_INTEGER)
    tail -= 1u;
  for (size_t index = subset_base; index < tail; index += 1u)
  ...
  for (size_t index = tail; index < program->type_count; index += 1u) {
    ...
    if (type->kind != W_SEED_HIR0_TYPE_INTEGER ||
        ...
        !hir_text_is(program, type->name,
                     hir0_fixed_integer_name(type->integer_is_signed,
                                             type->integer_bit_width)))
      return false;
    if (type->integer_bit_width == 16u)
      width_index = 1u;
    ...
    if ((have_previous && rank <= previous_rank) ||
      ...
      return false;
    seen[type->integer_is_signed ? 1u : 0u][width_index] = true;
    previous_rank = rank;
    have_previous = true;
  }
  *integer_type_base = tail;
  return true;
}
...
static bool hir_enum_subset_layout(const w_seed_hir0_program *program,
  ...
  *u64_bool_tuple_type_index = W_SEED_HIR0_NONE;
  size_t tail = program->type_count;
  /* Generic fixed-integer records are appended after the historical
   * optional tail. Remove them before recovering the legacy optional indices
   * and enum-subset range. */
  if (!hir_integer_tail_layout(program, *subset_base, &tail)) return false;
  if (program->external_symbol_count == 7u) {
    if (tail == *subset_base) return false;
    *usize_type_index = (uint32_t)(tail - 1u);
    tail = *usize_type_index;
  }
  if (tail > *subset_base &&
      program->types[tail - 1u].kind ==
          W_SEED_HIR0_TYPE_U64_BOOL_TUPLE) {
    *u64_bool_tuple_type_index = (uint32_t)(tail - 1u);
    tail -= 1u;
  }
  if (tail > *subset_base &&
  ...
  if (tail > *subset_base &&
      program->types[tail - 1u].kind == W_SEED_HIR0_TYPE_F64) {
    *f64_type_index = (uint32_t)(tail - 1u);
    tail -= 1u;
  }
  if (tail > *subset_base &&
      program->types[tail - 1u].kind == W_SEED_HIR0_TYPE_U64) {
    *u64_type_index = (uint32_t)(tail - 1u);
    tail -= 1u;
  }
  if (tail < *subset_base) return false;
  *subset_count = tail - *subset_base;
  return true;
}
...
static bool hir_type_index_valid(const w_seed_hir0_program *program,
  ...
  if (type_index < 4u) {
    ...
    return type->kind == expected[type_index] &&
           ...
           type->first_subset_member == W_SEED_HIR0_NONE &&
           type->subset_member_count == 0u &&
           ((type_index == 2u && type->integer_is_signed &&
  ...
  if (u64_type_index != W_SEED_HIR0_NONE && type_index == u64_type_index)
    return type->kind == W_SEED_HIR0_TYPE_U64 &&
           type->owner_module == W_SEED_HIR0_NONE &&
           type->external_module_index == W_SEED_HIR0_NONE &&
           type->external_symbol_index == W_SEED_HIR0_NONE &&
           type->enum_index == W_SEED_HIR0_NONE &&
           ...
           type->subset_member_count == 0u &&
           !type->integer_is_signed && type->integer_bit_width == 64u &&
           hir_text_is(program, type->name, HIR0_U64_NAME);
  if (f64_type_index != W_SEED_HIR0_NONE && type_index == f64_type_index)
    return type->kind == W_SEED_HIR0_TYPE_F64 &&
           type->owner_module == W_SEED_HIR0_NONE &&
           type->external_module_index == W_SEED_HIR0_NONE &&
           type->external_symbol_index == W_SEED_HIR0_NONE &&
           type->enum_index == W_SEED_HIR0_NONE &&
  ...
  if (never_type_index != W_SEED_HIR0_NONE &&
    ...
    return type->kind == W_SEED_HIR0_TYPE_NEVER &&
           ...
           type->first_subset_member == W_SEED_HIR0_NONE &&
           type->subset_member_count == 0u &&
           hir_text_is(program, type->name, HIR0_NEVER_NAME);
  if (u64_bool_tuple_type_index != W_SEED_HIR0_NONE &&
      type_index == u64_bool_tuple_type_index)
    return type->kind == W_SEED_HIR0_TYPE_U64_BOOL_TUPLE &&
           type->owner_module == W_SEED_HIR0_NONE &&
           type->external_module_index == W_SEED_HIR0_NONE &&
           type->external_symbol_index == W_SEED_HIR0_NONE &&
           type->enum_index == W_SEED_HIR0_NONE &&
           ...
           type->lifecycle == W_SEED_HIR0_LIFECYCLE_VALUE_COPY &&
           type->release_contract == W_SEED_HIR0_RELEASE_CONTRACT_NONE &&
           hir_text_is(program, type->name, HIR0_U64_BOOL_TUPLE_NAME);
  size_t integer_type_base = program->type_count;
  if (!hir_integer_tail_layout(program, subset_base, &integer_type_base))
    return false;
  if (type_index >= integer_type_base && type_index < program->type_count) {
...
static bool hir_integer_type_facts(const w_seed_hir0_program *program,
...
static bool hir_integer_types_equal(const w_seed_hir0_program *program,
  ...
  return hir_integer_type_facts(program, left, &left_signed, &left_width) &&
         hir_integer_type_facts(program, right, &right_signed, &right_width) &&
  ...
  for (size_t block_index = 0u; block_index < program->block_count;
    ...
    for (size_t ordinal = 0u; ordinal < block->block_argument_count;
      ...
      if (argument->owner_block != block_index ||
          ...
          !hir_type_index_valid(program, argument->type_index) ||
          (argument->type_index < 4u && argument->type_index != 2u &&
...
/* usize is an optional legacy record, so generic fixed-integer records may
 * follow it. Never infer its index from type_count's final slot. */
static uint32_t hir0_usize_type_index(const w_seed_hir0_program *program) {
  if (program == NULL || program->type_count > UINT32_MAX)
    return W_SEED_HIR0_NONE;
  for (size_t index = 4u; index < program->type_count; index += 1u)
...
static bool hir0_usize_count_comparison_operands(
  ...
  if (usize_type == W_SEED_HIR0_NONE) return false;
...
static bool verify_value_tree(
  ...
  if (value->kind == W_SEED_HIR0_VALUE_TUPLE_ELEMENT) {
    if (value->unsigned_integer_value > 1u ||
        !hir_type_index_valid(program, value->type_index) ||
        program->types[value->type_index].kind !=
            (value->unsigned_integer_value == 0u ? W_SEED_HIR0_TYPE_U64
    ...
    if ((size_t)root_index != *value_cursor ||
        !hir_type_index_valid(program, tuple->type_index) ||
  ...
  if (value->kind == W_SEED_HIR0_VALUE_BINARY_FLOAT) {
    const bool comparison =
        ...
        value->binary_operator <= W_SEED_HIR0_BINARY_GREATER_EQUAL;
    if (value->binary_operator > W_SEED_HIR0_BINARY_GREATER_EQUAL ||
  ...
  if (value->kind == W_SEED_HIR0_VALUE_BINARY_I64) {
    ...
    const bool generic_wrapping_types_ok =
        ...
        hir_integer_type_facts(program, value->type_index,
                               &generic_result_signed, &generic_result_width) &&
        generic_result_signed && generic_result_width != 0u &&
        ((value->binary_operator == W_SEED_HIR0_BINARY_WRAPPING_POWER ||
          value->binary_operator == W_SEED_HIR0_BINARY_WRAPPING_SHIFT_LEFT)
             ? (program->values[value->right_value].type_index <
                    program->type_count &&
  ...
  if (value->kind == W_SEED_HIR0_VALUE_BINARY_U64) {
    ...
    if (wrapping || overflowing) {
      ...
      const bool generic_integer_types_ok =
          integer_wrapping &&
          hir_integer_type_facts(program, value->type_index, &result_signed,
                                 &result_width) &&
          !result_signed &&
          hir_integer_types_equal(
              program, value->type_index,
  ...
  if (value->kind == W_SEED_HIR0_VALUE_UNARY_I64) {
    ...
    const bool generic_wrapping =
        ...
        hir_integer_type_facts(program, value->type_index, &result_signed,
                               &result_width) &&
        result_signed && value->left_value < program->value_count &&
        hir_integer_types_equal(program, value->type_index,
                                program->values[value->left_value].type_index);
    const bool ordinary_signed_integer =
        !wrapping &&
        hir_integer_type_facts(program, value->type_index, &result_signed,
                               &result_width) &&
        result_signed;
    if ((value->unary_operator != W_SEED_HIR0_UNARY_NEGATE &&
         value->unary_operator != W_SEED_HIR0_UNARY_BIT_NOT && !wrapping) ||
  ...
  if (value->kind == W_SEED_HIR0_VALUE_CALL_RESULT) {
    ...
    const bool valid_integer = hir_integer_type_facts(
        program, value->type_index, &is_signed, &bit_width);
    int64_t minimum = INT64_MIN;
    int64_t maximum = INT64_MAX;
    if (bit_width != 64u && bit_width != 0u) {
      const int64_t bound = INT64_C(1) << (bit_width - 1u);
      minimum = -bound;
      maximum = bound - 1;
    }
    if (!valid_integer || !is_signed || value->integer_value < minimum ||
        value->integer_value > maximum ||
        value->binding_index != W_SEED_HIR0_NONE ||
    ...
    const uint64_t maximum =
        bit_width == 64u
            ? UINT64_MAX
            : (bit_width == 0u ? 0u : (UINT64_C(1) << bit_width) - 1u);
    if (!valid_integer || is_signed || value->unsigned_integer_value > maximum ||
        value->binding_index != W_SEED_HIR0_NONE ||
        value->parameter_index != W_SEED_HIR0_NONE ||
        value->integer_value != 0 || value->bool_value ||
        value->byte_offset != 0u || value->byte_count != 0u)
...
static bool verify_process_exit_or_panic(
    const w_seed_hir0_program *program, size_t function_index,
    size_t block_index, uint32_t exit_code_type) {
  if (program == NULL || function_index >= program->function_count ||
      block_index >= program->block_count ||
      block_index >= program->terminator_count)
    return false;
  const w_seed_hir0_block *block = &program->blocks[block_index];
  const w_seed_hir0_terminator *term = &program->terminators[block_index];
  if (block->owner_function != function_index ||
      block->block_argument_count != 0u || term->owner_block != block_index ||
      term->target_block != W_SEED_HIR0_NONE ||
      term->else_block != W_SEED_HIR0_NONE ||
      term->first_edge_argument != W_SEED_HIR0_NONE ||
      term->edge_argument_count != 0u)
    return false;
  if (term->kind == W_SEED_HIR0_TERMINATOR_PANIC)
    return term->result_type < program->type_count &&
           program->types[term->result_type].kind == W_SEED_HIR0_TYPE_NEVER;
  return term->kind == W_SEED_HIR0_TERMINATOR_RETURN_VALUE &&
...
static bool verify_cfg_process_terminal_branch(
  ...
  if (branch_block->owner_function != function_index ||
      ...
      !verify_process_exit_or_panic(program, function_index, then_index,
                                    exit_code_type) ||
      !verify_process_exit_or_panic(program, function_index, else_index,
                                    exit_code_type))
    return false;
  return true;
}
...
static bool verify_cfg_terminal_region(const w_seed_hir0_program *program,
  ...
  return start + 1u == end &&
         (term->kind == W_SEED_HIR0_TERMINATOR_RETURN_UNIT ||
          ...
          term->kind == W_SEED_HIR0_TERMINATOR_PANIC);
}

/* Cleanup records are static cross-record evidence.  Keep this validator
 * independent of CFG recursion so a forged record cannot create a second
 * route, alias a terminator call, or hide an argument-bearing/non-Unit call
 * behind the ordinary instruction checks. */
static bool verify_cleanup_record_shape(const w_seed_hir0_program *program,
                                        size_t cleanup_index) {
  if (program == NULL || cleanup_index >= program->cleanup_count ||
      program->cleanup_count != 1u)
    return false;
  const w_seed_hir0_cleanup *cleanup = &program->cleanups[cleanup_index];
  if (cleanup->owner_function >= program->function_count ||
      cleanup->invoke_terminator >= program->terminator_count ||
      cleanup->normal_block >= program->block_count ||
      cleanup->error_block >= program->block_count ||
    ...
    return false;
  const w_seed_hir0_function *owner =
      &program->functions[cleanup->owner_function];
  if (owner->module_index >= program->module_count) return false;
  const size_t invoke_block = cleanup->invoke_terminator;
  const size_t normal_block = cleanup->normal_block;
  const size_t error_block = cleanup->error_block;
  if (invoke_block != owner->first_block || normal_block != invoke_block + 1u ||
      error_block != invoke_block + 2u || error_block >= program->block_count ||
      program->blocks[invoke_block].owner_function != cleanup->owner_function ||
      program->blocks[normal_block].owner_function != cleanup->owner_function ||
      program->blocks[error_block].owner_function != cleanup->owner_function)
    return false;
  const w_seed_hir0_terminator *invoke = &program->terminators[invoke_block];
  if (invoke->owner_block != invoke_block ||
      invoke->kind != W_SEED_HIR0_TERMINATOR_INVOKE ||
      invoke->target_block != normal_block || invoke->else_block != error_block ||
      invoke->call_index == W_SEED_HIR0_NONE ||
  ...
  if (normal->instruction_count != 1u || error->instruction_count != 1u ||
  ...
  if (normal_instruction->kind != W_SEED_HIR0_INSTRUCTION_CALL ||
      ...
      normal_instruction->ordinal != 0u || error_instruction->ordinal != 0u)
    return false;
  ...
  if (cleanup->normal_call == cleanup->error_call ||
      cleanup->normal_call == invoke->call_index ||
      cleanup->error_call == invoke->call_index ||
      normal_call->owner_instruction != cleanup->normal_instruction ||
      ...
      normal_call->first_requirement != W_SEED_HIR0_NONE ||
      normal_call->requirement_count != 0u ||
      error_call->first_requirement != W_SEED_HIR0_NONE ||
      error_call->requirement_count != 0u || normal_call->result_type != 0u ||
      error_call->result_type != 0u ||
      normal_call->callee_identity != cleanup->cleanup_identity ||
      error_call->callee_identity != cleanup->cleanup_identity ||
      normal_call->source_expression != error_call->source_expression ||
      !span_equal(normal_call->source_span, error_call->source_span) ||
      !span_valid(normal_call->source_span,
                  program->modules[owner->module_index].source_length) ||
      !span_valid(error_call->source_span,
                  program->modules[owner->module_index].source_length))
    return false;
  if (normal_call->callee_identity >= program->identity_count)
    return false;
  const w_seed_hir0_identity *identity =
  ...
  if (identity->kind != W_SEED_HIR0_IDENTITY_FUNCTION ||
      ...
      identity->parameter_count != 0u || identity->return_type != 0u ||
      identity->first_requirement != W_SEED_HIR0_NONE ||
      identity->requirement_count != 0u)
    return false;
  const w_seed_hir0_function *target = &program->functions[identity->target_index];
  if (target->module_index != owner->module_index || target->is_async ||
      target->is_throws || target->is_unsafe || target->has_borrow_clause ||
      target->is_anonymous_entry || target->parameter_count != 0u ||
      target->return_type != 0u ||
      identity->owner_module != target->module_index ||
      identity->first_parameter != target->first_parameter)
    return false;
  if (!span_valid(cleanup->source_span,
                  program->modules[owner->module_index].source_length) ||
      cleanup->source_span.start_byte > normal_call->source_span.start_byte ||
      cleanup->source_span.end_byte < normal_call->source_span.end_byte)
    return false;
  return true;
}

static bool verify_cleanup_records(const w_seed_hir0_program *program) {
  if (program == NULL) return false;
  if (program->cleanup_count == 0u) return true;
  return program->cleanup_count == 1u &&
         verify_cleanup_record_shape(program, 0u);
}

/* A synchronous typed propagation slice is an invoke-owned three-block
 * region.  The call descriptor is attached to the first block's INVOKE
 * terminator; its normal and error successors each receive exactly one
 * typed block argument and finish the function.  Keeping this proof separate
 * from the structured branch verifier prevents a throwing call from being
 * mistaken for an ordinary completed CALL instruction. */
static bool verify_cfg_invoke(const w_seed_hir0_program *program,
                              size_t function_index) {
  if (program == NULL || function_index >= program->function_count)
    return false;
  const w_seed_hir0_function *function = &program->functions[function_index];
  if (function->block_count != 3u || program->block_count < 3u ||
      function->first_block > program->block_count - 3u)
    return false;
  const size_t invoke_block = function->first_block;
  const size_t normal_block = invoke_block + 1u;
  const size_t error_block = invoke_block + 2u;
  if (program->blocks[invoke_block].owner_function != function_index ||
      program->blocks[normal_block].owner_function != function_index ||
      program->blocks[error_block].owner_function != function_index)
    return false;
  const w_seed_hir0_block *invoke = &program->blocks[invoke_block];
  const w_seed_hir0_block *normal = &program->blocks[normal_block];
  const w_seed_hir0_block *error = &program->blocks[error_block];
  const w_seed_hir0_terminator *invoke_term =
  ...
  const w_seed_hir0_cleanup *cleanup = NULL;
  if (program->cleanup_count != 0u) {
    if (!verify_cleanup_records(program) ||
        program->cleanups[0].owner_function != function_index ||
        program->cleanups[0].invoke_terminator != invoke_block)
      return false;
    cleanup = &program->cleanups[0];
  }
  const uint32_t cleanup_instruction_count = cleanup == NULL ? 0u : 1u;
  if (!function->is_throws || function->error_type == W_SEED_HIR0_NONE ||
      !hir_type_index_valid(program, function->return_type) ||
      (function->return_type < 4u && function->return_type != 2u &&
       function->return_type != 3u) ||
      !hir_type_index_valid(program, function->error_type) ||
      program->types[function->error_type].kind != W_SEED_HIR0_TYPE_ENUM ||
      invoke->instruction_count != 0u ||
      normal->instruction_count != cleanup_instruction_count ||
      error->instruction_count != cleanup_instruction_count ||
      invoke->block_argument_count != 0u ||
      invoke->first_block_argument != W_SEED_HIR0_NONE ||
      invoke_term->owner_block != invoke_block ||
      invoke_term->kind != W_SEED_HIR0_TERMINATOR_INVOKE ||
      invoke_term->ordinal != invoke->instruction_count ||
      invoke_term->call_index == W_SEED_HIR0_NONE ||
      invoke_term->call_index >= program->call_count ||
      invoke_term->value_index != W_SEED_HIR0_NONE ||
      invoke_term->result_type != function->return_type ||
      invoke_term->error_type != function->error_type ||
      invoke_term->target_block != normal_block ||
      invoke_term->else_block != error_block ||
      invoke_term->first_edge_argument != W_SEED_HIR0_NONE ||
      invoke_term->edge_argument_count != 0u ||
      invoke_term->logical_operator != W_SEED_HIR0_LOGICAL_NONE ||
      invoke_term->switch_enum_index != W_SEED_HIR0_NONE ||
      invoke_term->first_switch_edge != W_SEED_HIR0_NONE ||
      invoke_term->switch_edge_count != 0u ||
      invoke_term->switch_carrier_width != 0u)
    return false;
  if (normal->block_argument_count != 1u ||
      normal->first_block_argument == W_SEED_HIR0_NONE ||
      error->block_argument_count != 1u ||
      error->first_block_argument == W_SEED_HIR0_NONE ||
  ...
  if (normal_argument->owner_block != normal_block ||
      ...
      !span_equal(normal_argument->source_span, invoke_term->source_span) ||
      !span_equal(error_argument->source_span, invoke_term->source_span))
    return false;
  if (normal_term->owner_block != normal_block ||
      ...
      normal_term->else_block != W_SEED_HIR0_NONE ||
      normal_term->first_edge_argument != W_SEED_HIR0_NONE ||
      normal_term->edge_argument_count != 0u ||
      normal_term->logical_operator != W_SEED_HIR0_LOGICAL_NONE ||
      normal_term->switch_enum_index != W_SEED_HIR0_NONE ||
      normal_term->first_switch_edge != W_SEED_HIR0_NONE ||
      normal_term->switch_edge_count != 0u ||
      normal_term->switch_carrier_width != 0u ||
      ...
      error_term->call_index != W_SEED_HIR0_NONE ||
      error_term->error_type != W_SEED_HIR0_NONE ||
      error_term->target_block != W_SEED_HIR0_NONE ||
      error_term->else_block != W_SEED_HIR0_NONE ||
      error_term->first_edge_argument != W_SEED_HIR0_NONE ||
      error_term->edge_argument_count != 0u ||
      error_term->logical_operator != W_SEED_HIR0_LOGICAL_NONE ||
      error_term->switch_enum_index != W_SEED_HIR0_NONE ||
      error_term->first_switch_edge != W_SEED_HIR0_NONE ||
      error_term->switch_edge_count != 0u ||
      error_term->switch_carrier_width != 0u)
    return false;
  const w_seed_hir0_call *call = &program->calls[invoke_term->call_index];
  ...
  return target->module_index == function->module_index &&
         target->is_throws && !target->is_async &&
         target->error_type == function->error_type &&
...
static bool verify_cfg_function(const w_seed_hir0_program *program,
  ...
  if (function->first_block < program->block_count &&
      program->terminators[function->first_block].kind ==
          W_SEED_HIR0_TERMINATOR_INVOKE)
    return verify_cfg_invoke(program, function_index);
  if (function->first_block < program->block_count &&
      program->terminators[function->first_block].kind ==
...
static uint32_t hir0_call_owner_function(const w_seed_hir0_program *program,
  ...
  if (call->owner_instruction == W_SEED_HIR0_NONE) {
    if (call->owner_terminator == W_SEED_HIR0_NONE ||
        call->owner_terminator >= program->terminator_count ||
        call->owner_block >= program->block_count)
      return W_SEED_HIR0_NONE;
    const w_seed_hir0_terminator *terminator =
        &program->terminators[call->owner_terminator];
    if (terminator->kind != W_SEED_HIR0_TERMINATOR_INVOKE ||
        terminator->call_index != call_index ||
        terminator->owner_block != call->owner_block)
      return W_SEED_HIR0_NONE;
    ...
    if (segment->kind == W_SEED_HIR0_INTERPOLATION_TEXT) {
      ...
      if (child->type_index >= program->type_count ||
          (program->types[child->type_index].kind != W_SEED_HIR0_TYPE_I64 &&
           program->types[child->type_index].kind != W_SEED_HIR0_TYPE_U64 &&
           program->types[child->type_index].kind != W_SEED_HIR0_TYPE_INTEGER &&
...
static bool hir0_static_yield_scalar_type(
    const w_seed_hir0_program *program, uint32_t type_index) {
  if (program == NULL || type_index >= program->type_count) return false;
  const w_seed_hir0_type_kind kind = program->types[type_index].kind;
  return kind == W_SEED_HIR0_TYPE_UNIT || kind == W_SEED_HIR0_TYPE_I64 ||
         kind == W_SEED_HIR0_TYPE_INTEGER || kind == W_SEED_HIR0_TYPE_BOOL;
}
...
static bool hir0_static_yield_helper_graph(
    const w_seed_hir0_program *program, uint32_t function_index,
    uint32_t owner_module, const uint8_t *body_never, uint8_t *state,
    size_t depth) {
  if (program == NULL || body_never == NULL || state == NULL ||
      function_index >= program->function_count ||
      function_index >= W_SEED_FRONTEND_MAX_CST_NODES ||
      depth >= W_SEED_HIR0_MAX_NESTING)
    return false;
  if (state[function_index] == 2u) return true;
  if (state[function_index] != 0u) return false;
  state[function_index] = 1u;

  const w_seed_hir0_function *function = &program->functions[function_index];
  bool valid = function->module_index == owner_module &&
               !function->is_async && !function->is_throws &&
               !function->is_unsafe && !function->has_borrow_clause &&
               !function->is_anonymous_entry &&
               hir0_function_bit_get(body_never, function_index) &&
               hir0_static_yield_scalar_type(program, function->return_type) &&
               range_valid(function->first_parameter,
  ...
  for (size_t ordinal = 0u; valid && ordinal < function->parameter_count;
    ...
    const size_t parameter_index =
        (size_t)function->first_parameter + ordinal;
    valid = hir0_static_yield_scalar_type(
        program, program->parameters[parameter_index].type_index);
  }
  for (size_t block_ordinal = 0u;
       valid && block_ordinal < function->block_count; block_ordinal += 1u) {
    ...
    for (size_t ordinal = 0u; valid && ordinal < block->instruction_count;
         ordinal += 1u) {
      ...
      if (instruction->kind == W_SEED_HIR0_INSTRUCTION_BINDING) {
        valid = instruction->binding_index < program->binding_count &&
                hir0_static_yield_scalar_type(
                    program,
      ...
      if (call->owner_block != block_index ||
          ...
          call->execution_kind != W_SEED_HIR0_CALL_DIRECT ||
          call->callee_identity >= program->identity_count ||
          !hir0_static_yield_scalar_type(program, call->result_type)) {
        valid = false;
        break;
      }
      const w_seed_hir0_identity *identity =
      ...
      if (identity->kind != W_SEED_HIR0_IDENTITY_FUNCTION ||
          ...
          !hir0_static_yield_helper_graph(
              program, identity->target_index, owner_module, body_never, state,
              depth + 1u))
        valid = false;
    }
  }
 ...
 * async scalar function, one block, one or more yield instructions, and a
 * bounded acyclic graph of local pure scalar helpers. The yields remain
 * explicit in verified HIR but a closed launch/join consumer may choose a
 * legal serial schedule and erase them. */
static bool hir0_function_static_yields(
  ...
  for (size_t ordinal = 0u; ordinal < block->instruction_count; ordinal += 1u) {
    ...
    if (instruction->kind == W_SEED_HIR0_INSTRUCTION_EXECUTION_YIELD) {
      ...
      if (identity->kind != W_SEED_HIR0_IDENTITY_FUNCTION ||
          identity->target_index >= program->function_count ||
          !hir0_static_yield_helper_graph(program, identity->target_index,
                                          function->module_index, body_never,
                                          helper_state, 0u))
        return false;
    } else {
      return false;
...
static bool hir0_function_cooperative_yields(
    const w_seed_hir0_program *program, uint32_t function_index) {
  if (!hir0_function_static_yields(program, function_index) ||
      program == NULL || function_index >= program->function_count)
    return false;
  const w_seed_hir0_function *function = &program->functions[function_index];
  if (function->first_block >= program->block_count) return false;
  const w_seed_hir0_block *block = &program->blocks[function->first_block];
  size_t yield_count = 0u;
  for (size_t ordinal = 0u; ordinal < block->instruction_count; ordinal += 1u)
    if (program->instructions[(size_t)block->first_instruction + ordinal].kind ==
 ...
 * `.main` and caller-bound `.domain` lanes are bounded by the verified HIR
 * record counts; the historical cooperative trace lane remains exact-two. This
...
static bool hir0_physical_task_scope(const w_seed_hir0_program *program) {
  ...
  if ((!root->is_anonymous_entry && !process_root) ||
    ...
    return false;
  const uint32_t root_block_index = root->first_block;
  ...
  uint32_t previous_call_instruction = W_SEED_HIR0_NONE;
  uint32_t previous_launch_binding = W_SEED_HIR0_NONE;
  uint32_t previous_join_binding = W_SEED_HIR0_NONE;
  uint8_t body_never[HIR0_DIRECT_FUNCTION_BITSET_BYTES];
  ...
  for (size_t call_index = 0u; call_index < program->call_count;
    ...
    if (launch_instruction->kind != W_SEED_HIR0_INSTRUCTION_BINDING ||
        launch_instruction->owner_block != call->owner_block ||
    ...
    const w_seed_hir0_binding *launch =
        &program->bindings[launch_binding_index];
    if (launch->task_role != W_SEED_HIR0_TASK_ROLE_LAUNCH ||
        launch->task_peer_binding == W_SEED_HIR0_NONE ||
    ...
    if (launch_value->kind != W_SEED_HIR0_VALUE_CALL_RESULT ||
      ...
      return false;
    const uint32_t join_binding_index = launch->task_peer_binding;
    if (join_binding_index >= program->binding_count ||
        join_binding_index == launch_binding_index)
      return false;
    const w_seed_hir0_binding *join = &program->bindings[join_binding_index];
    if (join->task_role != W_SEED_HIR0_TASK_ROLE_AWAIT_RESULT ||
        join->task_peer_binding != launch_binding_index ||
        join->owner_block != call->owner_block ||
        join->owner_instruction <= launch->owner_instruction ||
        join->owner_instruction >= program->instruction_count)
      return false;
    if (physical_count != 0u &&
        (previous_call_instruction >= instruction_index ||
         previous_launch_binding >= launch_binding_index ||
         previous_join_binding >= join_binding_index))
      return false;
    previous_call_instruction = (uint32_t)instruction_index;
    previous_launch_binding = launch_binding_index;
    previous_join_binding = join_binding_index;
    physical_count += 1u;
  }
  ...
  if (physical_count != required_tasks)
    return false;
  /* Rewalk the one root block in instruction order. Launch and join bindings
   * must match the exact physical call immediately preceding each launch, and
   * all joins must follow every launch in the same order. No branch, loop,
   * root yield, arbitrary call, or extra binding may hide behind the scope. */
  size_t physical_seen = 0u;
  size_t launch_seen = 0u;
  size_t join_seen = 0u;
  uint32_t previous_join_launch_binding = W_SEED_HIR0_NONE;
  ...
  for (size_t ordinal = 0u; ordinal < root_block->instruction_count;
       ordinal += 1u) {
    const size_t instruction_index =
        (size_t)root_block->first_instruction + ordinal;
    if (instruction_index >= program->instruction_count) return false;
    const w_seed_hir0_instruction *instruction =
        &program->instructions[instruction_index];
    if (instruction->owner_block != root_block_index ||
        instruction->ordinal != ordinal)
      return false;
    if (instruction->kind == W_SEED_HIR0_INSTRUCTION_EXECUTION_YIELD)
      return false;
    if (instruction->kind == W_SEED_HIR0_INSTRUCTION_BINDING) {
      ...
      if (binding->task_role == W_SEED_HIR0_TASK_ROLE_NONE && process_root &&
        ...
        if (join_seen != 0u || launch_seen >= required_tasks ||
            ...
            program->instructions[instruction_index - 1u].call_index >=
                program->call_count ||
            !hir0_call_is_physical_dispatch(
                program->calls[program->instructions[instruction_index - 1u]
                                   .call_index]
                    .execution_kind))
          return false;
        launch_seen += 1u;
        ...
        if (launch_seen != required_tasks || join_seen >= required_tasks ||
            ...
            (join_seen != 0u &&
             previous_join_launch_binding >= binding->task_peer_binding))
          return false;
        previous_join_launch_binding = binding->task_peer_binding;
        join_seen += 1u;
      } else {
...
static bool verify_records(const w_seed_hir0_program *program) {
  ...
  if (program->module_count == 0u || program->function_count == 0u ||
      ...
      program->types[0].integer_is_signed ||
      program->types[0].integer_bit_width != 0u ||
      program->types[1].integer_is_signed ||
      program->types[1].integer_bit_width != 0u ||
      !program->types[2].integer_is_signed ||
  ...
  if (has_external_process) {
    ...
    if (program->external_symbol_count == 7u) {
      const uint32_t usize_type = hir0_usize_type_index(program);
      if (usize_type == W_SEED_HIR0_NONE ||
          !hir_type_index_valid(program, usize_type))
        return false;
    }
  }
  ...
  size_t ordinary_call_count = 0u;
  size_t invoke_call_cursor = 0u;
  ...
  for (size_t instruction = 0u; instruction < program->instruction_count;
    ...
    if (value->kind == W_SEED_HIR0_INSTRUCTION_CALL) {
      while (call_instruction_cursor < program->call_count &&
             program->calls[call_instruction_cursor].owner_instruction ==
                 W_SEED_HIR0_NONE)
  ...
  if (call_instruction_cursor > program->call_count ||
      ...
      ordinary_call_count + binding_instruction_cursor +
  ...
  for (size_t call = 0u; call < program->call_count; call += 1u) {
    ...
    if ((!invoke_call && value->owner_instruction >= program->instruction_count) ||
        (invoke_call && (value->owner_terminator == W_SEED_HIR0_NONE ||
                         value->owner_terminator >= program->terminator_count)) ||
        value->owner_block >= program->block_count ||
    ...
    if (invoke_call) {
      const w_seed_hir0_terminator *terminator =
          &program->terminators[value->owner_terminator];
      if (value->owner_terminator != value->owner_block ||
          terminator->owner_block != value->owner_block ||
          terminator->kind != W_SEED_HIR0_TERMINATOR_INVOKE ||
          terminator->call_index != call ||
          value->ordinal != program->blocks[value->owner_block]
                                  .instruction_count)
        return false;
    } else {
      if (value->owner_terminator != W_SEED_HIR0_NONE)
        return false;
      instruction = &program->instructions[value->owner_instruction];
      if (instruction->kind != W_SEED_HIR0_INSTRUCTION_CALL ||
    ...
    if ((!host_call && !local_call) ||
        identity->parameter_count != value->argument_count ||
        identity->return_type != value->result_type)
      return false;
    if (invoke_call &&
        (value->execution_kind != W_SEED_HIR0_CALL_DIRECT || !local_call ||
         value->argument_count != 0u || value->first_requirement != W_SEED_HIR0_NONE))
      return false;
    if (host_call) {
    ...
    if (local_call) {
      const w_seed_hir0_function *target =
          identity->target_index < program->function_count
              ...
              : NULL;
      const w_seed_hir0_function *owner_function =
          &program->functions[program->blocks[value->owner_block]
                                   .owner_function];
      if (target == NULL ||
          (invoke_call
               ? (!owner_function->is_throws || !target->is_throws ||
                  target->is_async ||
                  target->error_type != owner_function->error_type)
               : target->is_throws))
        return false;
    }
  ...
  for (size_t terminator = 0u; terminator < program->terminator_count;
    ...
    if (value->owner_block != terminator ||
        ...
        !span_valid(value->source_span, source_length))
      return false;
    if (value->kind != W_SEED_HIR0_TERMINATOR_INVOKE &&
        (value->call_index != W_SEED_HIR0_NONE ||
         value->error_type != W_SEED_HIR0_NONE))
      return false;
    ...
    if (value->kind == W_SEED_HIR0_TERMINATOR_INVOKE) {
      if (value->call_index == W_SEED_HIR0_NONE ||
          ...
          value->result_type == W_SEED_HIR0_NONE ||
          !hir_type_index_valid(program, value->result_type) ||
          (value->result_type < 4u && value->result_type != 2u &&
           value->result_type != 3u) ||
          value->error_type == W_SEED_HIR0_NONE ||
          !hir_type_index_valid(program, value->error_type) ||
          program->types[value->error_type].kind != W_SEED_HIR0_TYPE_ENUM ||
          ...
          value->edge_argument_count != 0u)
        return false;
      invoke_call_cursor += 1u;
      continue;
    }
    if (value->kind == W_SEED_HIR0_TERMINATOR_PANIC) {
      if (value->panic_code != W_SEED_HIR0_PANIC_CODE_EXPLICIT ||
          value->call_index != W_SEED_HIR0_NONE ||
          value->value_index == W_SEED_HIR0_NONE ||
          value->value_index >= program->value_count ||
          value->result_type == W_SEED_HIR0_NONE ||
          !hir_type_index_valid(program, value->result_type) ||
          program->types[value->result_type].kind !=
              W_SEED_HIR0_TYPE_NEVER ||
          value->target_block != W_SEED_HIR0_NONE ||
          value->else_block != W_SEED_HIR0_NONE ||
          value->first_edge_argument != W_SEED_HIR0_NONE ||
          ...
          program->values[value->value_index].byte_count >
              W_SEED_HIR0_MAX_VALUE_BYTES)
        return false;
      continue;
    }
  ...
  if (invoke_call_cursor > program->call_count ||
...
bool w_seed_hir0_program_from_output(const w_seed_hir0_output *output,
  ...
  w_seed_hir0_program candidate = {
      ...
      .cleanups = output->cleanups,
      .cleanup_count = counts.cleanups,
      .cleanup_capacity = output->cleanup_capacity};
...
bool w_seed_hir0_verify(const w_seed_hir0_program *program,
  ...
  w_seed_hir0_counts counts = {
      ...
      .cleanups = program->cleanup_count};
  if (result->required.modules != counts.modules ||
      ...
      result->required.cleanups != counts.cleanups ||
