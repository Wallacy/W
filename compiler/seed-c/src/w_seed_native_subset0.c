...
/* NativeSubset0 admits only the existing immutable, flat two-i64 product
 * representations.  This is a private value carrier, not a public layout or
 * calling-convention promise. */
static bool program_flat_product_type_supported(
    const w_seed_hir0_program *program, uint32_t type_index) {
  if (program == NULL || type_index >= program->type_count) return false;
  const w_seed_hir0_type *type = &program->types[type_index];
...
static bool program_has_flat_aggregate_records(
    const w_seed_hir0_program *program) {
  if (program == NULL) return false;
  for (size_t index = 0u; index < program->type_count; index += 1u)
    if (program->types[index].kind == W_SEED_HIR0_TYPE_TUPLE ||
        program->types[index].kind == W_SEED_HIR0_TYPE_VALUE_STRUCT)
      return true;
  for (size_t index = 0u; index < program->value_count; index += 1u)
    if (program->values[index].kind == W_SEED_HIR0_VALUE_TUPLE ||
        program->values[index].kind == W_SEED_HIR0_VALUE_TUPLE_ELEMENT ||
  ...
  for (size_t ordinal = 0u; ordinal < value->interpolation_segment_count;
    ...
    if (segment->kind == W_SEED_HIR0_INTERPOLATION_TEXT) {
      ...
      const bool runtime_value =
          ...
          effective->kind == W_SEED_HIR0_VALUE_VALUE_STRUCT_FIELD ||
          effective->kind ==
...
static bool program_value_lowerable(const w_seed_hir0_program *program,
  ...
  if (value->kind == W_SEED_HIR0_VALUE_PARAMETER_READ) {
    ...
    return (scalar || enumeration ||
            program_flat_product_type_supported(program, value->type_index)) &&
  ...
  if (value->kind == W_SEED_HIR0_VALUE_TUPLE_ELEMENT) {
    ...
    if (tuple->type_index >= program->type_count) return false;
    if (program->types[tuple->type_index].kind ==
        W_SEED_HIR0_TYPE_TUPLE) {
      const w_seed_hir0_type *tuple_type = &program->types[tuple->type_index];
      const uint32_t ordinal = value->projection_ordinal;
      return ordinal < 2u &&
             program_flat_product_type_supported(program, tuple->type_index) &&
             tuple_type->first_tuple_component <=
                 program->tuple_component_count &&
             program->tuple_components[(size_t)tuple_type->first_tuple_component +
                                      ordinal]
    ...
    return (type == W_SEED_HIR0_TYPE_U64 || type == W_SEED_HIR0_TYPE_BOOL) &&
  ...
  if (value->kind == W_SEED_HIR0_VALUE_TUPLE) {
    if (type != W_SEED_HIR0_TYPE_TUPLE ||
        !program_flat_product_type_supported(program, value->type_index) ||
    ...
    for (size_t ordinal = 0u; ordinal < 2u; ordinal += 1u) {
      const w_seed_hir0_tuple_element *element =
          &program->tuple_elements[(size_t)value->first_tuple_element + ordinal];
      if (element->owner_value != value_index || element->ordinal != ordinal ||
          ...
          !program_value_lowerable(program, element->value_index,
                                   owner_function, false, depth + 1u))
        return false;
    }
    return true;
  ...
  if (value->kind == W_SEED_HIR0_VALUE_VALUE_STRUCT) {
    if (type != W_SEED_HIR0_TYPE_VALUE_STRUCT ||
        !program_flat_product_type_supported(program, value->type_index) ||
    ...
    for (size_t ordinal = 0u; ordinal < 2u; ordinal += 1u) {
      ...
      if (initializer->owner_value != value_index ||
          ...
          !program_value_lowerable(program, initializer->value_index,
                                   owner_function, false, depth + 1u))
        return false;
  ...
  if (value->kind == W_SEED_HIR0_VALUE_VALUE_STRUCT_FIELD) {
    ...
    if (receiver->type_index >= program->type_count ||
        ...
        !program_flat_product_type_supported(program, receiver->type_index))
      return false;
...
static bool program_verified_i64_loop_cfg_is_supported(
  ...
  if (program == NULL || function_index >= program->function_count)
    return false;
  const w_seed_hir0_function *function = &program->functions[function_index];
  if (program->types == NULL || function->return_type >= program->type_count ||
  ...
  const size_t start = function->first_block;
  const size_t end = start + function->block_count;
  size_t indegree[W_SEED_NATIVE_SUBSET0_MAX_BLOCKS] = {0u};
  ...
  for (size_t block_index = start; block_index < end; block_index += 1u) {
    ...
    if (block->owner_function != function_index ||
        ...
        (block->block_argument_count != 0u &&
         ((size_t)block->first_block_argument >
              program->block_argument_count ||
          block->block_argument_count >
              program->block_argument_count - block->first_block_argument)))
      return false;
    ...
    for (size_t instruction_ordinal = 0u;
      ...
      const w_seed_hir0_binding *binding =
          &program->bindings[instruction->binding_index];
      if (binding->owner_instruction != instruction_index ||
          binding->owner_block != block_index ||
          binding->type_index >= program->type_count ||
          program->types[binding->type_index].kind != W_SEED_HIR0_TYPE_I64 ||
          binding->initializer_value >= program->value_count ||
          program->values[binding->initializer_value].type_index !=
              binding->type_index ||
          !program_value_lowerable(program, binding->initializer_value,
                                   (uint32_t)function_index, false, 0u))
        return false;
    }
    ...
    if (term->kind == W_SEED_HIR0_TERMINATOR_BRANCH) {
      if (term->value_index >= program->value_count ||
          ...
          program->types[program->values[term->value_index].type_index].kind !=
              W_SEED_HIR0_TYPE_BOOL ||
          !program_value_lowerable(program, term->value_index,
                                   (uint32_t)function_index, false, 0u))
        return false;
      if (indegree[(size_t)term->target_block - start] == SIZE_MAX ||
          indegree[(size_t)term->else_block - start] == SIZE_MAX)
        return false;
      indegree[(size_t)term->target_block - start] += 1u;
      indegree[(size_t)term->else_block - start] += 1u;
      branch_count += 1u;
      continue;
    }
    if (term->kind == W_SEED_HIR0_TERMINATOR_JUMP) {
      if (term->target_block < start || term->target_block >= end ||
          term->else_block != W_SEED_HIR0_NONE ||
          (term->edge_argument_count == 0u
               ? term->first_edge_argument != W_SEED_HIR0_NONE
               : term->first_edge_argument == W_SEED_HIR0_NONE) ||
          (term->edge_argument_count != 0u &&
           ((size_t)term->first_edge_argument >
                program->edge_argument_count ||
            term->edge_argument_count >
                program->edge_argument_count - term->first_edge_argument)) ||
          term->edge_argument_count !=
              program->blocks[term->target_block].block_argument_count ||
          indegree[(size_t)term->target_block - start] == SIZE_MAX)
        return false;
      indegree[(size_t)term->target_block - start] += 1u;
      for (size_t ordinal = 0u; ordinal < term->edge_argument_count;
           ordinal += 1u) {
        const size_t edge_index =
            (size_t)term->first_edge_argument + ordinal;
        const w_seed_hir0_edge_argument *edge =
            &program->edge_arguments[edge_index];
        const w_seed_hir0_block *target =
            &program->blocks[term->target_block];
        const w_seed_hir0_block_argument *argument =
            &program->block_arguments[(size_t)target->first_block_argument +
                                      ordinal];
        if (edge->owner_terminator != block->terminator_index ||
            edge->owner_block != block_index || edge->ordinal != ordinal ||
            edge->type_index != argument->type_index ||
            edge->type_index >= program->type_count ||
            program->types[edge->type_index].kind != W_SEED_HIR0_TYPE_I64 ||
            edge->value_index >= program->value_count ||
            program->values[edge->value_index].type_index != edge->type_index ||
            !program_value_lowerable(program, edge->value_index,
                                     (uint32_t)function_index, false, 0u))
          return false;
      }
      continue;
  ...
  for (size_t local = 0u; local < function->block_count; local += 1u)
    if (indegree[local] == 0u) ready[ready_count++] = (uint32_t)local;
  size_t visited = 0u;
  for (size_t cursor = 0u; cursor < ready_count; cursor += 1u) {
    const size_t block_index = start + ready[cursor];
    const w_seed_hir0_terminator *term =
        &program->terminators[program->blocks[block_index].terminator_index];
    visited += 1u;
    if (term->kind == W_SEED_HIR0_TERMINATOR_JUMP) {
      const size_t target = (size_t)term->target_block - start;
      if (indegree[target] == 0u) return false;
      indegree[target] -= 1u;
      if (indegree[target] == 0u) ready[ready_count++] = (uint32_t)target;
    } else if (term->kind == W_SEED_HIR0_TERMINATOR_BRANCH) {
      const size_t then_target = (size_t)term->target_block - start;
      const size_t else_target = (size_t)term->else_block - start;
      if (indegree[then_target] == 0u || indegree[else_target] == 0u)
        return false;
      indegree[then_target] -= 1u;
      indegree[else_target] -= 1u;
      if (indegree[then_target] == 0u)
        ready[ready_count++] = (uint32_t)then_target;
      if (indegree[else_target] == 0u)
        ready[ready_count++] = (uint32_t)else_target;
    }
  }
  return visited < function->block_count;
}
...
static bool program_function_maximum(
  ...
  if (function->return_type >= program->type_count ||
      (!(program->types[function->return_type].kind ==
         ...
         program_enum_type_supported(program, function->return_type, NULL,
                                      NULL) ||
         (!process_entry &&
          program_flat_product_type_supported(program,
                                              function->return_type))) &&
       !(process_entry &&
  ...
  for (size_t parameter = 0u; parameter < function->parameter_count;
    ...
    const bool scalar_or_enum =
        ...
        program_enum_type_supported(program, type_index, NULL, NULL) ||
        (!process_entry &&
         program_flat_product_type_supported(program, type_index));
  ...
  if (verified_i64_loop_cfg) {
    if (process_entry) return false;
    /* The backend admission requires binding-only loop functions, so there is
     * no repeated or unbounded stdout contribution to the entry buffer. */
    cached[function_index] = 0u;
    state[function_index] = 2u;
    return true;
  }
...
w_seed_native_subset0_status w_seed_native_subset0_select_program(
  ...
  bool verified_i64_loop_cfg_functions[
      W_SEED_NATIVE_SUBSET0_MAX_FUNCTIONS] = {false};
  ...
  for (size_t function = 0u; function < program->function_count;
       function += 1u)
    verified_i64_loop_cfg_functions[function] =
        program_verified_i64_loop_cfg_is_supported(program, function);
  for (size_t function = 0u; function < program->function_count;
       function += 1u)
  ...
  *selection = (w_seed_native_subset0_program){
  ...
  (void)memcpy(selection->verified_i64_loop_cfg_functions,
               verified_i64_loop_cfg_functions,
               sizeof(verified_i64_loop_cfg_functions));
...
w_seed_native_subset0_status w_seed_native_subset0_select_process(
  ...
  if (program == NULL || hir_result == NULL || selection == NULL ||
      !w_seed_hir0_verify(program, hir_result))
    return W_SEED_NATIVE_SUBSET0_INVALID;
  if (program_has_flat_aggregate_records(program))
    return W_SEED_NATIVE_SUBSET0_UNSUPPORTED;
...
static bool process_float_source_diamond_supported(
    const w_seed_hir0_program *program,
    const w_seed_native_subset0_process *process,
    process_float_diamond_facts *facts) {
  if (program == NULL || process == NULL || process->function == NULL ||
      facts == NULL || process->function->block_count != 7u)
    return false;
  const w_seed_hir0_function *function = process->function;
  if (function->first_block > program->block_count ||
      function->block_count > program->block_count - function->first_block)
  ...
  const uint32_t arguments_parameter_index =
      function->first_parameter + process->arguments_parameter_ordinal;
  if (branch_block->owner_function != process->function_index ||
      ...
      then_block->owner_function != process->function_index ||
      then_block->ordinal != 1u || then_block->instruction_count != 0u ||
      then_block->block_argument_count != 0u ||
      then_block->terminator_index >= program->terminator_count ||
      else_block->owner_function != process->function_index ||
      else_block->ordinal != 2u || else_block->instruction_count != 0u ||
      else_block->block_argument_count != 0u ||
      else_block->terminator_index >= program->terminator_count ||
      join_block->owner_function != process->function_index ||
  ...
  if (branch->owner_block != branch_index ||
      branch->kind != W_SEED_HIR0_TERMINATOR_BRANCH ||
      branch->ordinal != 0u || branch->value_index >= program->value_count ||
      branch->target_block != then_index || branch->else_block != else_index ||
      branch->edge_argument_count != 0u ||
      then_jump->owner_block != then_index ||
  ...
  const uint32_t arm_blocks[2] = {then_index, else_index};
  const uint32_t arm_terminators[2] = {then_block->terminator_index,
                                       else_block->terminator_index};
  for (size_t ordinal = 0u; ordinal < 2u; ordinal += 1u) {
    const w_seed_hir0_edge_argument *edge = edges[ordinal];
    if (edge->owner_terminator != arm_terminators[ordinal] ||
        edge->owner_block != arm_blocks[ordinal] || edge->ordinal != 0u ||
...
/* Admit the original constant-source process root or the bounded
 * runtime-selected f64 diamond and publish the normal/error edges. This is a
 * separate relation from INTEGER_EXACTLY; both typed error arms are consumed
 * by the process cleanup contract. */
static bool process_float_to_integer_rounding_root_supported(
  ...
  if (!function->is_throws || function->error_type >= program->type_count ||
      ...
      program->edge_argument_count != (function->block_count == 7u ? 2u : 0u))
    return false;

  const bool has_diamond = function->block_count == 7u;
  process_float_diamond_facts diamond = {0};
  if (has_diamond &&
      !process_float_source_diamond_supported(program, process, &diamond))
    return false;
  ...
  if (conversion->owner_block != split_index ||
      ...
      (has_diamond &&
       (normal_index != function->first_block + 4u ||
        non_finite_index != function->first_block + 5u ||
        out_of_range_index != function->first_block + 6u)))
    return false;
  ...
  for (size_t value_index = 0u; value_index < program->value_count;
    ...
    const w_seed_hir0_value *value = &program->values[value_index];
    if (value->type_index >= program->type_count) return false;
    const w_seed_hir0_type_kind kind = program->types[value->type_index].kind;
    if (kind == W_SEED_HIR0_TYPE_F32 || kind == W_SEED_HIR0_TYPE_F64) {
      const bool selected_arm =
          has_diamond &&
