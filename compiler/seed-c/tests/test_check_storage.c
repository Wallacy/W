...
static bool test_growth_preserves_each_arena(void) {
  ...
  CHECK(storage.staging_capacity[2u] == 8u &&
        storage.revalidation_capacity[2u] == 8u &&
        storage.published_capacity[2u] == 8u);
  CHECK(storage.staging_total == 8u && storage.revalidation_total == 8u &&
        storage.published_total == 8u);
  for (size_t index = 0u; index < 7u; index += 1u) {
    storage.staging_bytes[2u][index] = (uint8_t)(index + 1u);
    storage.revalidation_bytes[2u][index] = (uint8_t)(index + 11u);
    storage.published_bytes[2u][index] = (uint8_t)(index + 21u);
  }
  CHECK(w_seed_check_storage_grow(&storage, 2u, 9u) ==
        W_SEED_CHECK_STORAGE_OK);
  for (size_t index = 0u; index < 7u; index += 1u) {
    CHECK(storage.staging_bytes[2u][index] == (uint8_t)(index + 1u));
    CHECK(storage.revalidation_bytes[2u][index] ==
          (uint8_t)(index + 11u));
    CHECK(storage.published_bytes[2u][index] == (uint8_t)(index + 21u));
  }
  CHECK(w_seed_check_storage_bind_request(&storage, 2u, &request));
  CHECK(request.staging_bytes == storage.staging_bytes[2u] &&
        request.staging_capacity == 16u);
  CHECK(request.revalidation_bytes == storage.revalidation_bytes[2u] &&
        request.revalidation_capacity == 16u);
  CHECK(request.bytes == storage.published_bytes[2u] &&
        request.byte_capacity == 16u);
  CHECK(storage.staging_total == 16u &&
        storage.revalidation_total == 16u && storage.published_total == 16u);
  uint8_t *stable_staging = storage.staging_bytes[2u];
  uint8_t *stable_revalidation = storage.revalidation_bytes[2u];
  uint8_t *stable_published = storage.published_bytes[2u];
  CHECK(w_seed_check_storage_grow(&storage, 2u, 12u) ==
        W_SEED_CHECK_STORAGE_OK);
  CHECK(storage.staging_bytes[2u] == stable_staging &&
        storage.revalidation_bytes[2u] == stable_revalidation &&
        storage.published_bytes[2u] == stable_published &&
        storage.staging_capacity[2u] == 16u &&
        storage.revalidation_capacity[2u] == 16u &&
        storage.published_capacity[2u] == 16u);
  w_seed_check_storage_destroy(&storage);
  return true;
}
...
static bool test_growth_keeps_requests_disjoint(void) {
  ...
  CHECK(storage.staging_total == 8u && storage.revalidation_total == 8u &&
        storage.published_total == 8u);
  CHECK(storage.staging_bytes[0u] != storage.staging_bytes[1u]);
  CHECK(storage.revalidation_bytes[0u] != storage.revalidation_bytes[1u]);
  CHECK(storage.published_bytes[0u] != storage.published_bytes[1u]);
  CHECK(storage.staging_bytes[0u] != storage.revalidation_bytes[0u] &&
        storage.staging_bytes[0u] != storage.published_bytes[0u]);
  w_seed_check_storage_destroy(&storage);
  return true;
}
...
static bool test_node_growth_preserves_data_and_binds_slot(void) {
  ...
  CHECK(w_seed_check_storage_grow_nodes(&storage, 2u, 7u) ==
        W_SEED_CHECK_STORAGE_OK);
  CHECK(storage.node_capacity[2u] == 8u && storage.node_total == 8u);
  (void)memset(storage.nodes[2u], 0x5Au,
               7u * sizeof(storage.nodes[2u][0]));
  CHECK(w_seed_check_storage_grow_nodes(&storage, 2u, 9u) ==
        W_SEED_CHECK_STORAGE_OK);
  CHECK(storage.node_capacity[2u] == 16u && storage.node_total == 16u);
  for (size_t index = 0u; index < 7u; index += 1u) {
    const uint8_t *bytes = (const uint8_t *)&storage.nodes[2u][index];
  ...
  CHECK(slot.nodes == storage.nodes[2u] && slot.node_capacity == 16u);
  w_seed_check_storage_destroy(&storage);
  return true;
}
...
static bool test_capacity_is_atomic(void) {
  ...
  CHECK(storage.staging_capacity[0u] == 8u &&
        storage.revalidation_capacity[0u] == 8u &&
        storage.published_capacity[0u] == 8u);
  CHECK(w_seed_check_storage_grow(
...
static bool test_aggregate_capacity_is_separate_and_atomic(void) {
  ...
  CHECK(storage.staging_bytes[0u] == old_staging &&
        storage.revalidation_bytes[0u] == old_revalidation &&
        storage.published_bytes[0u] == old_published);
  CHECK(storage.staging_total ==
            (size_t)W_SEED_CHECK_STORAGE_MAX_TOTAL_SOURCE_BYTES &&
        storage.revalidation_total ==
            (size_t)W_SEED_CHECK_STORAGE_MAX_TOTAL_SOURCE_BYTES &&
        storage.published_total ==
            (size_t)W_SEED_CHECK_STORAGE_MAX_TOTAL_SOURCE_BYTES);
  w_seed_check_storage_destroy(&storage);
  return true;
}
...
static bool test_aggregate_limit_uses_exact_fallback(void) {
  ...
  CHECK(w_seed_check_storage_grow(&storage, 2u, mebibyte) ==
        W_SEED_CHECK_STORAGE_OK);
  CHECK(storage.staging_total == 13u * mebibyte);
  CHECK(w_seed_check_storage_grow(&storage, 3u, 3u * mebibyte) ==
        W_SEED_CHECK_STORAGE_OK);
  CHECK(storage.staging_capacity[3u] == 3u * mebibyte &&
        storage.revalidation_capacity[3u] == 3u * mebibyte &&
        storage.published_capacity[3u] == 3u * mebibyte);
  CHECK(storage.staging_total ==
            (size_t)W_SEED_CHECK_STORAGE_MAX_TOTAL_SOURCE_BYTES &&
        storage.revalidation_total ==
            (size_t)W_SEED_CHECK_STORAGE_MAX_TOTAL_SOURCE_BYTES &&
...
static bool test_allocation_failure_is_atomic(void) {
  ...
  CHECK(storage.staging_total == 4u && storage.revalidation_total == 4u &&
        storage.published_total == 4u);
  for (size_t index = 0u; index < 4u; index += 1u) {
    CHECK(storage.staging_bytes[0u][index] == (uint8_t)(index + 1u));
    CHECK(storage.revalidation_bytes[0u][index] ==
          (uint8_t)(index + 11u));
    CHECK(storage.published_bytes[0u][index] == (uint8_t)(index + 21u));
  }
