...
int main(int argc, char **argv) {
  ...
  w_seed_wrt0_artifact count_only = {NULL, 0u};
  w_seed_wrt0_artifact proven_full = {NULL, 0u};
  if (!w_seed_wrt0_get(W_SEED_WRT0_TARGET_LINUX_X86_64,
                       W_SEED_RUNTIME_REQUIREMENTS_NONE, &compact) ||
      !w_seed_wrt0_get(
          W_SEED_WRT0_TARGET_LINUX_X86_64,
          W_SEED_RUNTIME_REQUIREMENTS_PROCESS_ARGUMENT_COUNT, &count_only) ||
      !w_seed_wrt0_get(W_SEED_WRT0_TARGET_LINUX_X86_64,
                       W_SEED_RUNTIME_REQUIREMENTS_PROCESS_ARGUMENTS,
                       &proven_full) ||
      compact.llvm_ir == NULL || compact.llvm_ir_length == 0u ||
      count_only.llvm_ir == NULL || count_only.llvm_ir_length == 0u ||
      compact.llvm_ir_length >= artifact.llvm_ir_length ||
      count_only.llvm_ir == artifact.llvm_ir ||
      count_only.llvm_ir_length >= artifact.llvm_ir_length ||
      ...
      !contains(&count_only, "declare i32 @main(i64)") ||
      !contains(&count_only, "w_seed_linux_count_only_start") ||
      !contains(&count_only, "load i64, ptr %stack, align 8") ||
      !contains(&count_only, "call i32 @main(i64 %argc)") ||
      contains(&count_only, "@w_seed_linux_initial_argc") ||
      contains(&count_only, "@w_seed_linux_initial_argv") ||
      contains(&count_only, "@w_seed_process_argc") ||
      contains(&count_only, "@w_seed_process_argv") ||
      contains(&count_only, "store i64 %argc") ||
      contains(&count_only, "store ptr %argv"))
    return 1;
  ...
  if (w_seed_wrt0_get((w_seed_wrt0_target)99,
      ...
      w_seed_wrt0_get(W_SEED_WRT0_TARGET_LINUX_X86_64,
                      W_SEED_RUNTIME_REQUIREMENTS_UNKNOWN, &rejected) ||
      rejected.llvm_ir != sentinel.llvm_ir ||
      rejected.llvm_ir_length != sentinel.llvm_ir_length ||
      w_seed_wrt0_get(W_SEED_WRT0_TARGET_LINUX_X86_64,
                      (w_seed_runtime_requirements)99, &rejected) ||
      rejected.llvm_ir != sentinel.llvm_ir ||
      rejected.llvm_ir_length != sentinel.llvm_ir_length ||
      w_seed_wrt0_get(W_SEED_WRT0_TARGET_LINUX_X86_64,
